function run_dpo_catalog(stage, maxCellsIn, batchSecIn)
%% Purpose:
%
%   FRONT DOOR for the DPO -> TULIP costate catalog: the proven three-gate
%   pipeline (direct solve -> ms_tfmin refinement -> pumpkyn tfMin
%   acceptance), walked down a thrust ladder, swept over (DPO period x
...
%   resumable.
%
%   The DPO box comes from the measured admissibility survey
%   (DPO_tulip/direct/results/dpo_bounds.mat, via run_dpo_bounds; Darin's
%   criteria: >= 500 km periselene, within 100 Mm of the Moon):
%     - admissible for periods tau 0.048-4.695 ND (0.21-20.8 days),
%       583 native catalog members; the upper boundary is the 100 Mm
%       criterion (DPOs grow with period), the long-period tail (tau > ~9)
%       also fails the periodicity guard.
%
%   Endpoints are built through costate_common/get_family_orbit, so this
%   front door differs from the halo catalog's ONLY in the departure-family
...
%% Outputs:
%
%   none (files under DPO_tulip/direct/results/catalog/):
%     dpo_tau<t>_Np<n>.mat + _progress.txt   one per sheet
%
%   Unattended:  ./run_dpo_batched.sh (same hang-proof batch pattern as
%   HALO_tulip/run_halo_batched.sh)
%
%% Revision History:
...
%% Departure: DPOs selected by period (measured box 0.048-4.695 ND):
   dpoTauList = [1.0 2.0 3.0 4.0];   % periods (ND; 1 ND = 4.43 d)
...
%% Solver / gates (identical to the DRO and halo catalogs):
...
%% Pilot: move each orbit axis separately, few cells each:
  pilotSheets = [2.0 7;              % mid-box DPO, known tulip
                 3.0 9];             % different DPO period AND petal count
pilotCellsMax = 6;
...
sheetName = @(tau,Np) fullfile(catDir, sprintf('dpo_tau%s_Np%d', ...
...
mkOpts = @(tau,Np,mc,bs) struct('rungs',rungs, 'ispS',ispS, 'm0kg',m0kg, ...
    ...
    'depFamily','dpo', 'depParams',struct('tau',tau), ...
...
case 'report'
    fprintf('\nDPO->TULIP CENSUS (%d x %d sheets):\n', ...
            numel(dpoTauList), numel(NpList));
    for tau = dpoTauList
        for Np = NpList
            f = [sheetName(tau,Np) '.mat'];
            if isfile(f)
                Q = load(f);
                fprintf('  dpo tau=%.2f Np=%2d: %4d entries, %2d/%d pairs, %2d full ladders\n', ...
                    tau, Np, nnz(Q.OK), nnz(any(Q.OK,3)), nD*nA, ...
                    nnz(sum(Q.OK,3)==numel(Q.rungs)));
            else
                fprintf('  dpo tau=%.2f Np=%2d: (not started)\n', tau, Np);
            end
        end
    end
