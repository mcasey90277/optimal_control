...
| 10 | **continuous-time accuracy gate (G1) and method-agreement gate (G2)** — re-integrate each interval at 1e-11/1e-13 from its own left node and compare at the right node; tolerance stated in KILOMETRES, not solver units. G2 compares t_f against an independently converged indirect solution where one exists | `DRO_tulip/direct/certify/certify_dro_mintime.m` + `dro_residual.m` + `tests/test_dro_residual.m` (built 2026-08-02, DRO→tulip only; **generalizing it to the other three campaigns is the obvious next move**) |
...
| condition | status |
|---|---|
| **(a) state dynamics** satisfied along the trajectory | YES — machine-tight, but **at the collocation nodes, to collocation order**. Not the same as satisfying the continuous ODE; see the caveat below. **MEASURED 2026-08-02 on DRO→tulip and the gap is not academic: NLP defect 1.4e-14 against a TRUE continuous local error of 1.5 ND (~1e5 km), a ratio of 1e7.** The residual scales as altitude^-4.24, matching trapezoidal LTE. Second-order collocation never resolved that transfer at any density tried (12,600 km at N=400, 1,100 km at N=1600). Fourth-order Hermite-Simpson at N=1600 reaches 0.32 km and CERTIFIES. Instrument: `DRO_tulip/direct/certify/dro_residual.m` (self-tested 3/3). **Every campaign in this register quotes 1e-14 defects as evidence of a good solve; none has measured this.** |
