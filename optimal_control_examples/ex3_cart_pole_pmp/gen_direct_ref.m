...
[Z, J, exitflag, output, lambda] = fmincon(obj, Z0, [], [], [], [], lb, ub, nlc, opts);
% A positive exit flag alone is not evidence of a stationary point (flag 2 is
% "step too small"): the fixture seeds a costate solve, so its multipliers
% must come from a point that is FEASIBLE and first-order optimal.
assert(exitflag > 0, 'gen_direct_ref:fmincon', 'fmincon exit flag %d', exitflag);
assert(output.constrviolation < 1e-9, 'gen_direct_ref:infeasible', ...
       'constraint violation %.2e', output.constrviolation);
assert(output.firstorderopt < 1e-4, 'gen_direct_ref:notStationary', ...
...
fprintf(['gen_direct_ref: J = %.6f, max|u| = %.2f N, exit flag %d, first-order optimality %.2e, ' ...
         'constraint violation %.2e, %d iterations\n'], ...
        J, max(abs(U)), exitflag, output.firstorderopt, output.constrviolation, output.iterations);
end
