...
$\texttt{cScale}\approx1.005$), an order of magnitude below the basin spreads
but real. Every $m_f$ in this guide from the fixed-$\tau_f$ engine is a lower
bound on the free-$\tau_f$ optimum by about $2\times10^{-4}$; the honest fix is
the sparse slack state, which keeps the KKT matrix banded and is already what
the min-time anchor and the thrust ladder use.
...
$m_f$ values are lower bounds on the free-$\tau_f$ optima by $\sim2\times10^{-4}$
(measured 2026-09-06 on two rows). The rows are genuine extremals of the
