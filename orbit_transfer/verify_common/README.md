...
| `foc_ipopt_inertia.m` | Interprets IPOPT's `δ_w` Hessian-regularization history: converge with `δ_w=0` and the reduced Hessian is PD without correction. LEAD-0 port of the tulip campaign's `ipopt_certify`, which was the one second-order instrument that actually delivered. |
