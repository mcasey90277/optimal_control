function [z, resNorm, out] = mintime_solve(rv0, rvf, zGuess, Tmax, c, muStar, maxEvals)
% MINTIME_SOLVE  Min-time CR3BP TPBVP: pumpkyn's analytic-STM residual at REAL
% solver budgets.  (Our own real-budget wrapper; module-local copy of the
% ztl rebuild -- pumpkyn.cr3bp.tfMin itself hardcodes 100 evals and stalls
% hard multi-rev cases.)
%
% Rebuilds exactly the residual/Jacobian of pumpkyn.cr3bp.tfMin's shooting
% residual -- rendezvous (6), lambda_m(tf)=0, H(tf)=0 (free-final-time
% transversality) -- with dR/dx from the propagated 14x14 STM
% (pumpkyn.cr3bp.tfMinProp, ode45 RelTol 1e-10, throttle-switch event restarts)
% -- and solves it with lsqnonlin trust-region-reflective at a real budget.
%
