...
$\Tvec$), \eqref{eq:midstat} forces $\Tvec_{k+1/2}$ exactly parallel to
$\lamv$ at the \emph{discrete} level, with no discretization error at all.
...
the midpoint, the same solutions and the same duals give $1.2\times
10^{-6}$~degrees at \emph{every} grid, in vacuum and under drag alike ---
which is not a physical number but the double-precision floor of
$\arccos$ near unity, $\arccos(1-\epsilon) = 1.207\times10^{-6}$~degrees.
The alignment is exact to floating point.
