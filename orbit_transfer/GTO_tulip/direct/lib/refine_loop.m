...
%              propTol [default 1e-4 kg], outDir where the history .mat and
%              summary figure are written [default this folder], solFile if
%              nonempty the FINAL refined solution is saved there in the seed
%              layout (out, factor, tauf0, sigma, rv0, rvf) so downstream
%              verification / animation can consume it [default '' = skip],
%              and pass-through indicator/refiner opts
%              M/epsEval/mode/nbr/K/hFloor/maxAdd
%
% OUTPUTS:
...
% outDir defaults to the campaign RESULTS tree, not to this file's folder.
% It used to default to `here` -- i.e. library code wrote its history .mat and
% figure into the library directory beside its own source. Harmless until the
% 2026-07-26 flatten put that directory under lib/, at which point solver
% artifacts started landing among the solver code.
if ~isfield(opts, 'outDir')
    rcfg = minfuel_config();
    opts.outDir = rcfg.dirs.root;
end
if ~isfield(opts, 'solFile'),   opts.solFile = '';       end
...
tf  = out.X(8, end);
tmpFile = fullfile(opts.outDir, sprintf('.refine_tmp_%s.mat', opts.tag));
...
for r = 1:(opts.maxRounds + 1)
    ...
    save(fullfile(opts.outDir, sprintf('refine_history_%s.mat', opts.tag)), 'history');
...
% persist the FINAL refined solution (seed layout) for downstream stages
if ~isempty(opts.solFile)
    write_seed(opts.solFile, out, factor, tauf0, sigma, rv0, rvf);
    fprintf('refine_loop: wrote refined solution %s\n', opts.solFile);
end

make_figure(history, diag, opts.outDir, opts.tag);
end
