...
they cross the extremal near $t_c$ (green), well before $b = 5$. Second strip: the
difference $y(t;p+\delta) - y_0(t)$, whose zeros are the crossings. Third row: the
...
slope range you give it, and for the one you select it draws the six panels of
Figure~\ref{fig:explorer} and writes a readout with a verdict. The slider changes
the slope perturbation $\delta$; \emph{Shrink} animates $\delta \to 0$. The
checkbox \emph{divide by delta} rescales the difference strip to
$(y(t;p+\delta) - y_0(t))/\delta$, which is the quantity that becomes the Jacobi
field $h$ as $\delta \to 0$ (drawn dashed beside it).
...
where it crosses $y_0$ (\file{cov\_perturbed.m}). The difference strip shows the
same event as a zero of $y(t;p+\delta) - y_0(t)$; divided by $\delta$ that
difference converges to $h$, which is how views~1 and~2 become one.
...
first order). Now tick \emph{divide by delta}: the four rescaled differences lie
close to the dashed Jacobi field and approach it as $\delta$ shrinks (their
largest distance from $h$ on $[0,5]$ is $0.116, 0.078, 0.045, 0.024$ for
$\delta = 0.6, 0.3, 0.15, 0.075$). On the oscillator presets they sit exactly on
it, for any $\delta$. Extremal 1: $y'(0) = -1.834052$, $J = 3.690547$, first zero
of $h$ at $6.636 > b$: a weak local minimiser, and the lower of the two costs.
\end{chk}
\end{expt}
