...
xf       = d('xf', [1;0;0;0;0]);
ipx      = d('ipoptExtra', struct());   % opt-in IPOPT overrides forwarded per step
fp       = d('fp', struct());
...
for ke = 1:numel(sched)
    ...
    o = casadi_lt_mee(sigma, Xk, Uk, dLk, struct('par', opts.par, ...
        ...
        'liftDL', liftDLh, 'scaleNLP', scaleNLPh, 'ipoptExtra', ipx));
    ...
    for ns = 1:maxSolves
        o = casadi_lt_mee(sigma, Xk, Uk, dLk, struct('par', opts.par, ...
            ...
            'xf',xf,'liftDL',liftDLh,'scaleNLP',scaleNLPh,'ipoptExtra',ipx));
        ok = o.success && o.maxDefect < 1e-8;
