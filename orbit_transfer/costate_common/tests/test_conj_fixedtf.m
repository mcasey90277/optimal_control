...
% (g) a root on the bracket ending at t_f. LQ with T = pi + tiny: the
%     crossing sits at pi, inside the last bracket. The determinant at t_f
%     is small but RESOLVED (sin(1e-3), nowhere near roundoff), so opposite
%     nonzero signs put the zero strictly inside (t_K, t_f): a conjugate
%     point, FAIL (Astra review 2026-09-11). Only when the final sign cannot
%     be trusted does the verdict fall back to ENDPOINT.
K = 48;  T = pi + 1e-3;
info = lqInfo(T, K);
out = ms_conjugate_test(info, spec);
ok = chk(ok, ~out.atFinal && out.nInterior == 1 && out.nEndResolved == 1 && strcmp(out.verdict, 'FAIL') && ~out.pass, ...
         sprintf('resolved root in the last bracket -> FAIL, interior (atFinal=%d nInterior=%d verdict=%s)', ...
                 out.atFinal, ternary(isfield(out,'nInterior'), out.nInterior, -1), ...
                 ternary(isfield(out,'verdict'), out.verdict, '?')));
% with EVERY sign declared untrustworthy (resolvedTol = Inf) nothing can be
% concluded either way: UNDETERMINED, with the unresolved count reported
% (Astra review #2, 2026-09-11: an unresolved sample is never a PASS)
spec2 = spec;  spec2.resolvedTol = Inf;
out2 = ms_conjugate_test(info, spec2);
ok = chk(ok, out2.atFinal && out2.nInterior == 0 && strcmp(out2.verdict, 'UNDETERMINED') && ~out2.pass ...
             && out2.nUnresolved == numel(out2.detScaled) - out2.firstFullRank + 1, ...
         sprintf('the same with all signs untrusted -> UNDETERMINED (atFinal=%d nInterior=%d nUnresolved=%d verdict=%s)', ...
                 out2.atFinal, out2.nInterior, out2.nUnresolved, out2.verdict));
