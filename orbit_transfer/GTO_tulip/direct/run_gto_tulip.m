% RUN_GTO_TULIP  THE front door: min-fuel GTO -> south-pole tulip (CR3BP).
%
% Set the parameters in section 1, run, and get a solved transfer plus whichever
% of the optional stages you asked for. This is the ONE entry point for the
...
% PSR/ folder that sat beside sundman_minfuel/ as if they were alternatives,
% made the campaign look like it had two competing pipelines. It has one.
% (A second, redundant front door was briefly added the same day under
% sundman_minfuel/ before this was noticed; its guards were folded in here and
% it was deleted.)
%
% One run of this script does:
%
...
factor = 1.80;                   % t_f / t_f_min. Backbones exist 1.12-1.95;
                                 ...
                                 %   the reference when this became THE front door.)
...
%seedSpec   = '/Users/msc/Desktop/optimal_control/orbit_transfer/GTO_tulip/direct/PSR_data/psr_data_tf1p150_sw20_minEps0p2.mat';
%seedSpec   = '/Users/msc/Desktop/optimal_control/orbit_transfer/GTO_tulip/direct/PSR_data/psr_data_tf1p150_sw22_minEps0p1.mat';
...
% Programmatic override hook (sweep/batch drivers, house run_cr3bp_geo pattern):
% if the caller's workspace defines TULIP_OVERRIDES (struct), its fields replace
% the parameters set above. Absent -> byte-identical interactive behaviour. This
% is what run_tulip_front.sh drives; run_one.m remains the function-call route
% for callers that want a return value.
    ...
    if ~isfile(ef)
        ...
        error('run_gto_tulip:noBackbone', ...
            ['no energy backbone at factor %.3f (looked for %s).\n' ...
             'Backbones on disk: %s\n' ...
             'Below ~1.12 this is the OPEN near-min-time band, not a missing run: ' ...
             'the energy backbone will not converge there. Inside the band, ' ...
             'generate one with gen_tulip_energy_2p / orchestrate/backbone_walk.sh, ' ...
             'or set seedSpec = ''neighbor'' with seedFactor at a solved factor.'], ...
...
% Which optimum did we land in? MEASURED 2026-07-26: at factor 1.150 the
% energy-backbone route and the run_certified_minfuel chain both converge
% machine-tight to 25 switches at the SAME t_f -- and land on DIFFERENT local
% optima (m_f 0.847086 vs 0.849066; dV 3.4176 vs 3.3696 km/s, +1.43%). Both are
% valid extremals; the SEED ROUTE decides which. Without this line a run at the
% flagship factor reports numbers 1.4% off the campaign headline and looks
% perfectly healthy. Higher m_f is better (min fuel = max final mass).
certRef = fullfile(here, 'lib', 'sundman_minfuel_certified.mat');
if isfile(certRef)
    K = load(certRef);
    if isfield(K,'out') && isfield(K.out,'X') && ...
            abs(K.out.X(8,end) - outDirect.X(8,end)) < 1e-9      % same t_f
        dmf = outDirect.mf - K.out.mf;
        fprintf('[stage 2] basin check vs certified reference (same t_f): m_f %+.6f', dmf);
        if     dmf < -1e-6, fprintf('  -> WORSE optimum; quote run_certified_minfuel\n');
        elseif dmf >  1e-6, fprintf('  -> BETTER than the banked reference; investigate\n');
        else,               fprintf('  -> same optimum\n');
        end
    end
end
