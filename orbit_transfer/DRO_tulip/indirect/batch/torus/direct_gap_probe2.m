...
logF = fullfile(D, 'results', 'direct_gap_probe2.txt');
...
for sA = targets
    ...
    for s = seeds
        if s.col == 18 && abs(sA - 0.7837) > 1e-6, continue, end     % seed 18 only proved itself at its own phase
        ...
        try
            ...
            rec.o = o;                                        % FIRST: nothing below may lose it
        ...
        save(fullfile(D, 'results', 'direct_gap_probe2.mat'), 'P', 'targets', 'seedCols', 'N', 'floorKm');
    end
end
