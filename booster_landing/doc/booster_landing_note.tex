...
%
Magnitudes are written as $\sqrt{\sum(\cdot)^2 + \varepsilon}$ with
$\varepsilon = 10^{-12}$, rather than as \texttt{norm}/\texttt{abs},
throughout the implementation, so that the dynamics function stays
complex-step differentiable and stays defined at $\lVert\vvec\rVert = 0$
and $\lVert\Tvec\rVert = 0$ without a \texttt{max} guard. Both
implementations of the dynamics --- the plain-MATLAB
\texttt{pdg\_dynamics} and the symbolic CasADi twin inside
\texttt{solve\_pdg\_colloc} --- use the same $\varepsilon$, so they smooth
identically. It is invisible in flight: $\lVert\Tvec\rVert \sim 10^5$~N and
$\lVert\vvec\rVert \sim 10^2$~m/s here, making the guard $\sim10^{-22}$
relative. Analytic Jacobians
...
Table~\ref{tab:params} is the complete \emph{physical and PDG} parameter
set --- vehicle, boundary conditions, path constraints, discretization,
mission criteria and atmosphere --- and every trajectory number in this note
follows from it plus the two solvers. The remaining configuration lives in
three other tables and is cross-referenced where it is used: the
Monte-Carlo dispersions and seed (Table~\ref{tab:dispersions}), the tracker
weights and schedule (\S\ref{sec:pole}, \S\ref{sec:battery}), and the
certification thresholds (\S\ref{sec:certify}).
...
at $N = 60$ converges in 32 IPOPT iterations.
...
Route~B's discretization is deliberately \emph{unlike} Route~A's, which is
what makes the comparison in \S\ref{sec:crossval} informative. At a fixed
$t_f$ the horizon is divided by $N_{conv} = 120$ uniform nodes,
$t = \mathrm{linspace}(0, t_f, N_{conv})$ with spacing $h$, and the linear
dynamics \eqref{eq:linear} are enforced by the \emph{trapezoid} rule on each
of the $N_{conv} - 1$ intervals,
%
\begin{equation}
\rvec_{k+1} = \rvec_k + \tfrac{h}{2}(\vvec_k + \vvec_{k+1}),
\quad
\vvec_{k+1} = \vvec_k + \tfrac{h}{2}(2\gvec + \uvec_k + \uvec_{k+1}),
\quad
z_{k+1} = z_k - \tfrac{h}{2}\alpha(\sigma_k + \sigma_{k+1}) .
\label{eq:trap}
\end{equation}
%
The decision variables are $(\rvec, \vvec, z, \uvec, \sigma)$ at the nodes
and nothing else --- there are no midpoint variables, so $\uvec$ and
$\sigma$ enter as node values and the trapezoid rule treats them as
piecewise linear. All of the convex constraints are imposed
\emph{per node}: the second-order cone $\lVert\uvec_k\rVert \le \sigma_k$,
the Taylor band \eqref{eq:taylor}, $\sigma_k \ge 0$, the $z$ bracket, the
glideslope in its homogeneous quadratic form, $z_k \ge 0$, and (only when
$\theta_{\max}$ is finite) the pointing cone $u_{z,k} \ge
\cos\theta_{\max}\,\sigma_k$. The boundary conditions are imposed at the
first and last node and the objective is $\min -z(t_f)$. The subproblem is
built in CasADi and solved with IPOPT, exactly as Route~A is.

Two order-of-accuracy facts follow and matter later. Route~B is
\emph{second} order in $h$ against Route~A's third-order Hermite--Simpson,
and Route~B's control representation is piecewise linear against Route~A's
per-segment quadratic. The two routes therefore share no discretization
order and no control basis, and \S\ref{sec:crossval}'s $N_{conv}$ sweep is
what rules out Route~B's own trapezoid error as the source of the residual.

The outer loop over $t_f$ is the golden-section search described under
\emph{Free final time} below, and it is worth recording here how an
infeasible probe enters it. Each probe returns a validity code: $3$ (tight
relaxation, \texttt{Solve\_Succeeded}), $2$ (tight, acceptable-level
convergence only), $1$ (converged but not tight), or $0$ (solver failed or
threw). Codes $3$ and $2$ contribute $m_f$; codes $1$ and $0$ contribute
$-\infty$. The shipped \texttt{tf\_curve} shows exactly one such probe --- the
first, at $t_f = 14.5836$~s, code $0$, entered as $-\infty$; the other
thirteen are codes $3$ and $2$ --- which means the function the search actually
optimizes is a $-\infty$-substituted surrogate rather than $m(t_f)$ itself.
Unimodality of the latter does not by itself license golden section on the
former; it is legitimate here because the infeasible region is a \emph{left}
tail --- too little time to arrest --- so the surrogate inherits
unimodality.
...
The theorem below is stated for the \emph{exact exponential relaxation}
\eqref{eq:linear}--\eqref{eq:relax}, which is the object the literature
proves it for. It is \emph{not} a statement about the program this campaign
actually solves, which is the Taylor restriction \eqref{eq:taylor}; the
boundary between the two is drawn explicitly after the proof.

\begin{theorem}[Lossless convexification, \cite{acikmese2007,acikmese2011}]
\label{thm:lossless}
Consider the relaxed problem \eqref{eq:linear}--\eqref{eq:relax} with
$\rvec(t_f)$ and $\vvec(t_f)$ fully constrained, and assume
%
\begin{enumerate}
\item[(A1)] \emph{Normality / controllability.} The linear system
\eqref{eq:linear} is controllable and the extremal is normal, so the
abnormal multiplier does not vanish the cost and the degenerate branch
$\lamv \equiv \mathbf{0}$ is excluded (this is the technical content of
\cite{acikmese2011}).
\item[(A2)] \emph{Constraint qualification.} A constraint qualification
holds at the optimum, so a KKT/maximum-principle multiplier set exists;
in particular the state path constraints (glideslope, $z\ge0$,
$m \ge m_{dry}$) admit multipliers of bounded variation.
\item[(A3)] \emph{Existence.} The relaxed problem has an optimal solution
on $[0,t_f]$.
\end{enumerate}
%
Let $(\rvec^\ast, \vvec^\ast, z^\ast, \uvec^\ast, \sigma^\ast)$ be such an
optimal solution.
...
The sketch that follows is offered as \emph{intuition} for why the
relaxation is tight, in the case where the argument is cleanest: it is not
a self-contained proof, and the place it is loose is identified explicitly.
...
that no state-constraint multiplier appears. That case is the relevant one
here: on the flagship solution the glideslope margin
$z - \lVert(x,y)\rVert\tan\gamma_{gs}$ is strictly positive at every node,
falling to $0.90$~m at the last interior node and to zero only at the pad
itself, where both sides vanish because $\rvec(t_f) = \mathbf{0}$. The cone
is therefore inactive on the whole interior, and what binds at the endpoint
is the terminal boundary condition, not the cone. The dynamics are then linear
...
Theorem~\ref{thm:lossless} is about the exact relaxation
\eqref{eq:linear}--\eqref{eq:relax}. The \emph{implemented} Route~B program
is the Taylor restriction \eqref{eq:taylor}, discretized. Two statements
should be kept apart, because conflating them would over-claim:
%
\begin{itemize}
\item \textbf{Losslessness transfers, and is certified.} The tightness
argument turns entirely on the pointwise minimization of
$\lamv\!\cdot\!\uvec$ over the ball $\lVert\uvec\rVert \le \sigma$. Replacing
the exponential throttle band by the Taylor band alters only the
\emph{$\sigma$-admissible set}; it never touches that pointwise
minimization, so the argument goes through verbatim for
\eqref{eq:taylor} as well. Gate G4 (\S\ref{sec:certify}) then
\emph{measures} $\max_t\big|\lVert\uvec\rVert-\sigma\big|$ on the computed
solution and certifies $\lVert\uvec\rVert = \sigma$ \emph{for the program
that was actually solved}.
\item \textbf{The Taylor restriction is a separate, measured model error.}
What G4 does \emph{not} certify is that the solved program is the original
problem. Equation~\eqref{eq:taylor} is an inner approximation of the true
throttle band on both sides, so Route~B solves a slightly harder problem
than the continuous-time original. That discrepancy is not argued away; it
is measured, as gate G3's $|\Delta m_f|$, and \S\ref{sec:crossval} reports
it and identifies it.
\end{itemize}
%
One further condition makes \eqref{eq:taylor} non-vacuous rather than merely
formal: its upper bound $\mu_2(1-\delta z)$ is positive only for
$\delta z < 1$. That holds here with enormous room --- the measured maximum
of $\delta z$ on the flagship Route~B solution is $0.034$ --- so the
restriction is non-vacuous by measurement, not by assumption.
...
angle in \eqref{eq:primer} on the \emph{collocation} solver's duals --- at
the segment midpoints, which \S\ref{sec:certify} shows is the one place the
discrete stationarity condition makes that comparison exact.
...
propellant, or $1.4\times10^{-5}$ of the ignition mass $m_0$.
...
refine until it closed. It does not close. The refinement evidence
(recorded in \texttt{process/task-5-report.md} and summarized in
\texttt{certify\_pdg.m}'s own header) has two halves, and they should be
quoted with their conditions:
%
\begin{itemize}
\item at \emph{fixed} $t_f$ --- which removes the golden search's own
$t_f$-selection noise --- sweeping $N_{conv} \in \{120, 180, 240\}$ moves
the gap only from $0.737$ to $0.731$~kg, i.e.\ it sits in a
$0.73$--$0.74$~kg band and if anything drifts the wrong way;
\item allowing the golden search to reselect $t_f$ at a tightened
tolerance ($0.05 \to 0.01$~s) does not help either, and widens the observed
band to \textbf{$0.70$--$0.94$~kg} --- $0.699$~kg at $N_{conv}=120$ and
$0.943$~kg at $N_{conv}=240$ --- because $m(t_f)$ is shallow but not flat
near its optimum.
\end{itemize}
%
The $0.70$--$0.94$~kg band is the one to quote, since it is the envelope of
every refinement tried; the $0.73$--$0.74$~kg figure is the fixed-$t_f$
subset of it. Both were measured mid-campaign at the then-current
configuration (before the de-rate of \S\ref{sec:derate}), which is why
their absolute level differs from the flagship's $0.428$~kg --- the
flagship value is the same quantity measured at the shipped
$\etaT = 0.87$. What no version of the experiment produces is a downward
trend: the gap is not a discretization artifact.
...
substantive gates G1--G5, of which the second (continuous accuracy) has two
halves: G2 asks whether the reconstructed control \emph{flies} to the right
place, and G2ff asks whether that same reconstruction is \emph{admissible}
while doing it. They share a reconstruction and a failure history, which is
why they are one gate and not two. A sixth, G0, checks the time base the
other five are computed on; it is described last because it certifies the
certifier rather than the solution.
...
difference between Routes A and B. The mass row is reclassified as a
measurement, per \S\ref{sec:crossval}. All three rows are \emph{scored}:
the trajectory row was computed and printed but excluded from the verdict
until the 2026 external review pointed out that two solutions could then
agree on final mass and final time while flying materially different paths
to the pad. Its threshold is dimensioned from the mission rather than from
the measurement --- $5$~m, one third of the $R_{pad} = 15$~m landing
requirement, so the two independently formulated solvers must agree on
\emph{where} the vehicle is to well inside the accuracy that decides
success. The measured value is $0.373$~m.
...
valid unrescaled), must be under $0.01^\circ$.

\emph{At which instant} that angle is evaluated turned out to matter more
than the threshold. An external code review of this campaign found that the
comparison was being made between segment $k$'s defect dual and the
\emph{node} control $\Tvec_k$, and that the correct partner is the
\emph{midpoint} control $\Tvec_{k+1/2}$. The argument is short. In the
transcription \eqref{eq:hs} the midpoint control enters exactly one
constraint block --- segment $k$'s defect, through the Simpson weight
$\tfrac{4h}{6}f(x_{k+1/2},\Tvec_{k+1/2})$ --- plus its own annulus row,
whose gradient is parallel to $\Tvec_{k+1/2}$ itself. Its stationarity
condition is therefore
%
\begin{equation}
0 \;=\; \frac{\partial \mathcal{L}}{\partial \Tvec_{k+1/2}}
\;=\; -\frac{4h}{6}\,B(x_{k+1/2})^\top \lambda_k
\;+\; \mu_k\, \Tvec_{k+1/2},
\label{eq:midstat}
\end{equation}
%
and since the rows of $B$ that touch $\Tvec$ are $I/m$ (velocity) and
$-\Tvec^\top/(\lVert\Tvec\rVert I_{sp}g_0)$ (mass, itself radial in
$\Tvec$), \emph{every} term in \eqref{eq:midstat} other than $\lamv$ is
radial in $\Tvec_{k+1/2}$. Collecting them,
\eqref{eq:midstat} reads $(4h/6)\,\lamv/m = c\,\Tvec_{k+1/2}$ for a scalar
$c$ absorbing the annulus multiplier $\mu_k$ and the mass-flow term, and so
forces $\Tvec_{k+1/2}$ exactly parallel to $\lamv$ at the \emph{discrete}
level, with no discretization error at all.
Two conditions are what make that collection possible, and both hold at the
midpoint of this transcription: the midpoint state carries \emph{no} path
constraint row (glideslope, $z \ge 0$ and $m \ge m_{dry}$ are imposed at
nodes only), and there is no active pointing-cone row (this campaign runs
with $\theta_{\max} = \infty$). A finite active cone would add a
non-radial term and the alignment would no longer follow --- which is
exactly why \texttt{certify\_pdg} stops scoring the primer angle when a
cone is present (\S\ref{sec:future}).
A node control has no such clean condition: $\Tvec_k$ appears in two
adjacent defect blocks \emph{and} inside both neighbours' Hermite
interpolants $x_{k\pm1/2} = \tfrac12(x_k + x_{k\pm1}) \pm \tfrac{h}{8}(f_k -
f_{k\pm1})$, so its stationarity condition mixes $\lambda_{k-1}$ and
$\lambda_k$. Scoring $\lambda_k$ against $\Tvec_k$ is thus an $O(h)$
time-shifted comparison, and $O(h)$ is precisely what the old numbers were:
the measured angle halved as $N$ doubled ($1.72^\circ$, $1.14^\circ$,
$0.85^\circ$, $0.57^\circ$ at $N = 20, 30, 40, 60$ in vacuum, and
$8.63^\circ, 5.11^\circ, 3.63^\circ, 2.61^\circ$ under drag). Compared at
the midpoint, the same solutions and the same duals give
$8.5\times10^{-7}$ to $1.2\times10^{-6}$~degrees at \emph{every} grid, in
vacuum and under drag alike --- which is not a physical number but the
double-precision floor of $\arccos$ near unity. That floor is one to two
ulps wide, because $\arccos$ is quantized there: $\arccos(1-\epsilon/2)$ is
$8.54\times10^{-7}$~degrees, the smallest attainable nonzero value, and
$\arccos(1-\epsilon)$ is $1.207\times10^{-6}$~degrees. Which of the two a
given run reports depends only on which representable double the dot
product lands on, and both values are observed: \texttt{certify\_pdg.m}'s
own measurement table records $8.5\times10^{-7}$ across the four vacuum
grids and $1.2\times10^{-6}$ across the four drag grids, while the
flagship report below reads $1.207\times10^{-6}$ in vacuum. The factor of
$1.4$ between them is arithmetic, not physics. The alignment is exact to
floating point.

This retired two open items at once. What the campaign had been calling a
``vacuum discretization artifact'' and a ``drag primer degradation''
(\S\ref{sec:phase2cert}) were the same bug, and neither was about physics.
What the gate now measures is the KKT stationarity residual of the solve,
so its threshold is calibrated against solver convergence depth rather than
mesh size: at $N=60$, varying the NLP tolerance alone gives $0.037^\circ$
(vacuum) and $92^\circ$ (drag) at $10^{-4}$, falling to the $\arccos$ floor
at both shipped defaults ($10^{-9}$ vacuum, $10^{-11}$ drag). The gate is
set at $0.01^\circ$ --- four orders of margin over every shipped
configuration, tolerant of three orders of degradation in convergence
depth, and still failing outright on a genuinely unconverged solve. One
threshold now serves vacuum and drag; the $10^\circ$ drag loosening
described in \S\ref{sec:phase2cert} is deleted, because what it
accommodated no longer exists.

\paragraph{G0 --- time-base consistency.}
A bookkeeping gate, added when the same review pointed out that the two
consistency checks it replaces were \texttt{assert} calls in a routine
documented as ``report-only and never throws''. G1 builds its own grid as
$h = t_f/N$ while G2 and G5 read \verb|sol.t| directly; if those ever
disagreed, the discrepancy would be invisible to G1, which never reads
\verb|sol.t| at all. G0 reports $|t(\mathrm{end}) - t_f|$ and the worst
grid-spacing error against $h$. Making it a gate rather than an assertion
matters in exactly the case that motivates certification: a malformed or
failed-solver solution used to throw, destroying the report for the other
gates rather than producing one.

\subsection{Flagship gate table}

The following is the verbatim gate report for the Phase-1 problem at the
production grid $N = 60$ / $N_{conv} = 120$ with no tolerance scaling
(re-certified 2026-08-09 after the review fixes above).
...
$|\Delta m_f|$ with $57\%$ of its (adjudicated) ceiling unused and its
trajectory row with a factor of $13$, G4 by a factor of $18$, and G5's
primer angle at the $\arccos$ machine-precision floor, four orders under a
gate that is itself $100\times$ tighter than the one it replaced. The bound
fraction $0.9917$ is exactly $1$ sample of $121$ off a bound --- one
midpoint sitting inside the annulus at the switch, which is the correct
answer for a discretized step, not a defect.
...
to work with at $180$~m/s, the approach is not hot enough.

It is worth being precise about how much of this Pontryagin's principle
actually gives. Because the Hamiltonian is linear in the thrust magnitude,
PMP gives the bang--bang \emph{form}: wherever the switching function is
nonzero, the optimal magnitude sits on a bound of the annulus. That is all
it gives here. It does \emph{not} say which bound comes first --- that is
scenario-dependent, and is the whole content of this section. And it does
not by itself \emph{bound the number of switches}: a switch-count bound is
an additional result requiring extra structure, and is not a theorem being
invoked here. That the count is \emph{one} on this problem family is an
empirical and structural observation --- a Hamiltonian linear in the
control, and no singular arc detected at any grid --- and it is
\emph{measured}, not derived: gate G5 reports a bound fraction of $0.9917$
and a single interior switch at every grid from $N = 15$ to $N = 240$.
...
distinguish \texttt{touchdown} / \texttt{arrest} / \texttt{depleted} /
\texttt{horizon}, because
...
mode as a perfect landing. The \texttt{depleted} class (propellant falling
through $m_{dry}$) was folded into \texttt{arrest} until the 2026 external
review; splitting it out matters because the two failures point at
different fixes. An arrest is a controllability failure with fuel still in
the tanks; a depletion is a fuel-budget failure. The campaign's single
Phase-1 Monte Carlo failure, discussed in \S\ref{sec:mc}, is a depletion
that had been reported as an arrest.
...
$r = 10^{-9}$ and $m = m_A$ the term $2m\sqrt{q_r r} = 0.0187$ is already
$1.9\times$ larger than $q_v$, putting $\omega_r$ at $0.059$~rad/s, further
below the bound.) These old weights do not in fact give a real pole pair:
evaluated at $m_A = 29\,624$~kg, the mean mass over the margin arc
$[0, t_{sw}]$ with $t_{sw} = 6.1552$~s --- the same $m_A$ the shipped
design uses, and the mass that makes $2m_A\sqrt{q_r r} = 0.0187$ --- the
exact closed-loop eigenvalues are
$-0.0905 \pm 0.0499\mathrm{i}$, an underdamped pair whose envelope decays
with an $11.0$~s time constant --- on a $16.6$~s flight. Either way the
...
lateral command is $\omega^2 |\Delta \rvec| = 0.49 \times 58 = 28.4$~m/s$^2
\approx 842$~kN, which together with the $\sim 338$~kN vertical feed-forward
gives a commanded magnitude of $\sqrt{842^2 + 338^2} = 907$~kN --- $7\%$
outside the $845$~kN rim --- so $\omega = 0.70$ spends the margin
arc's spare authority and modestly more. Beyond $\omega \approx 0.75$ the
...
crossing that G5 has already certified to be unique. What is rescheduled is
the \emph{lateral} channel, position \emph{and} velocity: the derived pair
$(q_{pos}, q_{vel})$ of \eqref{eq:inversion} occupies entries $(1,1),(2,2)$
and $(4,4),(5,5)$ of $Q_A$, and phase B replaces both with the small
constants it always had ($10^{-4}$ and $10^{-2}$). The \emph{vertical}
channel is untouched --- $Q(3,3) = 10^{-4}$ and $Q(6,6) = q_{v_z}$ are the
same in $Q_A$ and $Q_B$ --- because the vertical channel demonstrably
worked. That is the split: the schedule moves the lateral loop and leaves
the vertical one alone.
...
\emph{A note on the tables in this section.} The before/after pairs in
\S\ref{sec:pole} and \S\ref{sec:altindex} are \textbf{mid-campaign
snapshots}, each measured at the configuration current when that fix was
made --- \S\ref{sec:pole}'s pair predates both the altitude indexing of
\S\ref{sec:altindex} and the $\etaT = 0.87$ de-rate of \S\ref{sec:derate},
and the reachability table of \S\ref{sec:derate} was measured at the
then-shipped $\etaT = 0.93$. They are reported because the \emph{step} each
one measures is the evidence for the corresponding diagnosis. The shipped
configuration's numbers are Table~\ref{tab:battery}, which supersedes all of
them; where a case appears in both (e.g.\ $\Delta\rvec_0 = [50;-30;0]$~m at
$0.944$~m / $1.363$~m/s here versus $1.582$~m / $0.968$~m/s in
Table~\ref{tab:battery}), the difference is the intervening configuration
changes, not a discrepancy.
...
not have. The arithmetic is unforgiving, and it is worth doing in a
\emph{single} currency, because the loss and the reserve are naturally
quoted in two different ones. Take that currency to be the net
decelerating acceleration on the braking arc, $\Tmax/m - g_0$:
%
\begin{itemize}
\item a $-5\%$ thrust scale costs $0.05\,\Tmax/m$, which is
$7.2$--$7.7\%$ of $\Tmax/m - g_0$ over the mass range
$m \in [m_f, m_0] = [26\,465,\ 30\,000]$~kg;
\item reserving a fraction $1 - \etaT$ of thrust returns
$(1-\etaT)(\Tmax/m)\,/\,(\Tmax/m - g_0)$ of that same net deceleration ---
so $\etaT = 0.93$ returns $10.1$--$10.7\%$ and $\etaT = 0.87$ returns
$18.8$--$19.9\%$.
\end{itemize}
%
In those terms $\etaT = 0.93$ covers a $-5\%$ dispersion by only about
$1.4\times$, which is thin once the terminal arc's own demands are added,
while $\etaT = 0.87$ covers it by about $2.5\times$. A de-rate reserving
less than the loss is buying nothing at all on this dispersion. (The
$0.95/0.93 = 2.1\%$ figure quoted later in this section is the same
comparison in \emph{thrust} units, and is included only because that is
the form in which the $0.93$ decision was originally recorded; the two are
not directly comparable and $\etaT$'s sufficiency is judged in the net
deceleration currency above.)
...
\begin{table}[htbp]
\centering
\caption{Reachability bound (\texttt{process/task-7-report.md} \S5).
\textbf{Configuration:} measured at the \emph{then-shipped} $\etaT = 0.93$,
with a vertical-only model (all thrust vertical, no lateral demand), zero
tracking error, and the brake altitude located by bisection. It is a
mid-campaign snapshot, kept because it is the evidence for the diagnosis;
the shipped configuration is $\etaT = 0.87$.}
\label{tab:reach}
\begin{tabular}{lrr}
\toprule
...
Two cautions about reading it. The first column is a bisection
\emph{residual}, not a physical minimum --- the search stops once it can
hit the target, so the column is non-monotone (the nominal vehicle reads
worst of the four) and nothing should be inferred from its ordering. The
argument uses only the second column. And that column is a property of the
\emph{model}, not of any particular guidance solution: its $699.6$~m at
unit thrust scale is what a perfectly-flown vertical-only vehicle needs,
whereas the $\etaT = 0.93$ guidance whose battery was failing actually
braked at $718$~m and the shipped $\etaT = 0.87$ guidance brakes at
$914.7$~m (\S\ref{sec:minmax}). Three distinct altitudes are therefore in
play, and they answer three different questions.

With that pinned down, the argument is:
%
\begin{itemize}
\item \textbf{Why $\etaT = 0.93$ failed.} A $-5\%$ vehicle \emph{can} land
softly, but only by braking at $806.6$~m --- $88.6$~m above the $718$~m at
which the $0.93$ feed-forward actually brakes. \textbf{A fixed-reference
tracker inherits the brake altitude from its feed-forward and structurally
cannot re-time it}, so that $88.6$~m is unreachable by feedback and the
$-5\%$ case fails.
\item \textbf{Why $\etaT = 0.87$ passes.} The shipped feed-forward brakes
at $914.7$~m, which is \emph{above} the $806.6$~m the $-5\%$ vehicle needs
--- and within $11$~m of even the $-10\%$ vehicle's $925.2$~m. The
de-rate's sufficiency can thus be stated in the reachability table's own
currency and not only as reserved thrust headroom: the de-rate works
because it moves the nominal brake altitude up past the dispersed
vehicle's requirement.
\end{itemize}
%
Raising the loop bandwidth cannot substitute: a sweep from $5$ to
...
to be insufficient}: in thrust units it leaves only $0.95/0.93 = 2.1\%$ of
margin against a $-5\%$ dispersion, and the terminal arc consumes more than
that. The same insufficiency read in the net-deceleration currency above is
the $1.4\times$ cover, and in the reachability table's currency it is the
$88.6$~m the $0.93$ feed-forward cannot climb. A held-
...
failure modes & 1 propellant depletion, 0 arrests, 0 horizon expiries,
0 miss/speed failures \\
...
largest draws seen & $|\Delta\rvec_0| = 443.7$ m, $|\Delta\vvec_0| = 34.4$ m/s, thrust $-4.4\%$ / $+4.5\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
initial position error of $\Delta\rvec_0 = [-34.2;\ 425.3;\ 121.6]$~m ---
...
reached $m_{dry}$ with $0.00$~kg of usable propellant remaining, and the
simulation terminated on the propellant-depletion event. Every other run in
the campaign landed, and $95\%$ of
...
depletion run and is not a touchdown speed at all: among the $199$ genuine
...
\emph{One caveat on the magnitude, stated up front.} The $434.7$~kg is
conditional on the drag model of Eq.~\eqref{eq:drag} --- $C_d = 1.0$,
$A = 10.75$~m$^2$ (the body frontal area $\pi\cdot 1.85^2$, with no
base-drag, plume-interaction or grid-fin contribution and no Mach or
attitude dependence), and an exponential atmosphere --- and over this range
the saving scales roughly with the product $C_d A$. The
\emph{mechanism} below (minimum-throttle-arc extension, switch moves later)
is robust to that coefficient; the \emph{number} is model-dependent, and a
retro-propulsive base flow is exactly the regime in which an effective
$C_d A$ is hardest to pin down.

\paragraph{The minimum-throttle-arc extension mechanism.}
There is no coast here and the name matters: by Eq.~\eqref{eq:tw} the
engine is never off, so the vehicle is on a continuous $\Tmin$ burn
throughout the first arc and is spending propellant the whole time. What is
free is not the arc; it is drag's \emph{incremental} deceleration, which
adds to the $\Tmin$ burn's own at no additional propellant cost.
...
\emph{shortens} by $3.87$~s, netting $+0.94$~s. Because drag decelerates the
vehicle during the $\Tmin$ margin arc without burning anything extra, the
vehicle can afford to stay on minimum throttle for longer and arrive at the
braking point slower and lower --- $119.1$~m/s at $378.9$~m instead of
$175.4$~m/s at $914.7$~m. Less velocity to kill from a lower altitude means
a shorter, cheaper braking burn. Flight time goes up precisely because the
propellant-free \emph{increment} to the deceleration is doing more of the
work, which is the same trade the min--max argument of
...
\label{sec:phase2cert}

Gates G0, G1, G2, G2ff and G5 apply unchanged. G3 and G4 are \emph{skipped}, and
...
G5 primer angle [deg]   & $1.21\times10^{-6}$ & $<0.01$ & PASS \\
\bottomrule
\end{tabular}}
\end{center}

One adaptation is worth recording honestly, and one former adaptation has
been withdrawn.

The withdrawn one is G5's primer threshold, which this campaign loosened
from $1^\circ$ to $10^\circ$ under drag and reported as a genuine, if
unexplained, drag-model effect: the same coarse grid that gave a vacuum
primer angle of $1.14^\circ$ gave $5.11^\circ$ under drag, and at the
production grid drag still measured $2.605^\circ$, so finer meshing did
\emph{not} cure it the way it cured the vacuum artifact. The analysis
accompanying that loosening correctly observed that drag \emph{cannot}
change the direction condition --- drag enters the dynamics Jacobian $A$ but
never $B = \partial\dot{x}/\partial\Tvec$, so the Hamiltonian's
control-dependent term is untouched and the maximizing direction is still
$+\lamv/\lVert\lamv\rVert$ --- and then, having found no mechanism,
attributed the residual to degradation in how the discrete dual approximates
the continuous costate and left it open.

It was not that. It was the primer check comparing a segment's defect dual
against a \emph{node} control instead of that segment's \emph{midpoint}
control, an $O(h)$ time-base error described in \S\ref{sec:certify}. Under
the corrected comparison the drag primer angle is $1.21\times10^{-6}$
degrees --- the $\arccos$ floor, identical to vacuum, at every grid. The
$10^\circ$ threshold is deleted and drag is scored against the same
$0.01^\circ$ gate as vacuum. Two lessons are worth keeping from the episode.
First, the loosening was at least defensible in form: it kept the detector
and only moved the threshold, rather than dropping the check, so the gate
would still have caught the sign-flip class of bug it was there for. Second,
and less comfortably, an analysis that establishes there is \emph{no
mechanism} for an observed effect is evidence that the measurement is wrong,
not that the effect is mysterious. The correct response to ``drag cannot do
this, and yet the number moves'' was to distrust the number.

The adaptation that stands: the NLP convergence tolerance is tightened from $10^{-9}$ to
...
failure modes & 0 arrests, 0 depletions, 0 horizon expiries,
2 pad-radius misses \\
...
The campaign set out to solve a Falcon-9-class landing burn twice and prove
the answer, and it did both. The certified optimum is a \emph{min--max}
profile --- $6.155$~s at $\Tmin$, then $10.440$~s at $\etaT\Tmax$, one
interior switch at $914.7$~m --- not the max--min--max structure the Mars
PDG literature and this campaign's own specification anticipated. That is
not a solver quirk but a consequence of a single inequality,
$\Tmin > m g_0$ at every mass on the trajectory (\S\ref{sec:hoverslam}):
a vehicle that cannot hover has no coast to buy, so the fuel-optimal policy
is to fall at minimum throttle and brake as late as it can. Two
independently formulated solvers agree on that answer to $0.428$~kg of
final mass and $3.4$~ms of final time, and all six gate blocks pass with
margins of one to five orders --- including a primer-alignment residual at
the $\arccos$ machine floor once the comparison is made at the segment
midpoint, where the transcription's discrete stationarity condition makes
it exact.

Most of the engineering content, however, came from the tracking problem
rather than the optimization, and the pattern in it is worth naming: in
every case the fix followed from a derivation or a reachability bound, and
in every case a sweep had already searched the wrong axis. The
$\vvec(t_f) = \mathbf{0}$ endpoint is singular for this vehicle for the
same reason the optimum is min--max. The lateral response speed is capped
at $\sqrt{q_r/q_v}$ for \emph{every} $R$, so three rounds of $R$ sweeps
could not have succeeded and the fix was to invert the ARE for the pole
instead. A min-fuel trajectory has no time margin, so the reference must be
indexed by altitude and not by the clock --- a change that \emph{deleted}
three special cases. And a $-5\%$ engine must brake $88.6$~m higher than a
fixed feed-forward ever will, which is a reachability fact no gain can
overcome and is what forced the $\etaT = 0.87$ de-rate at a cost of
$82.30$~kg. The result is $99.5\%$ over $200$ dispersed vacuum runs, with
the single failure a propellant depletion at a $4.25\sigma$ cross-range
draw.

The Phase-2 finding is the one that looks wrong and is not: drag
\emph{saves} $434.7$~kg while \emph{lengthening} the flight by $0.94$~s.
The mechanism is the min--max argument again --- the propellant-free
increment drag adds to the deceleration lets the minimum-throttle arc run
$4.80$~s longer, so the vehicle reaches the braking point slower and lower
and the expensive arc contracts by $3.87$~s. Drag makes the approach
colder and the switch moves later, exactly as \S\ref{sec:minmax} predicts.

The campaign's methodological residue is smaller than its numerical one but
probably more durable. A certification gate can produce a false negative
and look like physics: G2's global-spline reconstruction failed an accurate
solution by a factor of $60\,700$, and G5's node-versus-midpoint comparison
manufactured both a ``vacuum discretization artifact'' and a ``drag primer
degradation'' that were the same $O(h)$ bug. In both cases the correct
response to ``there is no mechanism for this effect, and yet the number
moves'' was to distrust the number.
...
and is invalid once the cone is active. A finite cone does not by itself
force anything: while the cone constraint is \emph{inactive} the
unconstrained primer law still holds, and the check would still be valid.
But \emph{when the cone constraint is active}, the constrained optimum lies
on its boundary --- the KKT projection of the unconstrained direction onto
it --- and thrust need not be parallel to the costate, by design rather than
by bug. Since nothing tells the gate in advance which instants those are,
the software takes the conservative policy of not scoring the primer angle
whenever a finite cone is present; the measured $7.32^\circ$ at
$\theta_{\max} = 15^\circ$ (which survives the midpoint fix, unlike the
vacuum/drag artifact) shows the false-fail is real and not
hypothetical. \texttt{certify\_pdg.m} is cone-aware: when
...
cannot re-time the brake}: the reachability table (Table~\ref{tab:reach})
shows a $-5\%$ vehicle must brake at $806.6$~m, $88.6$~m above where the
$\etaT = 0.93$ feed-forward braked, and no feedback gain around a fixed
feed-forward can do that. A guidance law that re-solves the PDG problem in flight ---
...
\item \textbf{Higher-order costate recovery.} The primer check now uses the
one place in the Hermite--Simpson transcription where a defect dual and a
control satisfy an exact discrete stationarity condition --- the segment
midpoint (\S\ref{sec:certify}). That is enough to certify \emph{alignment},
which is what G5 needs, but it does not give a continuous costate
\emph{history} of known order: the node duals, which is where a costate
trajectory would naturally live, mix multipliers from adjacent segments.
Recovering $\lambda(t)$ to a stated order from the discrete adjoint system
would let the campaign report a genuine costate profile alongside the
primer angle, and would be the natural bridge to an indirect (shooting)
...
Phase-2 stage, and it is a \texttt{phase2 = true} run --- the file shipped
in \texttt{results/}, timestamped 2026-08-09 --- that this note is written
against. Every result number quoted here, Phase~1 and Phase~2 alike, was
read from that one file: the gate tables, the cross-validation table, the
acceptance battery, both Monte Carlos, and the vacuum-versus-drag
comparison. The exceptions are stated where they occur and are all
mid-campaign sweeps at superseded configurations, recorded in
\texttt{process/task-\{5,7\}-report.md}: the $\etaT$ cost table
(Table~\ref{tab:derate}), the reachability table (Table~\ref{tab:reach}),
the before/after tracking pairs in \S\ref{sec:pole}--\S\ref{sec:altindex},
and the Taylor-gap refinement sweeps of \S\ref{sec:crossval}. All
