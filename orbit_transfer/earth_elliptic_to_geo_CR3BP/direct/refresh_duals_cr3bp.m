...
mfSaved = S.X(6, end);
mfNew   = out.X(6, end);
info = struct('ipoptStatus', out.ipoptStatus, 'maxDefect', out.maxDefect, ...
    'drift', max(abs(out.X(:) - S.X(:))), ...
    'mfRelChange', abs(mfNew - mfSaved)/max(abs(mfSaved),1e-30), ...
    'improved', false);

if ~out.success
    error('refresh_duals_cr3bp:resolveFailed', ...
        'warm re-solve did not succeed (%s) -- cannot refresh duals.', out.ipoptStatus);
end
if out.maxDefect > feasTol
    error('refresh_duals_cr3bp:notFeasible', ...
        ['warm re-solve maxDefect %.3e > feasTol %.3e -- the recovered point ' ...
         'is not machine-tight, so its multipliers are not a KKT certificate.'], ...
        out.maxDefect, feasTol);
end
if mfNew < mfSaved - massTol*max(abs(mfSaved),1)
    error('refresh_duals_cr3bp:massDegraded', ...
        ['warm re-solve final mass DROPPED %.3e relative (%.6f -> %.6f) -- ' ...
         'refusing to verify against a worse point.'], ...
        info.mfRelChange, mfSaved, mfNew);
elseif info.mfRelChange > massTol
    info.improved = true;
    fprintf(['  [refresh_duals_cr3bp] NOTE: warm re-solve improved m_f by ' ...
             '%.3e relative (%.6f -> %.6f); gates describe the IMPROVED ' ...
             'extremal, not the banked artifact.\n'], ...
        info.mfRelChange, mfSaved, mfNew);
end
