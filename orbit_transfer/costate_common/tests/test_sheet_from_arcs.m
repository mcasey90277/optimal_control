...
% two DISTINCT roots that happen to share a final time: same t_f, different
% costates. Keying the duplicate merge on t_f alone would silently discard
% one of them and the grid point would lose a candidate.
...
% STUBCERT  Stand-in certifier: tag > 0 certifies; the tag also rides in the
% costates so two candidates can share a final time and still be distinct.
% INPUTS: p; sA.  OUTPUTS: C.
C = struct('ok', p(2) > 0, 'reason', 'certified', 'z', [p(2)*ones(7,1); p(1)], ...
           'tfDays', p(1), 'sA', sA);
