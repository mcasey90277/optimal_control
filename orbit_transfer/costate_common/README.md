...
| file | what | used by |
|---|---|---|
| `ms_bvp.m` | **DELEGATE** since 2026-09-16: the generic multiple-shooting BVP engine was promoted to the cross-folder library — `../../oclib/+oc/ms_bvp` (call it `oc.ms_bvp`) — when the cart-pole PMP-BVP demo (`optimal_control_examples/ex3_cart_pole_pmp`) became the second TOP-LEVEL consumer. This delegate keeps every costate_common caller working unchanged; the contract, the maths and the self-demos live with the implementation. Equivalence gate: `golden_cells` 20/20 **bit-identical** (z, ‖R‖ and iteration counts all to the last bit), the four-campaign ladder re-solve (HALO 4.551e-14, DPO 1.130e-14, HALO_HALO 3.478e-14, GTO 2.720e-14 max |Δt_f|), and `test_cartpole_pmp` 9/9. | DRO (+ internal), optimal_control_examples/ex3_cart_pole_pmp (delegates to `oclib`) |
...
| file | what | used by |
|---|---|---|
| `ms_conjugate_test.m` | **DELEGATE** since 2026-09-17: the Jacobi (conjugate-point) test was promoted to the cross-folder library — `../../oclib/+oc/ms_conjugate_test` (call it `oc.ms_conjugate_test`) — when the cart-pole study scripts (`optimal_control_examples`, cart-pole sub-project A Task 4) became the second TOP-LEVEL consumer. This delegate keeps every costate_common caller working unchanged; the instrument (both time conventions — free-final-time det([Φ_xλ·P, f(t)]) on the costate-scaling quotient, and fixed-final-time with `freeTime=false`, `quotientDir=[]`, rows 1:7 = the full state), its conventions and its tests live with the implementation. Equivalence gate: `golden_cells` 20/20 **bit-identical** to the pre-move capture (z, iters, normR all unchanged), the four-campaign ladder re-solve (HALO 4.551e-14, DPO 1.130e-14, HALO_HALO 3.478e-14, GTO 2.720e-14 max \|Δt_f\|), and `test_conj_fixedtf`/`test_conj_coverage`/`test_conj_spectrum`/`test_conj_resolve`/`test_folder_rules` all green, byte-identical pre/post. | DRO, verify_common (+ internal), `optimal_control_examples` cart-pole study scripts (delegates to `oclib`) |
