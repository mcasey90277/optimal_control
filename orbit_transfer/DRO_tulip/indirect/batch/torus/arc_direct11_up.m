%% ARC_DIRECT18_up  Pseudo-arclength continuation of the branch through the
% direct-found root at sA 0.4921 (17.96 d) in the up arrival direction,
...
ao.logFile = fullfile(ind, 'results', 'arrival_arc_direct11_up_long.log');
ao.partialFile = fullfile(ind, 'results', 'arrival_arc_direct11_up_long.partial.mat');  ao.saveEvery = 50;
t0 = tic;
A = arclength_arrival(anc, ao);
save(fullfile(ind, 'results', 'arrival_arc_direct11_up_long.mat'), 'A', '-v7.3');
if isfile(ao.partialFile), delete(ao.partialFile); end
fprintf('ARC direct11 up DONE: %d roots, sA %.4f -> %.4f, %d folds, %d crossings, stop = %s (%.0f s)\n', ...
