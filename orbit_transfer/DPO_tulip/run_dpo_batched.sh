#!/bin/bash
# RUN_DPO_BATCHED  Unattended DPO->tulip catalog runs in hang-proof batches.
#
# Same pattern as HALO_tulip/run_halo_batched.sh and
# DRO_tulip/run_ladder_batched.sh (see that file for the rationale: solver
# calls are uninterruptible in-process, so batches run under an OS timeout;
# resume + attempt counters make restarts free).
#
# Usage:   ./run_dpo_batched.sh [cells_per_batch] [batch_seconds]
# Example: ./run_dpo_batched.sh 8 240
...
MLOG="$HERE/direct/results/dpocat_matlab.log"
PUMPKYN=/Users/msc/Desktop/proj7/external/pumpkynPie

mkdir -p "$CATDIR"
LOCK="$HERE/direct/results/.dpocat_driver.lock"
if ! mkdir "$LOCK" 2>/dev/null; then
  echo "another dpo driver appears to be running (lock: $LOCK) -- exiting"
  exit 1
fi
trap 'rmdir "$LOCK" 2>/dev/null' EXIT

count_lines() { echo $(( $(cat "$CATDIR"/dpo_*_progress.txt 2>/dev/null | wc -l) )); }
...
for pass in $(seq 1 200); do
  ...
  "$MATLAB" -batch "here=pwd; cd('$PUMPKYN'); startup(); cd(here); \
                  addpath('$HERE'); \
                  run_dpo_catalog('all', $CELLS, $BSEC);" \
          >> "$MLOG" 2>&1 &
  MPID=$!
  ...
  if tail -30 "$MLOG" 2>/dev/null | grep -qa "^Error in\|^{Error\|Unrecognized"; then
    echo "=== MATLAB ERROR in pass $pass -- ABORTING (see dpocat_matlab.log)" >> "$LOG"
    exit 1
  fi
...
echo "=== dpocat run finished $(date)" >> "$LOG"
