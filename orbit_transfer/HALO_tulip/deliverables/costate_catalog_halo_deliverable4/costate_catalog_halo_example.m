%% costate_catalog_halo_example
%
%   Worked example for costate_catalog_halo_tulip.mat: the HALO -> TULIP
%   minimum-time costate catalog. Pick five coordinates -- halo period,
...
%   DRO catalog so the same pickers work on both -- here it holds the HALO
%   period (ND). The full departure reconstruction recipe rides in
%   sheets(k).dep_family / dep_params (a halo needs tau, Lpt AND pm).
%
...
%     costate_catalog_halo_tulip.mat  the catalog
...
%% 1. Load the catalog (sits beside this script):
 catFile = 'costate_catalog_halo_tulip.mat';
       L = load(catFile);
    cat_ = L.costate_catalog_halo_tulip;
      mu = cat_.constants.muStar;
   tStar = cat_.constants.tStar_s;

fprintf('catalog: %d sheets, %d entries; halo periods [%s] ND; petals [%s]\n', ...
...
%% 2. Choose the five coordinates of a transfer:
 tauHalo = 2.8;       % halo period (ND; 1 ND = 4.43 d) -- sheets at 1.75,2.2,2.8,3.4
      Np = 9;         % tulip petal count               -- sheets at 5,7,9,12
 depDays = 4.0;       % departure phase (days past the halo reference)
 arrDays = 10.0;      % arrival phase (days past the tulip reference)
 thrustN = 5.0;       % thrust (N), anywhere in the rung range

[tf_nd, z8, info] = costate_catalog_pick(cat_, tauHalo, Np, depDays, arrDays, thrustN);
...
% the sheet index the picker chose:
      ...
      sh = cat_.sheets(ks);
[tD,rvD] = get_family_orbit(sh.dep_family, sh.dep_params);   % the halo
...
title(sprintf('halo(\\tau=%.2f) \\rightarrow tulip(N_p=%d), %.1f N, t_f = %.2f d', ...
      info.delivered.tauDRO, info.delivered.Np, seedN, z8(8)*tStar/86400));
legend('transfer','halo','tulip','depart','arrive');

%% 7. The period axis at a glance: best flight time vs halo period at 5 N:
...
grid on
xlabel('halo period [days]'); ylabel('t_f at 5 N [days]');
title(sprintf('best (blue) and worst (red) phasing vs halo period, N_p = %d', ...
      info.delivered.Np));
