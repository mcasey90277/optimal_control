...
# and ../NLP_lowThrust_GTO_tulip/PSR/psr_batch.sh (one-process-per-item +
...
for T in "${rungs[@]}"; do
  ...
  while [ "$attempt" -le "$MAXATTEMPTS" ]; do
    ...
    "$MAT" -batch "addpath('$CASADI_DIR'); cd('$DIR'); reproduce_row($T)" >> "$LOG" 2>&1 &
    pid=$!
    waited=0
    hung=0
...
"$MAT" -batch "cd('$DIR'); reproduce_table3_collect([$rungsMatlab]);" 2>&1 | tee -a "$LOG"
