...
%    optimal_control/orbit_transfer/GTO_tulip/process/LOW_THRUST_MINFUEL_CAMPAIGN.md
%    optimal_control/orbit_transfer/GTO_tulip/process/ROADMAP.md
%    optimal_control/orbit_transfer/GTO_tulip/doc/sundman_minfuel_solution_note.tex
%==============================================================================
...
\begin{itemize}
  \item \textbf{Certified tulip solution} ($1.15\times$ legacy scale $=1.24\times$
        target-consistent): best certified row 24 switches, $\sim$99.4\%
        bang-bang, propellant 2.2487~kg, $\Delta V=3.345$~km/s, max defect
        $1.3\times10^{-13}$, terminal error 0 --- one of \textbf{at least six
        certified local optima at this $t_f$} (24--26 switches, $m_f$ spread
        0.847--0.850; the originally published 25-switch row, 2.264~kg /
        3.370~km/s, is 15~g worse). Report the multiplicity as a result: the
        seed route and the continuation path select the basin, and the switch
        count is a band. Contrast with the 3-switch local optimum (2.950~kg) to
        show the global many-switch structure. \emph{[Sources:
        \texttt{process/BASIN\_1150\_SWEEP.md}, \texttt{FRONT\_SWEEP.md};
        multiplicity is concentrated at $t_f\le1.35\times$ legacy, absent from
        $1.40\times$ up. Updated 2026-09-06.]}
    \item \textbf{Tulip $\Delta V$--$t_f$ front}: certified band $1.12$--$1.25\times$
        (+$1.85\times$) legacy $=1.21$--$1.35\times$ (+$2.00\times$)
        target-consistent. \emph{[Updated 2026-09-07 --- QUOTE THE FREE-$\tau_f$
        NUMBERS.]} The fixed-$\tau_f$ rows all carried the $1.15\times$ seed's
        regularized length $\tau_{f,0}$, an isoperimetric restriction that
        cost up to $9.4\times10^{-3}$ in $m_f$ (142~g at $1.40\times$; median
        $2.9\times10^{-3}$ over the front). Re-solved free-$\tau_f$ at the same
        pinned $t_f$ (all 17 rows): campaign-best
        $\Delta V=2.374$~km/s ($m_f=0.8911$, 1.633~kg) at $1.70\times$ legacy
        $=1.84\times$ target-consistent (was $2.434$~km/s at $1.65\times$);
        $1.65\times$ now $2.380$~km/s; $\sim$47\% below the legacy min-time
        $\Delta V$ of 4.4665~km/s (recompute against the 5.8267~ND anchor's own
        $\Delta V$). All 17 free rows pass the generic first-order gate on the
        9-state (cScale) layout and IPOPT's native inertia test (2026-09-07,
        \texttt{results/minfuel\_freetauf/gate\_summary.txt}), so they carry
        the same certificate as the fixed rows did; the engine now runs
        free-$\tau_f$ by default. Switch count grows/shrinks with
  ...
  \item \textbf{Hard transition band} $1.01$--$1.11\times$ legacy scale
        $=$ \textbf{$1.09$--$1.20\times$ target-consistent} (the certified front
        starts at $1.21\times$): resists all methods; near-min-time conditioning
        wall that afflicts even the smooth energy problem---itself a structural
        finding, and a nine-percent-above-min-time wall reads differently from
        a one-percent one. Thrust ladder (not in the original outline):
        25 $\to$ 20~mN closes in 4\% steps; ceiling $\sim$19.5--21~mN, cause
        open.
