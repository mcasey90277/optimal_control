...
| `oc.ms_conjugate_test` | The Jacobi (conjugate-point) test on a converged multiple-shooting extremal, both time conventions: free-final-time (`det([Φ_xλ·P, f(t)])` on the costate-scaling quotient) and fixed-final-time (same instrument, no flow column, `quotientDir = []`, `stateRows` = ALL state rows — a Jacobi field must vanish in the full state; the orbit campaigns pass 1:7 for r, v and m, the cart-pole study passes 1:4). A necessary-condition check (Jacobi), not a sufficiency proof; segment STMs come free from `ms_bvp`'s `opts.keepSTMs`. | `orbit_transfer` (every costate campaign, via the `costate_common` delegate) + `optimal_control_examples/ex3_cart_pole_pmp/cartpole_minenergy_study.m` (directly, as `oc.ms_conjugate_test`, its §7 SUFFICIENCY gate S2 — the second *top-level* consumer, on a fixed-t_f min-energy PMP extremal with no orbit, no CR3BP quantity and no pumpkyn call in it). | orbit: `golden_cells` 20/20 **bit-identical** pre- vs post-move (dro \|z-z8\| 3.7e-11/iters 1/normR 1.2e-13, halo 4.4e-11/1/6.0e-14, dpo 1.2e-10/1/2.9e-14, harv \|z-zref\| 4.5e-12/iters 2 — every number unchanged) PLUS the four-campaign ladder re-solve of one stored cell each, unchanged to the printed digit — HALO 4.551e-14, DPO 1.130e-14, HALO_HALO 3.478e-14, GTO 2.720e-14 max \|Δt_f\| (dZ8 1.689e-12/1.466e-12/2.354e-13/2.895e-13, OK flags matching); `test_conj_fixedtf`, `test_conj_coverage`, `test_conj_spectrum`, `test_conj_resolve`, `test_folder_rules` all green, PASS/FAIL output byte-identical pre- vs post-move |
...
3. ~~(part) local-residual engine~~ — `oc.local_residual` landed 2026-08-25
   ...
   landed 2026-09-17: the cart-pole min-energy study script
   (`optimal_control_examples/ex3_cart_pole_pmp/cartpole_minenergy_study.m`,
   cart-pole sub-project A Task 4) is its second TOP-LEVEL consumer, calling
   `oc.ms_conjugate_test` directly on a fixed-t_f min-energy PMP extremal with
   no orbit, no CR3BP quantity and no pumpkyn call in it — the delegate stays
   at the old path (see table for the equivalence gate). `ms_tfmin` and the
