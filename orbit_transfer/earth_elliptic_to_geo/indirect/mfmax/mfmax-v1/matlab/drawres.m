...
figure(fig1);
subplot(4,1,1);
title(['Control vs time, T_{max} = ' num2str(Tmax) ' N, clf = ' num2str(par(1)) ...
       ', m_f = ' num2str(M(N)) ' kg.']);
hold on;
plot(t,u(1,:));
xlabel('time');
...
plot3(r(1,:),r(2,:),r(3,:),'g');
% --------------------------------
% modification 2: 21th August 2007
% different color for thrust arcs and no thrust arcs
% author: J. Gergaud
J = find(sum(u.^2,1)~=0);
nJ = length(J);
j = 1;
...
%State
subplot(8,2,1);
title(['T_{max} = ' num2str(Tmax) ' N, clf = ' num2str(par(1)) ...
       ' m_f = ' num2str(M(N)) ' kg.']);
hold on;
ylabel ('P');
plot(t,P);
...
%
% --------------------------------
% modification 1: 16th August 2007
% author: J. Gergaud
% swith functions
figure
% singularity function \phi
ZZ = zeros(1,N);
W = 1+ex.*cos(L)+ey.*sin(L);
Z = hx.*sin(L)-hy.*cos(L);
C = 1+hx.^2+hy.^2;
f1 = [ZZ ; sqrt(P/mu0).*sin(L) ; -sqrt(P/mu0).*cos(L) ; ZZ ; ZZ ; ZZ];
f2 = [sqrt(P/mu0).*(2*P)./W  ; sqrt(P/mu0).*(cos(L)+(ex+cos(L))./W) ; ...
      sqrt(P/mu0).*(sin(L)+(ey+sin(L))./W) ; ZZ ; ZZ ; ZZ];
f3 = [ZZ ; (sqrt(P/mu0)./W).*(-Z.*ey) ; (sqrt(P/mu0)./W).*(Z.*ex) ; ...
      (sqrt(P/mu0)./W).*(C/2).*cos(L) ; (sqrt(P/mu0)./W).*(C/2).*sin(L) ; ...
	  (sqrt(P/mu0)./W).*(Z)];
Phi1 = sum(f1.*p(1:6,:),1);	
Phi2 = sum(f2.*p(1:6,:),1);
Phi3 = sum(f3.*p(1:6,:),1);
normPhi2 = (Phi1.^2+Phi2.^2+Phi3.^2);
normPhi = sqrt(normPhi2);
MinnormPhi = min(normPhi)
MaxnormPhi = max(normPhi)
cTmax = (3600^2) / 1e6; % conversion from Newtons
Tmax = Tmax*cTmax;
unmoinsTmaxBetapm0 = 1 - Tmax*Beta*p(7,1)
% For (\phi|\dot{\phi})=0
subplot(4,1,1:2)
plot3(Phi1,Phi2,Phi3)
v = axis;
hold on
...
Psi = 1 - Tmax*Beta.*p(7,:) - Tmax*normPhi./M;
subplot(4,1,4) 
plot(t,Psi)
hold on
plot(t,zeros(size(t)))
