...
for kf = 1:numel(files)
    ...
    if nS == 1
        ...
        ndp = nd_propulsion([], ob.ispS, ob.m0kg, ob.lStar, ob.tStar);
    ...
    sheets(nS,1).z8              = z8;
    % VERDICTS, when the sheet file carries them. A sheet built by
    % sheet_to_catalog_file already knows each entry's conjugate verdict and
    % hypothesis gates -- they were established to admit the entry at all --
    % so they ride along instead of needing a separate census pass.
    if isfield(Q, 'CONJ'),  sheets(nS,1).conj_pass     = Q.CONJ;   end
...
% provenance the schema demands whenever sheets carry verdicts
if isfield(sheets, 'conj_pass')
    nP = 0;  nF = 0;
...
if isfield(sheets, 'gate_dimS')
    cat_.hyp_gates = struct('date', datestr(now, 'yyyy-mm-dd'), ...
        'instrument', 'costate_common/mintime_hypothesis_gates', ...
        'meaning', ['gate_min_lamv: min_t |lam_v| (strong Legendre, need > 0); ' ...
                    'gate_min_qmt: min_t (|lam_v|/m + lam_m/c) (all-burn is the PMP control, need > 0); ' ...
                    'gate_dimS: lift-space dimension (1 = no abnormal lift). With conj_pass = 1 ' ...
                    'these are the BCT sufficiency hypotheses, sampled.']);
end
