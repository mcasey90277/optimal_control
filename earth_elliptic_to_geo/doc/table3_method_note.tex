...
not stated numerically in \cite{ghm2004}, but its benchmark constants are those
of Caillau \& Noailles \cite{cn2001} (the min-time predecessor on the identical
transfer), whose p.\,255 gives the mass-flow coefficient
$\delta = 0.05112~\mathrm{km}^{-1}\mathrm{s}$ in $\dot m = -\delta\lVert
\mathbf{u}\rVert$. Since $\delta = 1/c = 1/(I_{sp}g_0)$, this fixes
$c = 1/\delta = 19.562$~km/s and hence $I_{sp} = c/g_0 = \mathbf{1994.8~s}$. We
run at the default $2000$~s ($0.27\%$ high; absolute masses $\sim0.3$~kg high,
trajectory structure $I_{sp}$-independent); pass $I_{sp}=1994.8$~s for the exact
source value. All other benchmark constants ($P^0,P^f,e^0,L^0,m^0,\mu^0$) are
confirmed identical to \cite{cn2001}.
...
\bibitem{cn2001} J.-B.~Caillau and J.~Noailles,
``Coplanar control of a satellite around the Earth,''
\emph{ESAIM: Control, Optimisation and Calculus of Variations}, 6, 2001, 239--258
(benchmark constants incl. $\delta$/$I_{sp}$, p.~255).
