...
\end{tikzpicture}
\caption{The ELFO pipeline. The generators above build the seed layer once;
\file{run\_elfo\_minfuel} is then the campaign's front door and carries the
same six-stage shape as its tulip sibling --- parameters, solve, refine
(deferred: PSR is single-primary tulip machinery), export, verify (including
the generic FOC gate), movie --- with \texttt{movieMode} and \texttt{rerun}
flags and per-stage resume. \file{elfo\_run\_one} is the function-call route
that \file{elfo\_batch.sh} drives for sweeps.}
\label{fig:elfopipe}
\end{figure}
