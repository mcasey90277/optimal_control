...
% --- results layout ---------------------------------------------------------
% PSR VENDORED COPY (2026-07-12): this file lives in PSR/lib, but the input
% DATA it points at -- the min-ENERGY backbones (energy seeds) and existing
% bang-bang solutions (neighbor seeds) -- stays in its home in sundman_minfuel
% ("reference in place"). So dirs point at ../../sundman_minfuel/results, NOT a
% PSR-local results tree. (PSR run outputs go to PSR/results and PSR_data via
% explicit paths in run_psr, so nothing is written back into sundman_minfuel.)
r = fullfile(here, '..', '..', 'sundman_minfuel', 'results');
