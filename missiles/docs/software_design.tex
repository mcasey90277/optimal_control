...
\date{7 August 2026; second edition 8 August 2026; third edition 9 August 2026;
      fourth edition 9 August 2026}
...
\medskip
\noindent\textbf{Third edition, 9 August 2026.} The second edition described
\code{BM/run\_ballistic\_target} as still taking its azimuth from the closed
form and still refusing a rotating Earth. It was converted later the same day
and both statements are now false; they are corrected in
Section~\ref{sec:entry}, which gains the measured cost of that conversion and
the one thing it did not buy --- a reachable envelope certified at the azimuth
the script actually flies. Section~\ref{sec:limits} is rewritten around the
seven open items two-axis targeting opened in place of the two it closed.

\medskip
\noindent\textbf{Fourth edition, 9 August 2026.} Two changes, both small, and
one of them exposes something the first three editions all missed: until
\code{dbc8e4a} \textbf{nothing in this library could write a figure to disk}.
\code{coorbital.viz.saveFigure} closes that as shared \pkg{viz} infrastructure
and \code{BM/run\_ballistic\_target} gains one user-block entry,
\code{plotFile}, on top of it (Sections~\ref{sec:viz} and~\ref{sec:entry}).
Separately, \code{globeMovie}'s caption gained the semi-transparent backdrop
the altitude inset and the legend already had, which is a design point and not
a cosmetic fix --- the same legibility rule applied to two of three overlays is
not a rule. Every count this document states as \emph{current} is re-measured
against 61 \code{.m} files and a suite that now reads \textbf{24 passed}; the
earlier editions' dated measurement tables are left as the dated records they
are, and Section~\ref{sec:verify} carries the fourth edition's own.
\end{abstract}
...
``specified in full and never implemented'' --- was written.

\textbf{Recounted for this edition, 9 August 2026: 61, 29 and 25.} Twenty-six
public library functions plus the same three package-private helpers under
\code{+coorbital}; \code{run\_tests.m} and twenty-four \code{test\_*.m} under
\code{tests/}; seven across \code{HGV/} and \code{BM/}. Two arrivals since the
third edition, each with its test: the closed-loop guidance spike's
\code{+coorbital/+guide/terminalConstraint.m}, and
\code{+coorbital/+viz/saveFigure.m}. The command is
\code{find missiles -name '*.m' | wc -l}; count it rather than quoting this
paragraph, which is what the third edition's own discrepancy list said about
...
\pkg{guide} & control      & control-schedule evaluation: open-loop \code{prescribed} and state-reading \code{pitchProgram}; plus \code{terminalConstraint}, the five burnout constraints PEG and VOA share, which is an interface and not a law \\
\pkg{prop}  & driver \emph{and} model & \code{phaseRun} and the three ODE events (propagator); \code{constThrust} (propulsion) \\
\pkg{viz}   & presentation & four figure functions, one export helper (\code{saveFigure}), and three package-private helpers \\
\bottomrule
\end{tabular}
\end{center}
...
\code{BM/run\_ballistic\_target} & launch point $\to$ destination & 3 & azimuth \emph{and} the mode's own range control, together \\
\bottomrule
\end{tabular}
\end{center}
...
And the last two rows are now the \emph{same} architecture, which they were not
when the second edition was written. \code{BM/run\_ballistic\_target} was
converted later the same day (\code{183d0e6}, corrected in \code{3cf70bb}): it
seeds on the closed-form bearing exactly as \code{run\_target} does, and
\code{coorbital.util.aimSolve} then drives the azimuth beside the mode's own
range control until both components of \code{missVector} are inside tolerance.
The solve was always a \pkg{util} function that applied verbatim; applying it
was scheduling, and the scheduling has happened.

\paragraph{What the conversion cost the ballistic script, measured
9~August~2026.} Same three columns as Section~\ref{sec:aimflow}'s table, on the
shipped $45^{\circ}$N $100^{\circ}$W $\to$ $62^{\circ}$N $28^{\circ}$W geometry
at $4828.045~\mathrm{km}$:
...
Case & Miss, seed only & Miss, solved & Residual evaluations \\
\midrule
Rotating Earth (\code{earthSpin} true) & $463\,211.19~\mathrm{m}$ & $52.46~\mathrm{m}$ & 7 \\
Shipped \code{'lofted'}                & $779.49~\mathrm{m}$      & $0.365~\mathrm{m}$  & 4 \\
Shipped \code{'depressed'}             & $457.27~\mathrm{m}$      & unchanged bit-for-bit & 1 \\
Shipped \code{'minimum-energy'}        & $39.01~\mathrm{m}$       & unchanged bit-for-bit & 1 \\
\bottomrule
\end{tabular}
\end{center}

Two structural readings. First, \textbf{the seed miss decomposes the way the
mechanism predicts}: the rotating case's $463~\mathrm{km}$ is
$-17\,764.17~\mathrm{m}$ down-range against $+462\,870.44~\mathrm{m}$
cross-range, a ratio of $26.1$ to $1$. A target being carried east would be a
down-range effect; a vehicle being deflected is a cross-range one, and the
decomposition is the evidence, not the total. Second, \textbf{the per-component
tolerance is what decides whether a shipped mode moves at all}: three of the
four rows have a seed inside $707.11~\mathrm{m}$ and converge at iteration
zero, and the one that moved, \code{'lofted'} at $779.49~\mathrm{m}$, is
outside it by $72~\mathrm{m}$. That last row deserves the same honesty
Section~\ref{sec:aimcoverage} applies to \code{run\_target}: its seed miss is
$779.490875~\mathrm{m}$ down-range against $-2.77\times10^{-5}~\mathrm{m}$
cross-range, so the Newton corrected it almost entirely on the \emph{loft}
axis and moved the azimuth by $6.3\times10^{-12}~\mathrm{rad}$. It exercises
the two-axis machinery --- one iteration, four evaluations, a real Jacobian and
a real step --- but it does not exercise the \emph{azimuth} axis. Only the
rotating case does that, at $-5.6295^{\circ}$ of aim. The whole run costs $701$ propagations rotating
and $733$ in the shipped minimum-energy mode, against $731$ before the
conversion --- the two-axis stage is two propagations on a mode that was
already spending seven hundred.

\paragraph{And one thing the conversion did not buy, which the design has to
own.} Stage 1 --- the max-range bracketing, the certified maximiser interval,
the two branch brackets and the minimum-energy minimisation --- runs entirely
at the \emph{seed} azimuth, deliberately, because range against loft is a curve
at a fixed azimuth and a surface otherwise, and a golden section on a surface
certifies nothing. So the reachable envelope, and therefore the too-far and
too-close refusals drawn against it, are quantities of an azimuth the script
may not end up flying. Measured size on this geometry: the maximum moves about
$0.28^{\circ}$ of loft per degree of azimuth, against a certified maximiser
interval $0.043^{\circ}$ wide, and turning the Earth on moves the envelope by
$30$ to $42~\mathrm{km}$ --- maximum range $5211.5 \to 5439.9~\mathrm{km}$, the
depressed floor $4708.5 \to 5085.8~\mathrm{km}$. The visible consequence is
that \code{'depressed'} plus \code{earthSpin} is refused for the shipped
target, which is correct physics on an easterly launch and not a solver fault.
The alternative --- re-bracketing inside every Newton evaluation --- would
multiply a seven-hundred-propagation mode by the cost of a whole bracketing and
would still certify nothing. \code{TODO.md} carries it as an open
approximation with those numbers attached.
...
(\code{run\_ballistic\_target}, re-counted 9 August 2026 after \code{plotFile}
joined it), with \code{run\_target} at 36. Every model
...
It is duplicated \emph{five} times --- once per entry script --- and
\code{TODO.md} lists the extraction into \pkg{util} as open, with the line
numbers, re-read on 9 August 2026 after three of the five had drifted, one of
them by more than seventeen hundred lines.

\paragraph{One block entry names a file, and there is no flag beside it.}
\code{BM/run\_ballistic\_target} writes its three figures to disk through
\code{coorbital.viz.saveFigure}, and the whole interface for that is one
whitelisted entry, \code{plotFile}: a path \textbf{stem without an extension},
defaulting to \code{fullfile(tempdir,'run\_ballistic\_target')} for the same
reason \code{movieFile} defaults there, that a picture is a build artefact and
does not belong in the source tree. Three properties of that design are
deliberate.

\begin{enumerate}
\item \textbf{The stem is the switch.} Saving happens when \code{showPlots} is
true \emph{and} the stem is non-empty; \code{plotFile = ''} draws the figures
without writing them. There is no second on/off flag, because two settings that
must agree are a state a caller can get wrong, and the script already
establishes the pattern with \code{movieFile} and \code{movieOn}. It is in the
whitelist, so a mistyped \code{'plotfile'} raises rather than silently doing
nothing --- which is design point 1 above doing exactly the work it was added
for.
\item \textbf{The suffixes are fixed, and name what each figure \emph{is}}:
\code{\_profile.png}, \code{\_ground\_track.png}, \code{\_globe.png}. The
alternative --- sanitising each figure's own title into a file name --- was
rejected on evidence already on disk. A title carries a miss distance, a minus
sign and a degree symbol, and what that sanitises to is
\code{china\_to\_la\_ground\_track\_10802\_km\_requi.png}: truncated, and a
\emph{different} name on every run, so the directory accumulates near-duplicates
that no longer correspond to anything. Fixed suffixes mean a re-run
\textbf{overwrites its own pictures}.
\item \textbf{The paths are printed and returned.} Printed in the style of the
existing \code{Movie:} line, and handed back on \code{info.plotFiles}, which is
present only when the figures were drawn \emph{and} saved --- the same
convention the rest of \code{info} follows, where a field's presence is itself
information.
\end{enumerate}

\code{HGV/run\_target} has the same gap and is \textbf{deliberately not wired},
which \code{TODO.md} carries as a scheduled follow-up rather than as a defect:
\code{saveFigure}'s header names both scripts as its intended callers, and doing
the second one in its own change keeps a diff that touches the block
\code{run\_target}'s pinned $511.2434604708133~\mathrm{m}$ is produced from
readable on its own.
...
The package has a \emph{fifth} public function, \code{saveFigure}, and it is the
one that deliberately does not take this argument list: it takes a figure handle
and a path. It is infrastructure rather than a figure function and is described
on its own below.
...
\code{globeMovie} is 979 lines, most of it header, and every visual feature in
...
\paragraph{The same backdrop, three times --- and the third one was late.} The
frame carries three overlays on top of the globe: the legend, the altitude
inset, and the caption. All three are white or light text over whatever the
camera happened to render behind them, and all three need the same remedy. The
legend got an opaque dark \code{Color} with white text when it was written. The
inset got a semi-transparent backdrop patch at $\alpha = 0.55$ after the glide
was measured descending behind its left edge. \textbf{The caption --- white bold
text at top centre, a figure-level annotation --- got nothing until
\code{1fffdf0}}, and on a high-latitude arc what it crosses is the polar ice
cap: measured on the Plesetsk-to-New-York flight, the middle of the caption
rendered white on white and the launch coordinates were unreadable for the
\emph{whole} flight, while both ends of the same line, over sky, were crisp. It
now carries \code{'BackgroundColor',[0.06 0.06 0.06]} at the same
\code{'FaceAlpha',0.55} --- low enough that the limb still reads through the
strip, which is why it is a backdrop and not an opaque bar.

The design point is not the fix, it is the shape of the defect. \emph{A
legibility rule applied to two of three overlays is not a rule; it is two
fixes.} Nothing in the package stated it, no test could see it --- the frame is
gone by the time the function returns, which is why \code{FrameFcn} exists at
all --- and the third overlay was therefore written without it and shipped
illegible. It is the shape Section~\ref{sec:blind} catalogues: a property held
by every existing case, enforced by nothing, and broken by the next case added.

\subsection{\code{saveFigure}: the one thing \pkg{viz} could not do}

Until \code{dbc8e4a} the package could draw a figure and could write an MP4,
and could not write a \emph{picture}. That gap survived three editions of this
document unremarked, and the evidence was sitting in \code{results/} the whole
time: every still already there when \code{dbc8e4a} landed --- the
\code{china\_to\_la\_*} and \code{pacific\_*} sets --- was produced by an ad-hoc
scratch driver in an earlier session, because no entry script could have
produced one.

\code{coorbital.viz.saveFigure(hFig,fileName,opts)} is the fifth public entry in
the package and the one that does not take \code{(traj,veh,env,opts)}: it takes
a figure handle and a path and returns the absolute path actually written. It is
\emph{infrastructure} rather than a figure function, and it exists for the same
reason \code{earthSurface} does --- so that two consumers cannot diverge. Four
decisions are made here rather than at each call site:

\begin{center}
\begin{tabular}{lp{4.0cm}p{6.4cm}}
\toprule
Decision & Default & Why not the obvious alternative \\
\midrule
Resolution & 200 dpi, an option & Zoomable and embeddable without resampling; a $6.5\times4.5$ inch figure lands near $1300\times900$ px. A slide and a paper want different answers, so it is an option and not a constant --- the same rule Section~\ref{sec:conv} states for the exaggeration factor \\
Background & \code{'current'}, \textbf{not white} & \code{exportgraphics} will lay a white card behind a dark figure. \code{globeMovie} styles its figure black, so a dark still exported on the default loses the starfield, the white legend text and the white annotation boxes. \code{'current'} reads the figure's own \code{Color}, so the caller need not know which it has \\
Missing directory & created & The entry scripts default their stem into \code{tempdir}, where it always exists, so this branch is reached only when a user has deliberately pointed elsewhere. Refusing that is a chore, not a safety property. A directory that \emph{cannot} be created still raises \\
After the write & size on disk read back & \code{exportgraphics} returns nothing, so a caller that checks only for the file existing is satisfied by a zero-byte stub from a failed write. Reading the size back is the cheapest available evidence that pixels arrived \\
\bottomrule
\end{tabular}
\end{center}

Two further properties are contracts rather than defaults. The extension is
\emph{optional} and defaults to \code{.png}, because the callers hold a path
\textbf{stem} --- one user-block entry from which several names are derived by
suffix --- and appending \code{'.png'} at each call site would be the same
string written once per figure. And \textbf{the figure is not closed and nothing
about it is modified}: ownership belongs to whoever created it, which is the
contract the rest of the package already keeps, and an export that disposed of
its subject would make saving a figure and looking at it mutually exclusive.

\paragraph{On the background default, one correction worth making explicitly.}
The dark styling belongs to \code{globeMovie} alone. \code{private/vizParent}
creates \emph{every} figure in this package with \code{'color',[1 1 1]},
\code{globe3D}'s still included, and \code{globeMovie} is the one function that
overrides it. So the \code{'current'} default is not there because \pkg{viz} is
a dark package; it is there because \pkg{viz} is a package with \emph{both}, and
a helper shared between them cannot assume either.

\code{tests/test\_saveFigure.m} measures rather than trusts, in the discipline
of Section~\ref{sec:test}: the resolution is read off the written pixels against
a second export at a known 100 dpi, the background off the corner pixel of a
deliberately black figure, and the write itself off \code{imread}. All three are
properties an assertion on the return value alone would miss.
...
The suite is 24 \code{test\_*.m} files against 36 library and script files:
re-measured 9 August 2026, $13\,250$ lines of test against $16\,654$ lines of
everything else, which is $44.3\,\%$ of the repository. (The first edition of
this document said ``roughly $60\,\%$'', and that was true when it was written;
the fraction fell because the two targeting scripts grew faster than their tests
did, not because any test was cut. The third edition's $44.9\,\%$ was measured
over 23 and 34 files on 8 August.) Several of the tests are longer than the code
...
Counted against the source, 9 August 2026: \textbf{26 public library functions,
23 with a self-demo, three ruled exceptions} --- \code{missileConst},
...
arrives. \textbf{Re-swept over all 61 \code{.m} files for this edition,
9 August 2026: zero \code{\%\#ok}, zero \code{for i =} or \code{for j =}, and
...
Section & Contains \\
\midrule
Recently closed        & \code{BM/run\_ballistic\_target.m}, now built; the per-phase \code{veh} field, now landed; the two targeting scripts' rotating-Earth refusal, now retired by two-axis targeting in \emph{both} of them; four applied reviews \\
Structural, deferred   & migrating the three older entry scripts off their EOM closures onto \code{phase.veh}; a state-scaled default \code{AbsTol}; the duplicated \code{overrideOf}/\code{maxOver} helpers and the deliberately duplicated \code{missVector}; the two \code{BM} scripts' missing re-integration payload; a degenerate-input sweep \\
Known limitations      & the seed-azimuth envelope and its approximate refusal gate; the minimum-energy loft held at its seed-azimuth minimiser; the unboxed stage-2 Newton; the \code{loftBracket} refusal's narrower \code{info}; the sufficient-but-not-necessary tolerance; \code{run\_target}'s uncoverable shipped case; \code{run\_ballistic\_target}'s jettisoned propellant; the one-sided phugoid warning; the optimistic booster structural coefficient; loose event tolerances \\
...
Documentation debt     & \code{docs/README.md}'s stale counts and its superseded rotating-Earth row; two unarchived reviews; no bibliography \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{Two rows of \code{TODO.md} were overtaken by this milestone, and one
of them was overtaken in its reasoning as well as in its status.} Both sat
under ``Known limitations'' in the version read for the second edition of this
document, both were written before the two-axis solve existed, and
\code{TODO.md} struck both on 9~August~2026. They are recorded here because the
\emph{reason} one of them was wrong is a design lesson and not a bookkeeping
one.

``Rotating-Earth targeting needs an outer azimuth iteration'' is now false for
\emph{both} targeting scripts, which fly it (Section~\ref{sec:aimflow} and the
ballistic table above). But the entry's \emph{mechanism} was wrong
independently of its status. It said the vehicle ``must be
...
and centrifugal terms. Both scripts' headers now say so, and both deleted the
eastward-drift figure they used to print
($\omega_E r_I \cos\phi_T t_{\text{flight}}$, $628~\mathrm{km}$ on the rotating
shipped \code{run\_target} case) precisely because that number measures a
mechanism the files no longer claim. The deflection they \emph{do} claim is
measured, not derived: $231.552~\mathrm{km}$ for \code{run\_target}, a factor
of $2.7$ smaller. \emph{A retracted mechanism's
number left standing under a corrected sentence is the harder error to notice
of the two, because the prose reads right.}

The ballistic conversion then supplied the discriminating evidence the
retraction had been argued for on general grounds: its rotating seed miss is
$26.1$ to $1$ cross-range over down-range. A moving target is a down-range
effect and a deflected vehicle is a cross-range one, so the decomposition
settles it in a way the total never could.

``Cross-range is measured and warned about, not solved'' is now false
everywhere. Both scripts drive it to zero, \code{info.crossWarn} is gone from
both, the $\arcsin$ cross-track relation is computed nowhere in the library,
and the cross-range line in each summary is evidence rather than a caveat.

What replaced those two rows is not nothing. Two-axis targeting opened seven
open items of its own, all of them consequences of solving on a surface with
machinery certified on a curve, and all of them are in \code{TODO.md} with
measured sizes: the seed-azimuth envelope and the approximate refusal gate it
drives; the minimum-energy loft held at its seed-azimuth minimiser while only
the cutoff is re-trimmed; the unboxed stage-2 Newton and the three layers of
diagnosis standing in place of a prohibition; the \code{loftBracket} refusal's
\code{info} not being a superset of the other three; the max-norm tolerance
being sufficient but not necessary; \code{missVector}'s deliberate duplication
between the two scripts; and \code{run\_target}'s shipped case, which cannot be
given two-axis coverage at any tolerance (Section~\ref{sec:aimcoverage}).

One addition this document generated in its second edition: \code{TODO.md}'s
documentation-debt section listed ``the two \LaTeX{} notes are forthcoming.''
Both exist, and \code{README.md} and \code{TODO.md} both stopped saying
otherwise on 9~August~2026.
...
brief, \code{docs/plan\_2026-08-08\_aim\_solve\_and\_closed\_loop.md}. For the
third edition, additionally: \code{BM/run\_ballistic\_target.m} in full ---
its header, its two-stage solve, its four refusal paths and its own
\code{missVector} --- and \code{tests/test\_runBallisticTarget.m}. For the
fourth edition, additionally: \code{+coorbital/+viz/saveFigure.m} in full,
\code{tests/test\_saveFigure.m}, \code{+coorbital/+viz/globeMovie.m}'s three
overlay blocks (legend, caption, inset backdrop),
\code{+coorbital/+viz/private/vizParent.m}, and
\code{BM/run\_ballistic\_target.m}'s user block, whitelist and figure block.
Every code fragment quoted above is copied from the source, not retyped.
...
\paragraph{Measured for the third edition, 9 August 2026, MATLAB R2025b.} Each
figure is read out of \code{info} at full precision on a run of the committed
code, not carried across from a commit message.
...
Measurement & Result \\
\midrule
Test suite                             & 23 passed, 0 failed, zero warnings, $314.6~\mathrm{s}$ wall \\
Ballistic, \code{'minimum-energy'}     & $39.0092688~\mathrm{m}$; \code{aimIter} 0, \code{aimEval} 1, \code{nProp} 733 \\
Ballistic, \code{'depressed'}          & $457.269860~\mathrm{m}$; \code{aimIter} 0, \code{aimEval} 1, \code{nProp} 64 \\
Ballistic, \code{'lofted'}             & seed $779.490875 \to 0.365282~\mathrm{m}$; 1 iteration, \code{nProp} 67 \\
Ballistic, \code{earthSpin} true       & seed $463\,211.19 \to 52.461~\mathrm{m}$; 2 iterations, \code{nProp} 701 \\
\quad seed decomposition               & $-17\,764.17$ down-range, $+462\,870.44~\mathrm{m}$ across ($26.1{:}1$) \\
\quad aim correction                   & $-0.0982538~\mathrm{rad} = -5.6295^\circ$ \\
\quad envelope shift                   & maximum range $5211.525 \to 5439.857~\mathrm{km}$ \\
Targeting, shipped case                & $511.2434605~\mathrm{m}$; \code{aimIter} 0, \code{aimEval} 1, \code{nProp} 14 \\
Targeting, \code{earthSpin} true       & seed $231\,551.628 \to 4.361~\mathrm{m}$; \code{nProp} 23 \\
File count under \code{missiles/}      & 59 \code{.m}: 25 public, 3 private, 5 entry, 2 vehicle, 24 test \\
Self-demo count, \pkg{coorbital}       & 25 public functions, 22 demos, 3 ruled exceptions \\
Wall clock, four entry-script runs     & $2.14$, $3.42$, $27.17$ and $27.04~\mathrm{s}$ \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Measured for the fourth edition, 9 August 2026, MATLAB R2025b.} The
suite was run twice, both green; the wall clock below is the second run, timed
around the whole \code{matlab -batch} invocation.
...
Measurement & Result \\
\midrule
Test suite                             & \textbf{24 passed, 0 failed}, zero warnings, $305~\mathrm{s}$ wall \\
File count under \code{missiles/}      & \textbf{61} \code{.m}: 26 public, 3 private, 5 entry, 2 vehicle, 25 test \\
\quad by folder                        & 29 under \code{+coorbital}, 25 under \code{tests/}, 7 across \code{HGV/} and \code{BM/} \\
Self-demo count, \pkg{coorbital}       & 26 public functions, 23 demos, 3 ruled exceptions \\
Convention sweep, 61 \code{.m} files   & 0 \code{\%\#ok}, 0 \code{for i/j =}, 1 \code{norm(} in a comment \\
Line counts                            & $13\,250$ test (24 files) against $16\,654$ library and script (36 files) \\
Override whitelist, longest            & \code{BM/run\_ballistic\_target}, 38 entries \\
\code{globeMovie} length               & 979 lines \\
\bottomrule
\end{tabular}
\end{center}

Two of the third edition's own recorded discrepancies are now closed, and are
noted here rather than edited out of the list below, which is a dated record.
Item~1, \code{docs/README.md}'s ``39 \code{.m} files'', was corrected in
\code{40d5449} and that section now carries the current count. Item~4's
statement that the longest whitelist is \code{BM/run\_ballistic\_target} at 37
was true when written; \code{plotFile} made it 38.

\paragraph{Not measured for the fourth edition.} No figure was exported and no
movie rendered for this document. The \code{saveFigure} properties described in
Section~\ref{sec:viz} --- the 200 dpi, the \code{'current'} background, the
byte-check --- are read off the source and off
\code{tests/test\_saveFigure.m}, which measures all three; the caption's
illegibility on the Plesetsk-to-New-York arc is the measurement recorded in
\code{1fffdf0} and in the source comment beside the annotation, not one taken
here.

Carried forward from the first edition, 7 August 2026, and not re-run here
because nothing since can move them:
...
\begin{enumerate}
\item \code{docs/README.md}'s conventions section says the style sweep covered
``39 \code{.m} files''. There are 59 as of 9 August 2026. \code{TODO.md}
already logs this as a count refresh; the sweep is still clean.
\item \code{DESIGN.md} \S11.1's ``17 library functions, 14 with a self-demo''
predates \pkg{viz}. The count on 9 August 2026 is 25 and 22, with the same
three ruled exceptions. \S11.1 is a dated record and is correctly left
unedited.
