...
$3535.4$~kg of propellant burned, and the residual is shown to be, in both
magnitude and \emph{sign}, the model error of the convex formulation's
Taylor mass bound rather than a discretization gap. The certified optimum is
...
\end{equation}
%
which is the separated (compressed) form of the third-order collocation
scheme in Kelly~\cite{kelly2017}. The annulus \eqref{eq:annulus} is imposed
...
throttle band on both sides. Both bounds are convex functions of $z$, so the
fixed-$t_f$ subproblem is a genuine convex program (SOCP-plus-quadratic),
and by convexity any local optimum is the global one~\cite{boyd2004} ---
which is the structural advantage this route buys in exchange for the
approximation in \eqref{eq:taylor}.

Conservatism has a testable consequence, which we will use in
\S\ref{sec:crossval}: because the convex problem is a \emph{restriction} of
the true problem, its optimal final mass can only be less than or equal to
the collocation solver's. The sign of the cross-method residual is
therefore predicted, not merely bounded.
...
\end{equation}
%
The dynamics are linear and $\mathcal{H}$ is independent of $\rvec$ and
$\vvec$ beyond the terms shown, so the costate equations are
%
\begin{equation}
...
forces $\lamr = \mathbf{0}$ and $\lamv \equiv \mathbf{0}$ identically; with
the terminal transversality conditions of a problem whose position and
velocity are both fully constrained at $t_f$, that contradicts
nontriviality of the costate. Therefore $\lamv$ vanishes at most at isolated
instants and \eqref{eq:primer} holds a.e.
\end{proof}
...
the velocity-costate direction, and its magnitude is bang--bang}. We do not
...
than the collocation route does, and must burn slightly \emph{more}
propellant. The measured sign agrees: $m_V(t_f) = 26464.161 <
26464.589 = m_C(t_f)$. The magnitude, $\sim 0.02\%$ of the fuel burned, is
...
converged NLP proves less than it appears to. Its architecture is six gates.
...
\paragraph{G2ff --- feed-forward annulus feasibility.}
...
$175.4$~m/s speed. No initial maximum-thrust arc. No singular arc: gate G5
...
defect that was measured, diagnosed, and fixed --- in that order --- and
each fix was derived rather than tuned.
...
\end{equation}
%
The slow (position-error) pole is the ratio,
%
\begin{equation}
\text{pole} \;=\; \sqrt{\frac{q_r}{\,2m\sqrt{q_r r} + q_v\,}} ,
\label{eq:pole}
\end{equation}
%
...
\end{equation}
%
a $10$~s time constant on a $16.6$~s flight --- \textbf{and no choice of $R$
whatsoever can beat it.} (At the shipped $r = 10^{-9}$ the term
$2m\sqrt{q_r r} = 0.0187$ is already $1.9\times$ larger than $q_v$, putting
the actual pole at $0.059$~rad/s, further below the bound.) Every $R$ sweep
had been buying gain \emph{magnitude} on both channels while the pole stayed
pinned by the $q_r{:}q_v$ ratio alone.
...
solution measures $K_{11}/m = 0.4808$ against the predicted $\omega^2 =
0.49$, and $K_{14}/m = 1.377$ against $2\zeta\omega = 1.40$.
...
for most of the braking arc with roughly $2\%$ of net deceleration to spare.
There is nothing left to add. Feedback cannot command thrust the engine does
not have.
...
$\etaT = 0.93$ was adjudicated first and shipped briefly, then \emph{measured
...
The shipped controller is a phase-scheduled TVLQR with $R = 10^{-9}I$,
...
The single failure is instructive rather than mysterious. Run 84 drew an
...
touchdowns the worst lies inside the p95 figure.
...
Drag \emph{saves} $434.7$~kg of propellant --- $12.3\%$ of the vacuum
...
footprint is the wind: a steady $25.7$~m/s crosswind pushes the vehicle
farther than the fixed-reference tracker fully recovers within the burn.
...
enabling it is the natural first step.
