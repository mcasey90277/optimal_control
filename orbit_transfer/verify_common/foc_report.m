...
%           .lamMassEndRel    free-mass transversality residual (relative) [scalar]
...
% --- Transversality: free-mass at terminal time ----------------------------
if isnan(rep.lamMassEndRel)
    s_trans = '--';
else
    s_trans = make_status_str(rep.lamMassEndRel, tolTrans, '<=');
end
fprintf(' Transversality |lam_m(tf)| rel     : %10.3e                 %s\n', ...
    rep.lamMassEndRel, s_trans);
