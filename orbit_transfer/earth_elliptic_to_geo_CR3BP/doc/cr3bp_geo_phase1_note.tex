...
\node[box, below=of e2b] (mu) {\textbf{$\mu$-continuation (gravity bridge)}\\
...
\emph{This section is deliberately speculative} --- it records the position
this repository's campaigns currently support, to be revised as evidence
...
degrade with \emph{arc sensitivity} --- the shooting Jacobian is a state
transition matrix whose norm grows roughly exponentially with revolution
count, amplified at each deep perigee pass --- while direct (collocation)
methods degrade with \emph{combinatorial structure} (bang-bang switch
discovery) and mesh resolution.
...
\item \textbf{Precision and certification once seeded: indirect.} With the
  ...
  shooting solve delivers machine-precision switch times and true costates
  at negligible cost --- its weakness (initialization) having been paid by
  the direct stage. The direct-seeded indirect finish is, in our view, the
  right marriage.
\item \textbf{The instructive exception} \cite{BCP2010}: indirect shooting
  ...
  kills \emph{cold or unguarded} shooting, and nonsmooth structure
  compounds it, but neither is fatal alone.
\end{itemize}
...
switches) the uniform mesh under-resolves switch clusters. One prerequisite
gates it: PMP steering needs the switching function, and the two-body primer
reconstruction is \emph{invalid under lunar gravity} until the ballistic
third-body rate is subtracted (\S 8). The CR3BP-aware primer and PSR are
therefore one work package: the corrected switching function both verifies
the structure and steers its refinement.
...
\item \textbf{Bang-bang second-order conditions via the induced
  ...
  Hessian, with no additional zeros of the switching function, is a genuine
  sufficient condition. Naturally shared with PSR: both live on the
...
\item \textbf{Indirect-side conjugate-point test.} Integrate the Jacobi
  ...
  $(0, t_f]$ \cite{BCP2010} --- the classical certificate, planned with the
  Phase-2 indirect solver, and dependent on the same CR3BP-aware costate
  reconstruction as PSR.
\end{enumerate}
