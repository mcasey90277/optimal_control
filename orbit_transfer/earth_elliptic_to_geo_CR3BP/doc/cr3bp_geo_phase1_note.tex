...
order. The four quarter-phase samples decompose exactly into harmonics
through second order:
\[
\Delta m_f(\varphi_0) \;\approx\;
\underbrace{+3.0}_{\text{mean}}
\;\underbrace{-\,8.5\cos\varphi_0 - 1.7\sin\varphi_0}_{\text{dipole}}
\;\underbrace{+\,57.5\cos 2\varphi_0}_{\text{quadrupole}}
\quad[\mathrm{g}].
\]
Quadrupole dominance is exactly what the physics prescribes: the tidal
field --- the difference between the direct lunar attraction and the
indirect (Earth-acceleration) term of Eq.~(3) --- derives from a potential
proportional to $P_2(\cos\psi)$ and is therefore invariant under
Moon\,$\to$\,$-$Moon reflection. At this rung the transfer spans only
...
month, at zero cost to the trajectory design. Whether this scales at lower
...
certificate as the rest of the campaign (\S 8).
...
\subsection{Two regimes, one consistent picture}

Read together with \S\ref{sec:phase}, the ladder completes a two-regime
description of lunar influence on this transfer family. While the transfer
is short against the lunar month, the tidal geometry set at departure
persists: the effect sits on the $\varphi_0$-dependent quadrupole
(\S\ref{sec:phase}), and the plateau value $\approx 50$\,g is simply the
$\varphi_0 = 0$ slice of that curve. As $t_f$ stretches past a lunar month
the Moon sweeps many cycles during the transfer, the quadrupole
contribution averages down, and what survives is a phase-independent
secular benefit of order $30$\,g --- still comfortably nonzero at $19$
lunar months. The corresponding operational statement: departure-epoch
selection is worth $\sim 125$\,g peak-to-peak at the fast end of the
ladder and, by this mechanism, approximately nothing at the slow end ---
though the slow end keeps its secular $\sim 30$\,g regardless of epoch.
A directly testable corollary (recorded, not yet run): a $\varphi_0$ sweep
at $0.2$ or $0.1$\,N should show the quadrupole amplitude collapsed.
...
\item \textbf{The predicted deep-rung wall did not exist.} The two-body
  campaign needed dedicated machinery (adaptive-$\varepsilon$ bisection,
  staged recovery) at $0.2$/$0.1$\,N; this campaign's front-door chain,
  lifted and properly budgeted, carried both rungs through
  $\varepsilon \to 0$ on the standard schedule. In hindsight a substantial
  part of the historical deep-rung difficulty is attributable to the
  unlifted formulation, not to the physics of sharpening.
...
phase-coherent regime, decaying past one lunar month of transfer time to a
phase-independent secular benefit of $\approx 30$\,g. At this problem's
...
(\S 8), deep-rung defects grow to $5\times10^{-13}$ (seven orders inside
