...
muts = { @(c) rmfield(c, 'axis3'),                 'missing axis3'; ...
         ...
         @(c) setIsp(c, 1710),                     'isp_s inconsistent with c_nd (review P0.1)' };
for k = 1:size(muts, 1)
...
c = struct('name', 'test_v3_minfuel', 'description', 'synthetic', ...
    ...
    'thruster', struct('isp_s', 900, 'm0_kg', 150, ...
                       'c_nd', 900*9.80665*382981.289129055/(1000*389703.264829278)), ...
...
function c = setIsp(c, isp)
% SETISP  Relabel the Isp without touching c_nd (the shipped-catalog bug).
% INPUTS: c; isp [s].  OUTPUTS: c.
c.thruster.isp_s = isp;
end
