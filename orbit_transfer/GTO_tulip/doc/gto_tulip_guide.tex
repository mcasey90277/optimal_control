...
is paid elsewhere, and it was misidentified for a month. It is \emph{not} that
the winding number cannot grow (the thrust ladder later walked 25 to
\SI{20}{\milli\newton} on this very formulation, \S\ref{sec:open}). It is
that the two conditions together restrict the problem: every physical
trajectory has its own regularized length,
\begin{equation}
\tau_f \;=\; \int_0^{t_f}\frac{\ud t}{\kappa(\vect{x}(t))} ,
\end{equation}
so holding $\tau_f$ at the seed's value adds the isoperimetric constraint
$\int_0^{t_f}\ud t/\kappa = \tau_{f,0}$ to the fixed-time problem, and the
solutions are extremals of that restricted problem. First-order checks cannot
see it --- the control minimization is unaffected; only the costate equation
acquires a $-\nabla\kappa\,(K/\kappa)$ term. It was found by external code
review (2026-09-06) and measured the same day: releasing $\tau_f$ through the
free-time solver's \texttt{cScale} slack state at the \emph{same} pinned $t_f$
raises $m_f$ by $2.06\times10^{-4}$ on the published flagship and
$2.46\times10^{-4}$ on the best certified row ($3$--$4$~g of propellant,
$\texttt{cScale}\approx1.005$). Those two rows sit at $1.15\times$, where the
seed's $\tau_{f,0}$ came from, and they are the \emph{least} restricted rows
of the campaign: the whole stored front carries that same
$\tau_{f,0}=151.68$ (the neighbour seed rescales the time state to the new
$t_f$ but never $\tau_{f,0}$), so the constraint bites harder the further
$t_f$ is from $1.15\times$. Re-solving the front free-$\tau_f$ (2026-09-07, all 17 rows, direct $\varepsilon=0$, defects $\le2\times10^{-11}$) moves
$m_f$ by $3\times10^{-5}$ at $1.14\times$ up to $9.4\times10^{-3}$ at
$1.40\times$ (\SI{142}{\gram}), median $2.9\times10^{-3}$ --- the size of
the basin spreads --- with $\texttt{cScale}$ from $0.982$ to $1.047$, and the
front minimum moves from $1.65\times$ (\SI{2.434}{\kilo\metre\per\second})
to $1.70\times$ (\SI{2.374}{\kilo\metre\per\second}, $m_f=0.89112$). Every
$m_f$ in this guide from the fixed-$\tau_f$ engine is therefore a lower bound
on the free-$\tau_f$ optimum by a row-dependent amount; the numbers to quote
are the free-$\tau_f$ ones. The fix is now in the engine (2026-09-07):
\texttt{casadi\_minfuel\_sundman} takes \texttt{opts.freeTauf}, which adds
the constant slack state $c$ with $\ud t/\ud\tau = c\,\kappa$ and
$\ud c/\ud\tau = 0$ (Betts' sparse free-time trick), so the regularized
length is effectively $c\,\tau_{f,0}$ while $t(\tau_f)=t_f$ stays pinned
and the KKT matrix stays banded; off, the solver is byte-identical to the one
every stored artifact was built with. \texttt{minfuel\_at\_tf} now runs it
by default, stores the effective length as the row's $\tau_{f,0}$ so no
neighbour seed inherits a length its solution did not have, and writes free
rows to \texttt{results/minfuel\_freetauf/}. A free-$\tau_f$ solution with
slack $c$ is exactly a fixed-$\tau_f$ KKT point at $c\,\tau_{f,0}$ (rows
1--8 of the defects and the objective coincide), which is how the two engine
modes were checked against each other: the port reproduces the free-time
solver's flagship answer to $6\times10^{-13}$ in $m_f$ from the fixed seed,
and the fixed engine sits still at the effective length to $5\times10^{-13}$.
...
\subsection{The mesh is inherited, not chosen}
\label{sec:mesh}

This is the single most important thing to understand before asking whether a
solution is converged, and it is easy to miss because no parameter announces it.

\paragraph{What actually sets the node placement.} Three quantities define the
discretization: the node count $N$, the normalized grid
$\sigma_k \in [0,1]$, and the total regularized length $\tau_f$. \emph{None of
them is a parameter of the front door.} All three are read off the energy
backbone at the requested $t_f$:
\begin{center}
\file{results/energy/energy\_f\#\#\#\#.mat} $\;\longrightarrow\;$
\texttt{sigma}, \texttt{tauf0}, and $N=\mathrm{numel}(\texttt{sigma})$
\end{center}
and every downstream solve at that $t_f$ --- the tight re-clean, all thirteen
$\varepsilon$ steps, any neighbour-seeded continuation --- inherits them
unchanged. The flagship runs at $N=4001$ because that is what the backbone has,
not because $4001$ was chosen or justified.

\paragraph{Where the nodes land, mathematically.} With $\tau = \sigma\,\tau_f$
and the Sundman law $\ud t/\ud\tau = \kappa = r_1^{\,p}$, the physical step over
interval $k$ is
\begin{equation}
\Delta t_k \;\simeq\; \tau_f\,\kappa(\vect{x}_k)\,\Delta\sigma_k
\;=\; \tau_f\, r_1(\vect{x}_k)^{p}\,\Delta\sigma_k ,
\qquad p = \tfrac{3}{2}.
\label{eq:dtk}
\end{equation}
\emph{Both} factors matter, and the second is easy to overlook: the $\sigma$
grid is \textbf{not uniform}. Measured across every energy backbone,
$\max\Delta\sigma_k / \min\Delta\sigma_k \approx 1.6\times10^{8}$ with
coefficient of variation $0.50$ --- and the grid is bit-identical at every
$t_f$. It is not a designed grid at all: it is inherited unchanged from the
no-resample map of the original time-mesh seed, and has been carried forward
into every backbone and every ladder rung since. The mesh is, quite literally,
a fossil of the first solution this campaign ever produced.

This has a practical consequence for anyone checking the discretization.
Plotting $\Delta t_k$ against $r_1$ does \emph{not} test the Sundman law,
because it conflates $\kappa$ with $\Delta\sigma_k$; the exponent it returns is
meaningless. The correct test divides the spacing out and fits
$\Delta t_k/\Delta\sigma_k \propto r_1^{\,p}$, which recovers $p = 1.500$ against
a nominal $1.5$ to three decimals. \file{certify/mesh\_diagnostic.m} does this
and reports both quantities separately.
So node density in \emph{time} scales as $r_1^{-3/2}$: dense at perigee, sparse
at apogee. In arc length, $\ud s = \|\vect{v}\|\ud t$, and near perigee
$\|\vect{v}\|\sim r_1^{-1/2}$, so $\ud s/\ud\sigma \sim r_1$ --- the mesh is
still finest exactly where the dynamics are fastest. For context, $p=1$ would
make the grid uniform in eccentric anomaly and $p=2$ uniform in true anomaly;
$p=3/2$ sits deliberately between the two.

Equation~\eqref{eq:dtk} is worth reading carefully, because it says the mesh
adapts to the trajectory's \emph{geometry} --- through $r_1$ --- and to nothing
else. It does not adapt to the \emph{solution}: the switching structure, where
the control is discontinuous and accuracy is hardest to obtain, has no influence
on where nodes are placed.

\paragraph{Why it is frozen: the no-resample discipline.} Interpolating a
converged solution onto a different mesh does not merely lose a little accuracy
--- it reintroduces a defect floor of order $10^{-2}$ that pins IPOPT in
restoration and never clears. The campaign's working recipe is therefore to map
a collocation-feasible solution into $\tau$ using its \emph{own} nodes, with no
interpolation anywhere. That discipline is what makes the $\varepsilon$ homotopy
work at all, and its unavoidable consequence is that $N$ cannot be varied
downstream: changing $N$ means resampling, and resampling is the thing that was
outlawed.

\paragraph{The consequence, stated plainly.} There is no mesh knob, and that is
not an oversight --- it follows from the no-resample rule. But it means
\textbf{the question ``is this mesh adequate?'' cannot be answered from inside
the pipeline}, because there is nothing to vary. Answering it requires
generating backbones at several densities with \file{gen\_tulip\_energy\_2p} and
comparing solutions across them: observed order of accuracy, and whether final
mass and switch times converge. That study is specified and has not been run, so
at present the honest position is that $N=4001$ is \emph{sufficient in practice}
and \emph{unjustified in theory}.

\paragraph{The one mechanism that does change the mesh.} PSR refinement
(stage~3) is the exception, and it is deliberately one-directional: it measures
where the PMP switching function localizes each switch worst, \emph{adds} nodes
in those intervals, and re-solves --- always starting from the backbone, never
coarsening, never resampling the existing nodes. It sharpens switch times below
the original mesh width. It does not, and is not intended to, establish that the
underlying mesh was adequate to begin with.
...
\begin{itemize}
\item \textbf{It is a statement about a restricted problem.} With $\tau_f$
fixed the NLP carries the isoperimetric constraint of \S3.2; the certified
$m_f$ values are lower bounds on the free-$\tau_f$ optima --- by
$2\times10^{-4}$ at $1.15\times$ but by up to $9.4\times10^{-3}$ elsewhere on
the front (measured 2026-09-06/07 on 17 rows, \S3.2). The rows are genuine extremals of the
problem the solver poses, not of the problem the campaign states.
...
\item \textbf{There are at least six certified optima at the flagship $t_f$,
and the published one is not the best.} The energy-backbone route and the
\file{run\_certified\_minfuel} chain converge machine-tight to 25 switches at
the \emph{same} $t_f$ and land on different local optima ($m_f = 0.847086$
versus $0.849066$). A thirteen-seed sweep on the flagship's own mesh
(\file{process/BASIN\_1150\_SWEEP.md}, 2026-07-29) found six, with 24, 25 and
26 switches, and its best --- 24 switches, $m_f = 0.850087$, \SI{2.2487}{kg}
of propellant, \SI{15.3}{g} (0.68\%) better than the flagship --- is certified
at parity with it. The seed route, and even the continuation path, decide
which basin a solve lands in. The multiplicity is concentrated at the
short-$t_f$ end of the front (\file{process/FRONT\_SWEEP.md}): report the best
certified row at each $t_f$ together with the multiplicity, and the switch
count as a band.
\end{itemize}
...
\midrule
best certified, basin sweep 2026-07-29 & 24 & 0.850087 & 3.3449 & 2.2487 \\
...
One caution on the scale: these factors are against $t_{f,\min} = 6.2907$~ND,
which is the minimum time to a \emph{different} tulip point (the max-$\dot y$
point of \file{indirect/min\_time/mintime\_params}). The certified minimum time
to the rendezvous the front actually targets is $5.8267$~ND
(\file{gen\_tulip\_mintime}, 2026-07-15), so against the target-consistent
anchor the front starts at $1.21\times$, the flagship sits at $1.24\times$, and
the unsolved band is $1.09$--$1.20\times$ --- a wall nine percent above minimum
time, not one. Physical $t_f$ and $\Delta V$ are unaffected; the configuration
value is left as it is because every stored artifact keys on it.

\paragraph{The thrust ladder.} Off-nominal thrust is reachable, but not far.
The recorded ``topology wall'' --- chaining \SI{25}{\milli\newton} to
\SI{20}{\milli\newton} in one step fails at the $\varepsilon=1$ energy step ---
turned out to be the continuation step size: walked in $\sim$4\% steps
(\file{run\_tulip\_ladder}, 2026-07-27) every rung to \SI{20}{\milli\newton}
converges machine-tight, and the \SI{20}{\milli\newton} rung sharpens to
$\varepsilon=0$ (11 switches, \SI{2.104}{kg},
\SI{3.113}{\kilo\meter\per\second}; certified-quality, not yet through the full
gate set). Two stacked limits remain: the fixed-$\tau_f$ solver stops at
\SI{21}{\milli\newton}, and the free-time (\texttt{cScale}) solver at
$\sim$\SI{19.5}{\milli\newton}, where the energy problem itself will not
converge. That ceiling is \emph{not} resolution (190 nodes per revolution) and
\emph{not} transfer-time margin (tested); the cause is open.
\file{run\_gto\_tulip} still refuses off-nominal thrust and points to the
ladder script.
