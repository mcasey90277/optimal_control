...
| conjugate point (Jacobi field) | catalog pipeline (any campaign with an `ms_bvp` solution) | **BUILT 2026-08-08** (migration #3) — free-final-time quotiented form, the two exact degeneracies (costate scaling, time reparametrization) handled, λ_m excluded; **runs**: golden-cells regression (3 engine cells, verdicts pinned), opt-in in `ms_tfmin` (`.conjTest`), advisory hook in `foc_check` (`opts.msInfo`, migration #5). **RUN AT CATALOG SCALE 2026-08-23** (`conj_catalog_pass`, K=24): all 15,896 entries of the five catalogs decided — **15,895 PASS / 1 FAIL / 0 unverified**; verdicts stored per sheet (`conj_pass`/`conj_ncross`/`conj_atfinal` + `.conj_test` provenance, schema-validated). The one refuted entry: DPO τ=2 → tulip Np=7, phases (2/3, 2/3), 15 N (nCross=1 interior, re-solve dz=1.6e-12). Still open: no Earth-campaign ms solution has exercised the `foc_check` hook. ~~free-time only~~ **FIXED-t_f FORM RESOLVED 2026-09-02**: the same instrument with the fixed-t_f spec (`freeTime=false`, no quotient — the running cost breaks the scaling invariance; rows [1:6 14] = the components vanishing under the terminal conditions; cols 8:14 — λ_m is live, the throttle law depends on it; the monitored det equals the ss BVP Jacobian at t_f), validated on the analytic LQ π-conjugate case (`tests/test_conj_fixedtf`, 5/5) and wired into `ms_minenergy`/`ms_minfuel` (`opts.conjTest`). Run over all 14 fixed-t_f records: 7/7 min-energy + 6/7 min-fuel PASS at the time. **CORRECTED 2026-09-05 (three-way code review, `DRO_tulip/reviews/minfuel_code_review_2026-09-05.md`; FINDINGS §22):** (a) the interior test must use the FULL state block `rows 1:7` — `[1:6 14]` is the terminal shooting Jacobian, meaningful only at t_f (a Jacobi field must vanish in r, v AND m for its zero-extension to be admissible); (b) on an initial coast the state block is structurally zero and the instrument had counted the exact-zero sample as a focal point — that was the one "refutation"; (c) the final segment was never sampled (8.3% of every transfer at K=12) and the `atFinal` rule subtracted a strictly interior crossing (0 firings measured across 14 + 4,405 verdicts). Instrument now: full block, equilibrated sign test, `.firstFullRank` coast skip, samples through t_f (`ms_bvp` returns `.Yend`), last bracket counted, spec echoed; `test_conj_fixedtf` 10/10 incl. coast + final-segment fixtures; golden 20/20 unchanged. Re-sweep (`run_conj_fixedtf_sweep`, verdicts bound to λ₀): **15/15 PASS**. The min-time catalogs' 18,249 verdicts predate the t_f sample — re-sweep pending. **70 mN phase library (2026-09-09..11):** the test is the DISCRIMINATOR (12 of 14 first-order-passing candidates refuted, FINDINGS 38); a dense singular-SPECTRUM companion (`conj_spectrum`) closes the sign test's two blind spots and LOCATES every candidate (start / endpoint / interior, two-level refinement) — 115/115 swept: 0 sign changes, 44 interior near-misses on a ridge, 0 zeros (FINDINGS 42). Necessary condition only — a pass does not certify, a fail refutes | `costate_common/ms_conjugate_test.m` (header = derivation); `golden_cells.m`; `DRO_tulip/indirect/ms_tfmin.m`; `verify_common/foc_check.m:318` |
...
done here; (iv) **E4 is NOT explained by E1 — tested the same evening.**
`certify_minfuel_pmp` run on both free-τ_f probe rows (X(1:8,:) with
τ_f' = cScale·τ_f0, so its dt/dτ = κ assumption holds exactly) fails identically
to the fixed rows: primer error 2.000 / 2.000, sign match 40.3% / 42.1%,
0/23 and 1/24 switches matched, while the raw duals of the same solves hit the
primer law on 100% of burn nodes (0.033° mean). Diagnosis
(`results/e1_freetauf/` + scratch `diag_ls_align`/`diag_ls_resid`): NOT the
sign-blind fit (only 1.3% of burn nodes anti-aligned) — it is SCATTER (61% of
burn nodes with |cos| < 0.99 against the thrust, 67% against the raw duals) and
a tenfold drift of |λ_LS|/|λ_raw| along the arc (0.008 → 0.073 by time
octile): the LS costate obeys a different growth law from the NLP's discrete
adjoint. The certifier's own header explanation (its trapezoid discretisation
of the CONTINUOUS adjoint is not the discrete adjoint of trapezoid collocation;
over ~40 revs the two solution families diverge) stands; the ∇κ term of E4 was
a red herring for this instrument. Construction error ruled out the same
evening: the certifier's recursion evaluated on the raw duals has a UNIFORM
per-interval relative residual (median 1.8e-4, 90th pct 3.8e-3, max 3.4e-2;
perigee decile 2.3e-4, near switches 2.8e-4, elsewhere 1.7e-4; worst intervals
late in the arc, none at a switch), i.e. accumulation, not a bad B_k. The LS
certifier remains PARTIAL/REVIEW on every row, fixed or free; the honest
independent check is still Tier 5 (an indirect multiple-shooting re-solve),
...
Status of the free rows (updated later on 2026-09-07): **gated, 17/17 PASS**
on the 9-state manifest `tulip_free` (`gate_free_tauf`, §6 row) and shipped as
the results set `results/minfuel_freetauf/`, which `aggregate_front` reads.
...
| 2026-09-07 (cScale port into the tulip engine, validated) | `casadi_minfuel_sundman` gains `opts.freeTauf` (constant slack state, dt/dτ = cScale·κ, t_f pinned; off = byte-identical). Four checks on the flagship: (A) fixed mode vs the pre-port engine at maxIter 5: X/U/lamAll identical; (B) free mode warm at the probe's free solution: sits still, m_f 0.8492722279 (Δ 5.7e-13 vs `casadi_energy_freetf`), cScale 1.004841 identical; (C) free mode from the FIXED seed: same m_f to 5.7e-13, 23 sw, 83 s — the port reproduces the free-time solver independently; (D) fixed engine at the EFFECTIVE length cScale·τ_f0 warm at the free solution: sits still to 4.9e-13 | GTO→tulip 1.150× flagship | all four as stated; contract test `tests/test_freetauf_port.m` (8-row X/lamDef kept, X9/lamDef9/cScale exposed, 9·N defect rows, manifest `tulip_free`) | a free-τ_f solution with slack c IS a fixed-τ_f KKT point at c·τ_f0 (rows 1–8 of the defects and the objective coincide) — so free rows are stored in the 8-row layout with the effective length as `tauf0`, and either engine mode re-solves them in place; `minfuel_at_tf` free by default, writes to `results/minfuel_freetauf/`; the τ_f0 fossil cannot recur (the stored length is always the solution's own) |
| 2026-09-07 (E2 signed minimum condition — statistic corrected, convention adjudicated) | `foc_check` signed test β = −q/‖q‖ (q = Lagrangian β-gradient sans the unit-norm dual) run on the fixed AND the free flagship with the raw IPOPT duals, no sign flip anywhere | GTO→tulip 1.150× flagship, 2381 / 2429 burn nodes | q·β/‖q‖ = −0.999 mean, wrong sign on 0 / 2 nodes (burn-edge nodes, throttle just past 0.5, ‖q‖ ≈ 0); betaNorm dual > 0 on 100% / 99.9% of burn nodes; median signed error 0.000, MAX 2.000 on both rows; also dot(β, λ_v^IPOPT) = +1.000 on every burn node | (i) the MAX was the wrong gate statistic: it failed a row whose sign law is 100% and primer alignment 0.06° on one edge node — gate is now `dirSignedPct` ≥ 99% (max + median still reported, `foc_report` prints the line); (ii) the minimum condition HOLDS on the raw duals for fixed and free rows alike; (iii) the engines' `primer = −λ_v/‖λ_v‖` is the ANTI-primer in IPOPT's multiplier convention (ψ_PMP = −λ_IPOPT for defects written X_{k+1} − X_k − h f), which the `if mean(cang) < 0` flip has been silently absorbing — the E2 remark that "the engine picks the costate sign from the observed steering" is confirmed, and the signed check is the flip-free instrument |
| 2026-09-07 (free-τ_f front GATED + shipped as a results set) | `GTO_tulip/direct/certify/gate_free_tauf.m`: per row a sit-still re-solve in the ported engine (`freeTauf`, `returnModel`, `lamG0` = the probe's own multipliers), `certified_guard`, `foc_check` on manifest `tulip_free` (nx = 9, ε = 0) + `foc_ipopt_inertia`, `foc_report` sidecar, then a `minfuel_at_tf`-layout row in `results/minfuel_freetauf/` (8-row X/lamDef, `out.cScale`/`X9`/`lamDef9`, top-level `tauf0` = effective length) | GTO→tulip ΔV–t_f front, all 17 free-τ_f rows 1.120–1.850× | **17/17 PASS**: m_f identical to the probe to the printed digit on every row (Δ = 0.0, re-solves converge in 0–1 iterations once the duals are seeded), defects ≤ 2e-11, KKT stationarity ≤ 2.5e-8 (15 rows ≤ 4e-12), sign law 100% everywhere, signed minimum condition on 99.80–100% of burn nodes, Hager-mapped |λ_m(t_f)| 1.3e-9–5.9e-9, no singular arcs, min relative |Ṡ| 0.47–52, IPOPT native inertia LOCAL MIN on all 17 (δ_w = 0 over the final iterations — note those are 1–2 iterations for a seeded re-solve, so the inertia line is weaker evidence here than on a from-scratch solve); `results/minfuel_freetauf/gate_summary.txt` | the free rows carry the same first-order + inertia certificate as the published fixed rows and are now the campaign's results set (`aggregate_front` reads them). Two lessons: (a) the first pass without `lamG0` left 7/17 rows wandering off within 800 iterations (`warm_start_init_point='yes'` with ZERO multipliers is a dual restart — every earlier "sit-still" re-solve in this repo, `run_foc_tulip` included, has been one; they happened to recover), so the engine now takes `opts.lamG0` and `run_foc_tulip` passes stored duals; (b) the E2 signed check's max statistic would have failed every row on one burn-edge node (row above) |
