...
| **status (2026-09-22)** | 24 x 24 torus FULL and rebuilt by one script (`reproduce_library_70mN`, FINDINGS 83-85); **24 x 48 built 2026-09-21** (1,095/1,152, audit clean, 57 holes from a pool defect). **INTERPOLATION between grid points is the live question**: 87% usable along arrival at 1/96, 0-2% on the 24 x 24 and 24 x 48 -- the departure axis is the limit and is unmeasured. Tools: `phase_catalog_interp`, `interp_study.m`, `score_interpolator`. State page: `DRO_tulip/process/INTERPOLATION_STATUS.md`; FINDINGS 86-92 |
...
| DRO → tulip, 3 cells × 13 γ values 1.1→2.0 (sparse) | **min-fuel** (ε ladder / Huber / huberc at fixed δ — per-entry family, schema v3.1) | **MIN-FUEL CATALOG v3.1 2026-09-07 (FINDINGS §29)**: `costate_catalog_dro_tulip_minfuel.mat`, **18 entries** (7 grid + 11 high-γ from the γ-walk + three-arm race, FINDINGS §28; 8 huber / 2 huberc / 8 ε), γ up to **2.0** on (2,5), all 26 verdicts PASS on the corrected fixed-t_f instrument, reflight ≤ 0.22 km; first catalog 2026-09-02 had 7 entries (2026-09-05: full-state block, initial-coast skip, t_f sampled; the earlier refutation/rescue was a structural-zero artifact — the rescue stays on mass), fuel gain +0.13…+0.95% of m₀ over min-energy, coast junction-fraction to 58%. **Propulsion is Isp 900 s / 0.07 N / 150 kg** (the 09-02 build mislabelled 1710 s; ΔV 0.46–0.92 km/s). Race: ε ships; the "PLQ-Huber refuted" verdict was retracted — it was our missing saltation matrix (FINDINGS §18–22) |
