...
\textbf{Known gap.} \file{foc\_check.m} does not currently receive
$\varepsilon$, so on an $\varepsilon>0$ leg it still folds \code{signPct},
the singular-arc count, and \code{sdotMinRel} into the advisory verdict. The
ELFO energy-seed run prints a caveat, but the verdict itself is not
$\varepsilon$-aware. Recorded as pre-promotion work: the manifest should carry
a throttle-cost kind (affine / quadratic) and skip these three checks
programmatically when it is not affine.
...
\end{equation}
with the symmetric formula at $\sigma_1$. The more principled alternative is to
form the exact discrete endpoint covector
$p_{N+1}=(I-\tfrac{h_N}{2}D_{N+1})^{\!\top}\Lambda_N$ plus terminal terms,
whose mass component is zero by terminal-node stationarity when the final mass
is genuinely free --- reported as a discrete identity rather than an estimate.
Until one is implemented, treat marginal misses on this line as a flag to
investigate, not a verdict: the two candidate explanations remain confounded.
...
behavior the tulip campaign already suspected. \textbf{But both external
reviewers independently judged those two numbers unable to diagnose grazing},
because of the recorded discretization confound (register finding I1): $S^d_k$
carries the local trapezoid weight $w_k$ and the difference is not divided by
the step, so on refined or PSR-split meshes (which densify \emph{exactly at
switches}) the statistic shrinks with $h$ even for a perfectly regular
crossing.

The agreed replacement --- deweight, then take a true divided difference, then
non-dimensionalize:
\begin{equation}
...
invariant to local refinement for a smooth transversal crossing. One reviewer
adds --- correctly --- that no threshold on a \emph{single} mesh is defensible;
regularity should be asserted only if $R_i$ stays above the floor on two
meshes. The same deweighting applies to the singular-arc test of
\S\ref{sec:singular}, which otherwise reports false positives in densified
regions. Until this lands, read a FAIL here as ``inspect this switch on the
mesh,'' not ``the switch is degenerate.''
