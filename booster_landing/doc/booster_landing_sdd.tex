...
\texttt{sqrt(sum(.\^{}2) + 1e-12)}. No \texttt{norm}, \texttt{abs} or
...
\item \textbf{Division guards (2026-08-09).} The $+10^{-12}$ under both
square roots is a guard, added after an external review, and it is what
makes the previous bullet's ``no \texttt{max()}'' constraint compatible
with being defined at the singular points. The raw forms divide by
$\lVert v\rVert$ (drag Jacobian block $vv^\top/\lVert v\rVert$) and by
$\lVert T\rVert$ (mass row of $B$), both $0/0$ at zero. Neither is
hypothetical: \texttt{sim\_closed\_loop}'s arrest mode passes through
$v_z=0$ by construction, and \texttt{tvlqr\_design} asks this function for
$A$ and $B$ along the whole trajectory. The smoothed forms have the correct
...
to machine precision, and the same $10^{-12}$ has always been in the
CasADi twin --- so the two implementations now smooth identically.
...
$L_\infty$ trajectory difference $< 5.0$\,m --- all
$\times$\,\texttt{tolScale}, all scored &
...
resolution in vacuum --- $\approx1.7^\circ$ at $N=20$, $1.19^\circ$ at
$N=30$, $0.92^\circ$ at $N=40$, $0.57^\circ$ at $N=60$ against a $1^\circ$
gate --- so $N=40$ was chosen as the smallest grid on which all gates passed
...
one interior switch. The no-switch fallback ($t_s = -10\,t_{Blend}$) is
conservative --- as of 2026-08-09; at its previous value of $0$ it was not,
see \S\ref{sec:mod:tvlqr}. This is a live TODO in
\file{lib/tvlqr\_design.m}.
...
is recorded here rather than silently resolved. None changed a certified
number.

Eight of the nine were \textbf{corrected in code on 2026-08-09}, in the same
pass that applied an external code review (\file{process/external-review-\allowbreak
fixes-report.md}); each is marked \textsc{fixed} below and its entry now
describes what was wrong and what it says instead. They are kept rather than
deleted because the list is also a record of how this document was used.

\begin{enumerate}

\item \textsc{fixed.} \textbf{\file{run\_booster\_landing.m} header listed
\texttt{.mc} unconditionally.} The \texttt{OUTPUTS} line read
``\texttt{R - everything: .P .solC .solV .rep .ctrl .out0 .mc .when}'',
while the code gates \texttt{R.mc} behind \texttt{cfg.doMC}. The README was
corrected on this point; the function header was not. The header now states
the \texttt{cfg.doMC} condition, matching how it already documented
\texttt{.mcD}.

\item \textsc{fixed.} \textbf{\file{certify\_pdg.m} said it ``never
throws''.} The header stated ``This function is report-only and never
throws'' while the file contained two time-base consistency assertions. The
external review made the same finding independently and pushed further:
throwing on a malformed \texttt{solC} destroys the report for \emph{all}
gates in exactly the case a report is most wanted. The assertions are now
gate G0 (\S\ref{sec:certify:g0}), with their messages preserved in
\texttt{rep.G0\_msg}, and the contract is true as written.

\item \textsc{fixed.} \textbf{\file{plot\_pdg\_solution.m} called panel (b)
``the max--min--max money plot''.} The certified structure for these
boundary conditions is \emph{min--max} --- a single $\Tmin\!\to\!\Tmax$
switch --- a finding significant enough that the theory note devotes a
section to it and G5's structure check was rewritten around it. The header
comment predated that finding and now reads min--max, with a note recording
the correction.

\item \textsc{fixed.} \textbf{\file{movie\_landing.m}'s \texttt{sol}
argument is unused.} The header documented it as ``\texttt{.tf} used for the
throttle panel's x-axis extent only''; the body sets that axis from
\texttt{out.t(end)} --- the closed-loop duration, which is the right choice
for a movie of the closed-loop run --- and never dereferences \texttt{sol}.
The argument is third of five and positional, so dropping it would silently
re-bind \texttt{outfile}/\texttt{opts} in every caller. The header now says
it is unused and says why it is kept.

\item \textsc{fixed.} \textbf{\file{solve\_pdg\_colloc.m}'s \texttt{INPUTS}
line said \texttt{.tol [def 1e-9]}.} The actual default is $10^{-11}$ when
\texttt{P.drag.on}. The same line cross-referenced the adaptation note that
explains this, so it was a stale summary rather than a missing explanation;
it now states both defaults.
...
\item \textsc{fixed.} \textbf{README described the test bootstrap as
...
which the README now says explicitly, alongside the real form in its folder
map.

\item \textsc{fixed.} \textbf{\file{test\_monte\_carlo\_small.m} pointed at
a probe that does not exist in code.} Its header said ``see
\texttt{run\_monte\_carlo.m}'s own probe at \texttt{N=P.N} for that (task-8
report, 50-run production-grid result)''. The 50-run production-grid probe
is a one-off run recorded in the task-8 report; \file{run\_monte\_carlo.m}
contains no such probe. The header now points at the report and the
README's flagship table.

\item \textsc{fixed.} \textbf{README's folder-map row for \file{results/}
listed \texttt{phase2\_footprint.png} as a product of ``a \texttt{cfg.phase2}
...
both, and the folder map now says so (as it now does for
\texttt{footprint.png}, which is under the same gate).
