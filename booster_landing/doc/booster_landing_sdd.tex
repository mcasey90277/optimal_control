...
against five gates. Only after certification does the guidance solution feed
...
\texttt{solV} and G3/G4 report \texttt{'skipped'}; G5's primer sub-check is
loosened (not removed) from $1^\circ$ to $10^\circ$. See
...
Seventeen \texttt{.m} files ship: 1 front door, 1 path bootstrap, 8
library, 2 certification, 4 visualization, plus the 11 test scripts.
...
threshold over the convex route's Taylor mass-bound model error
($0.70$--$0.94$\,kg, non-shrinking under refinement). See
...
\texttt{sqrt(sum(.\^{}2))}. No \texttt{norm}, \texttt{abs} or \texttt{max}
appears anywhere in this file, because \file{tests/test\_dynamics\_jac.m}
perturbs it with $h = 10^{-30}i$ and compares $\mathrm{Im}(f)/h$ against
...
\item The drag branch is not differentiable at $v=0$; the test evaluates at
a nonzero-velocity state deliberately, and no part of the campaign
linearizes at rest.
...
interior switch. The no-switch fallback is \texttt{ts = 0}, i.e.\ the
...
\texttt{.stop} is \texttt{'touchdown'} if event 1 fired, \texttt{'arrest'}
if event 2 or 3 fired, and \texttt{'horizon'} if none fired before the
$1.5\,t_f$ integration horizon. \texttt{.landed} is defined as
...
\texttt{.n\_landed}, \texttt{.n\_arrest}, \texttt{.n\_horizon} & scalars &
  counts derived from \texttt{mc.stop}; sum to $N_r$ \\
...
\item \textbf{Single source of truth for counts.} All three counts derive
...
\item \texttt{mc.ok} requires \texttt{landed}. An arrest or horizon run is a
failure however good its \texttt{miss}/\texttt{vtd} look.
\end{itemize}
...
\texttt{rep.G5\_primer\_mode} is \texttt{'skipped-cone'}, as a scored row at
10\,deg when \texttt{rep.drag\_on}, and at 1\,deg otherwise. \textbf{This
...
annotation box with success rate, \texttt{landed/arrest/horizon} breakdown
...
G3 & $|\Delta m_f| < 1.0\,$kg$\,\times$\,\texttt{tolScale},
$|\Delta t_f| < 0.2$\,s$\,\times$\,\texttt{tolScale};
$L_\infty$ trajectory difference is info-only &
...
the last sample; primer angle $<1^\circ$ / $<10^\circ$ / unscored &
...
sits strictly inside the annulus). The primer check, by contrast, runs at
\emph{nodes only} --- \texttt{lam\_defect} has no midpoint dual.

\paragraph{G5's three primer modes.} This is the one gate whose pass
criterion depends on the problem configuration:

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
Condition (checked in this order) & \texttt{G5\_primer\_mode} &
\texttt{G5\_pass} requires \\
\midrule
\texttt{isfinite(P.theta\_max\_deg)} & \texttt{'skipped-cone'} &
  \texttt{G5\_structOk} alone \\
\texttt{P.drag.on} (cone infinite) & \texttt{'scored'} &
  \texttt{G5\_structOk \&\& primer\_deg < 10} \\
otherwise (vacuum, no cone) & \texttt{'scored'} &
  \texttt{G5\_structOk \&\& primer\_deg < 1} \\
\bottomrule
\end{tabular}
\end{center}

\texttt{G5\_primer\_deg} is computed and reported in all three cases; only
its role changes. The ordering matters: a cone run can never also be scored
against the drag-loosened bound.

\emph{Cone}: with a finite pointing cone the primer condition is simply not
a necessary condition of the constrained problem. The optimal thrust rides
the cone boundary (the KKT projection of the unconstrained direction), so
$T$ is not parallel to $\lambda_v$ by design. Measured at
$\theta_{\max}=15^\circ$: both solvers independently bind the cone at
$\approx15.000^\circ$ (agreeing to $\sim4\times10^{-3}$ deg --- itself a
strong structural cross-check), while the naive primer angle reads
$\approx6$--$7^\circ$ on that perfectly converged optimum. Skipping is the
guard against that false fail.

\emph{Drag}: the threshold is \emph{loosened, not removed}. An earlier fix
set \texttt{G5\_pass = G5\_structOk} under drag, which would let a future
sign-flip bug pass silently --- exactly the class of bug the primer sign
convention exists to catch. Analytically drag should not change the
direction condition at all (drag enters $A$, never
$B = \partial\dot x/\partial T$), yet the measured angle is $5.11^\circ$ at
$N=30$ and still $2.61^\circ$ at $N=60$, i.e.\ mesh refinement does not cure
it the way it cures the vacuum discretization artifact. The $10^\circ$
bound is a margin choice ($>3\times$ the measured production value) that
still catches a gross misalignment or sign flip, which would land in the
$90$--$180^\circ$ range. Why the discrete duals approximate the continuous
costate less well under drag is an open question, flagged in the file rather
than force-closed.
...
\texttt{tolScale} scales \textbf{only} G3's $|\Delta m_f|$ and
$|\Delta t_f|$ tolerances. It does not touch G1, G2, G2ff, G4 or G5, and
this is deliberate:
...
\item \textbf{G5's primer has never been in its reach.} The coarse-grid
primer effect is real physics of the discretization, handled by choosing
$N\ge40$ in tests (\S\ref{sec:tests}), not by scaling.
...
\subsection{Internal assertions}

\texttt{certify\_pdg} is report-only in intent --- it returns FAIL flags
rather than throwing, and callers decide what a failure means --- with one
exception: two \emph{time-base consistency} assertions
(\texttt{certify\_pdg:timebase}) that G1's $h = t_f/N$ grid matches the
\texttt{solC.t} that G2 and G5 are built from, checking both
$t(\text{end}) = t_f$ and uniform spacing. If a future solver returned a
non-uniform \texttt{solC.t}, G1 and G2/G5 would silently disagree by an
$h$-independent offset invisible to G1, which never reads \texttt{solC.t}.
That failure mode is worth a hard stop.
...
\textbf{$N=40$ (the certification floor).} G5's primer-alignment angle is a
real function of mesh resolution in vacuum: $\approx1.7^\circ$ at $N=20$,
$1.19^\circ$ at $N=30$, $0.92^\circ$ at $N=40$, $0.57^\circ$ at $N=60$. The
gate is $1^\circ$. So $N=40$ is the smallest grid on which all five gates
pass \emph{honestly} rather than by a loosened threshold, and it is the grid
both \file{test\_certify\_nominal.m}'s first block and
\file{test\_run\_front\_door.m} use. This is not a \texttt{tolScale}
question --- the primer check was never in \texttt{tolScale}'s reach.
...
sharpest methodological lesson. At $N=30$, the pre-fix
\texttt{hs\_quad\_ctrl} reconstruction dipped up to 18\% below $\Tmin$ over
11.7\% of the flight \emph{between} nodes and midpoints --- invisible to
every gate that existed at the time. The simulator's saturating clamp turned
that into a real, grid-resolution-dependent open-loop altitude bias of
$+0.6$ to $1$\,m, and \emph{two full rounds of TVLQR weight tuning were
spent fighting a feedforward defect that was an artifact of the test grid}.
The conclusion recorded in the ledger was that the ``structural tension''
those rounds diagnosed did not exist. Two changes followed: the
reconstruction is now annulus-feasible by construction at any grid, gate
G2ff was added so the same class of bug cannot hide again, and closed-loop
testing uses the grid the shipped defaults are actually tuned against.
...
\texttt{sqrt(sum(.\^{}2))} for magnitudes --- no \texttt{norm}, no
\texttt{abs}, no \texttt{max} --- so that complex-step differentiation
validates its analytic Jacobians (\S\ref{sec:mod:dyn}). The CasADi RHS in
\file{solve\_pdg\_colloc.m} instead adds a $10^{-12}$ regularizer under the
square roots for symbolic smoothness at zero.
...
The phase-schedule switch detector finds the \emph{first upward} mid-annulus
crossing. On a max--min--max profile that is the second switch, and phase A
would wrongly cover the leading $\Tmax$ arc. Drag could produce such a
profile. The fix is to detect the \emph{last} upward crossing, or to build a
per-arc schedule, and it becomes mandatory the moment G5 certifies more than
one interior switch. The no-switch fallback (\texttt{ts=0}) is already
conservative. This is a live TODO in \file{lib/tvlqr\_design.m}.
...
cone programs solved with a general nonlinear interior-point method. A
dedicated conic solver is a documented drop-in and was deliberately not made
a dependency.

\item \textbf{The drag primer question} (\S\ref{sec:certify}) is open: why
...
%==============================================================
\section{Documented code-versus-documentation discrepancies}
\label{sec:discrepancies}

These were found while transcribing interfaces for this document. In each
case the \emph{code} is what \S\ref{sec:modules} documents; the discrepancy
is recorded here rather than silently resolved. None changes a certified
number.

\begin{enumerate}

\item \textbf{\file{run\_booster\_landing.m} header lists \texttt{.mc}
unconditionally.} The \texttt{OUTPUTS} line reads
``\texttt{R - everything: .P .solC .solV .rep .ctrl .out0 .mc .when}'',
while the code gates \texttt{R.mc} behind \texttt{cfg.doMC}. The README was
corrected on this point; the function header was not. The same header
correctly documents \texttt{.mcD} as conditional.

\item \textbf{\file{certify\_pdg.m} says it ``never throws''.} The header
states ``This function is report-only and never throws''; the file contains
three \texttt{assert} calls --- two time-base consistency assertions and,
transitively, none in the gate logic itself. The intent (gates report, they
do not throw) is correct and the assertions guard a genuinely
unrecoverable inconsistency, but the blanket claim is false as written.

\item \textbf{\file{plot\_pdg\_solution.m} calls panel (b) ``the
max--min--max money plot''.} The certified structure for these boundary
conditions is \emph{min--max} --- a single $\Tmin\!\to\!\Tmax$ switch --- a
finding significant enough that the theory note devotes a section to it and
G5's structure check was rewritten around it. The header comment predates
that finding.

\item \textbf{\file{movie\_landing.m}'s \texttt{sol} argument is unused.}
The header documents it as ``\texttt{.tf} used for the throttle panel's
x-axis extent only''; the body sets that axis from \texttt{out.t(end)} and
never dereferences \texttt{sol}. The argument is positional, so it cannot
simply be dropped without touching all callers.

\item \textbf{\file{solve\_pdg\_colloc.m}'s \texttt{INPUTS} line says
\texttt{.tol [def 1e-9]}.} The actual default is $10^{-11}$ when
\texttt{P.drag.on}. The same line cross-references the adaptation note that
explains this, so it is a stale summary rather than a missing explanation.

\item \textbf{The shipped \file{results/booster\_run.mat} predates
\texttt{.ctrlD}/\texttt{.Pd}.} Its top-level variables are \texttt{P},
\texttt{ctrl}, \texttt{mc}, \texttt{mcD}, \texttt{out0}, \texttt{rep},
\texttt{repD}, \texttt{solC}, \texttt{solD}, \texttt{solV}, \texttt{when}
--- no \texttt{ctrlD}, no \texttt{Pd}. The front door's own comment
acknowledges this (``populates on the next full run''). The schema in
\S\ref{sec:data} documents the code, which is what a fresh run will produce.

\item \textbf{README describes the test bootstrap as
\texttt{addpath('..'); setup\_paths;}.} The tests actually use
\texttt{addpath(fullfile(fileparts(mfilename('fullpath')), '..'))}, which is
cwd-independent; the literal README form is not. A simplification in prose,
not a defect, but the literal form should not be copied into a new test.

\item \textbf{\file{test\_monte\_carlo\_small.m} points at a probe that does
not exist in code.} Its header says ``see \texttt{run\_monte\_carlo.m}'s own
probe at \texttt{N=P.N} for that (task-8 report, 50-run production-grid
result)''. The 50-run production-grid probe is in the task-8 report;
\file{run\_monte\_carlo.m} contains no such probe.

\item \textbf{README's folder-map row for \file{results/} lists
\texttt{phase2\_footprint.png} as a product of ``a \texttt{cfg.phase2}
run''} without repeating the \texttt{cfg.doMC} condition, which the same
README states correctly in its \texttt{cfg} listing above. The code requires
both.
