function animate_minfuel_control(solFile, outStem, titleStr, mode)
% ANIMATE_MINFUEL_CONTROL  Movie of a saved full-spiral min-fuel control.
%
% Rotating CR3BP frame: trajectory colored red=burn / blue=coast, primer
% thrust arrows on the burn, over a synced bang-bang throttle subplot.
% Generalized from ANIMATE_COARSE_MINFUEL to take any saved solution file.
%
% INPUTS:
%   solFile  - .mat with fields r[3xN], s[1xN], U[4xN], tMesh, tf, switches,
%              maxDefect, tStar  (as saved by the tf-continuation extractor)
%   outStem  - output basename (writes <outStem>.mp4 and .gif)
%   titleStr - trajectory-panel title
%   mode     - 'preview' (3 stills) or 'movie' (MP4+GIF) [default movie]

if nargin < 4 || isempty(mode), mode = 'movie'; end
sp = [fileparts(mfilename('fullpath')) filesep];
S  = load([sp solFile]);
muStar = 0.012150585609624;  tStar = S.tStar;

r = S.r;  s = S.s;  w = S.U(1:3,:);
tDays = S.tMesh(:).'*tStar/86400;
nNode = numel(s);  burn = s > 0.5;
earth = [-muStar,0,0];  moon = [1-muStar,0,0];
yTul = [];
try, mf = load([sp 'minfuel_movie_data.mat']); yTul = mf.yTul; catch, end

allP = r.';  pad = 0.05*(max(allP)-min(allP)+eps);
...
else, frames=unique(round(linspace(2,nNode,300))); end
...
xlabel(axT,'x (rot, ND)'); ylabel(axT,'y'); zlabel(axT,'z'); title(axT,titleStr);
hBurn =plot3(axT,nan,nan,nan,'-','Color',[0.85 0.15 0.15],'LineWidth',1.8);
hCoast=plot3(axT,nan,nan,nan,'-','Color',[0.15 0.35 0.85],'LineWidth',2.4);
...
stairs(axS,tDays,s,'-','Color',[0.4 0.4 0.4],'LineWidth',0.9);
ylim(axS,[-0.05 1.08]); xlim(axS,[0 tDays(end)]);
xlabel(axS,'time (days)'); ylabel(axS,'throttle s');
title(axS,sprintf('min-fuel throttle: %d switches, %.0f%%/%.0f%% burn/coast (coherent, defect %.1g -- not yet sharp bang-bang)', ...
    S.switches, 100*mean(burn), 100*mean(~burn), S.maxDefect));
...
if ~strcmp(mode,'preview')
    vw=VideoWriter([sp outStem],'MPEG-4'); vw.FrameRate=24; vw.Quality=95; open(vw);
    gifFile=[sp outStem '.gif']; gifStride=2; gifW=640; gifDelay=1/12; gifMap=[];
end

fc=0; vidHW=[];
for f=frames
    fc=fc+1;
    bM=burn(1:f); cM=~bM; xb=r(1,1:f); yb=r(2,1:f); zb=r(3,1:f);
    ...
    if strcmp(mode,'preview')
        exportgraphics(fig,sprintf('%spreview_ctrl_%s.png',sp,tags{fc}),'Resolution',130);
    else
...
if ~strcmp(mode,'preview'), close(vw); fprintf('WROTE %s.mp4 + .gif (%d frames)\n',outStem,numel(frames));
else fprintf('WROTE ctrl preview PNGs\n'); end
close(fig);
end
