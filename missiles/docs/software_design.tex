...
\code{globeMovie} is 812 lines, most of it header, and every visual feature in
...
hold the apparent apogee at or under $0.3\,r_E$, floor at $2\times$, and cap at
the shipped $30\times$ so the short-range case is unchanged. It remains
overridable.
...
along-track, with a degenerate fallback to the polar axis when the endpoints
are coincident or antipodal. The camera direction is then
...
\code{tests/test\_viz.m} is 1569 lines and is a serious test: it reads the
