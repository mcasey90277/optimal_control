...
\midrule
H1 normal extremal & a lift with $\lambda_0=1$ exists & \textbf{have} (the BVP is solved with $\lambda_0=1$ to $10^{-10}$; pumpkyn agrees to $|\Delta z|\sim10^{-9}$). \gap{Does not exclude an abnormal lift of the same trajectory; see \S\ref{sec:abnormal}} \\
H2 strong Legendre & $H_{\alpha\alpha}|_{TS^2}=\tfrac{T}{m}|\lv|\,I\succ0$, i.e.\ $|\lv(t)|>0$ on $[0,\tf]$ & \textbf{gate to add}: $\min_t|\lv|$ from the flown arc (junction states give a lower bound only at $K$ points) \\
H3 all-burn is the PMP control & $Q_{mt}(t)>0$ on $[0,\tf]$, eq.~\eqref{eq:Qmt} & \textbf{gate to add}; without it the catalog's problem is the all-burn problem, not the throttle-bounded one \\
...
all-burn catalog entry. This is an argument, not a computation; the computation
would be: form the abnormal shooting function (drop the constant, keep
$H\equiv0$ and the endpoint conditions) and check that the accepted
$(\lr,\lv)$ direction is not a root of it. Cheap; not done. \gap{open}
...
having been skipped).}
\end{quote}
It is \emph{not} yet ``a strict strong local minimizer'', because H2 and H3 are
not gated, the abnormal-lift computation of \S\ref{sec:abnormal} has not been
done, and H5 is sampled. Adding the two gates and dense sampling upgrades this
to ``satisfies the hypotheses of the BCT sufficiency theorem to the extent they
are numerically checkable''; that is the sentence a methods paper can carry,
with the sampling caveat stated.
...
\begin{enumerate}
\item Gate $\min_{t}|\lv(t)|$ and $\min_t Q_{mt}(t)$ along the flown arc
(\texttt{seed\_from\_z8} already flies it); store both per entry. Cost: one
propagation per entry, already performed by \texttt{conj\_catalog\_pass}.
...
\item The abnormal-lift probe of \S\ref{sec:abnormal} on the golden cells first,
then at catalog scale if cheap.
