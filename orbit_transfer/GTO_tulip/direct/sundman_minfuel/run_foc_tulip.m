...
if nargin < 1 || isempty(matPath)
    matPath = fullfile(here, 'sundman_minfuel_certified.mat');
end
addpath(here);
...
% --- certified-quantity guard (NOT node-wise drift -- see refresh_duals_mee.m
% header, ref [4]): success-class status + machine-tight defect + a ONE-SIDED
% final-mass check (a warm re-solve is never penalized for finding a nearby
% point with EQUAL or HIGHER final mass; it is only refused if mass dropped).
feasTol = 1e-8;
massTol = 1e-6;
successClass = out.success && any(strcmp(out.ipoptStatus, ...
    {'Solve_Succeeded', 'Solved_To_Acceptable_Level'}));
if ~successClass
    error('run_foc_tulip:resolveFailed', ...
        ['%s: warm re-solve did not reach a success-class IPOPT status (%s) -- ' ...
         'cannot gate the FOC/KKT check on an unconverged point.'], matPath, out.ipoptStatus);
end
if out.maxDefect > feasTol
    error('run_foc_tulip:notFeasible', ...
        ...
        matPath, out.maxDefect, feasTol);
end
mfSaved = S.out.mf;
info = struct('drift', max(abs(out.X(:) - S.out.X(:))), 'ipoptStatus', out.ipoptStatus, ...
              'maxDefect', out.maxDefect, 'mfSaved', mfSaved, 'mfResolved', out.mf, ...
              'improved', false);
if out.mf < mfSaved - massTol*max(abs(mfSaved), 1)
    error('run_foc_tulip:massDegraded', ...
        ...
        matPath, mfSaved, out.mf);
elseif out.mf - mfSaved > massTol*max(abs(mfSaved), 1)
    info.improved = true;
    fprintf(['  [run_foc_tulip] NOTE %s: warm re-solve improved m_f (%.6f -> %.6f). ' ...
