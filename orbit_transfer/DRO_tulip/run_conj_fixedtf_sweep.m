...
for ks = 1:numel(sel)
    ...
    lg('energy (%d,%d)@%.2f: conv=%d normR=%.1e -> %s nCross=%d firstFullRank=%d atFinal=%d (%.0fs)', ...
       rec.iD, rec.iA, rec.gam, it.converged, it.normR, tern(it.conj.pass,'PASS','FAIL'), ...
...
lg('fuel %-6s (%d,%d)@%.2f p=%.4g: conv=%d normR=%.1e -> %s nCross=%d firstFullRank=%d atFinal=%d s1=%.2f (%.0fs)', ...
   src, cell, gam, p, it.converged, it.normR, tern(it.conj.pass,'PASS','FAIL'), ...
...
r = struct('kind', kind, 'source', src, 'cellIdx', cell, 'gamma', gam, 'p', p, ...
    ...
    'conjPass', c.pass, 'nCross', c.nCrossings, 'atFinal', c.atFinal, ...
    'firstFullRank', c.firstFullRank, 'dets', c.detScaled, 'sigRatio', c.sigRatio, ...
    'stateRows', c.stateRows, 'costateCols', c.costateCols, ...
    's1', it.s(1), 'mf', it.Y(7, end));
