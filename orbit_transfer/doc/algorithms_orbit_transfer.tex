...
never in between, and switch at exactly the right moments'' (the only exception is a singular stretch where the dial of \S\ref{sec:pmp} sits exactly on its threshold, which these transfers do not show). Finding \emph{when}
...
fraction: $m(0)=1$. Admissible arcs keep $m(t)>0$ and stay clear of both primaries; no dry-mass bound is imposed, so the free-mass condition $\lm(\tf)=0$ below is unconditional.
...
\subsection{Pontryagin's principle, once}\label{sec:pmp}
\begin{intu}
...
\end{intu}
\begin{rig}
With the normal multiplier $\lambda_0=1$ and, for min-time, the running-cost form $L\equiv1$, $\phi=0$ (used from here on: it gives $H(\tf)=0$ and $\lambda\cdot f=-1$; in the Mayer form the same conditions read $H(\tf)+1=0$),
...
\dot\lm = -\frac{sT}{m^2}\,|\lv|\qquad(\text{after }\alpha^*;\ \lm\text{ falls to its terminal zero, so }\lm\ge0).
\end{equation}
...
$\lambda_0$, $Y_j$ & $7\times1$, $14\times(K{+}1)$ & a \textbf{fixed-$\tf$} solution: the initial costate and the $K{+}1$ junction states of the multiple-shooting mesh, shipped in full: at many revolutions a $\lambda_0$-only reconstruction amplifies rounding into $\sim$100 km of ambiguity (measured on the 40-rev min-time flagship), so the junctions keep a numerically usable representation \\
...
\subsection{Conventions a reimplementer needs}\label{sec:conv}
\begin{itemize}\itemsep2pt
\item \textbf{Units.} $L^\star=389{,}703.26$ km, $T^\star=1/n$ (the Earth--Moon mean motion). Thrust and exhaust velocity enter as
$T_{nd}=\dfrac{T_{[N]}/m_0}{1000\,L^\star}\,{T^\star}^2$, $c_{nd}=\dfrac{I_{sp}\,g_0\,T^\star}{1000\,L^\star}$; the catalog validator recomputes $c_{nd}$ from the stored $I_{sp}$ and refuses a mismatch (that check is how the Isp mislabel of 2026-09-02 was caught).
\item \textbf{Gravity.} With $d=|r-(-\mu,0,0)|$, $\rho=|r-(1-\mu,0,0)|$:
$g(r)=\big(x-(1-\mu)\tfrac{x+\mu}{d^3}-\mu\tfrac{x-1+\mu}{\rho^3},\ y-(1-\mu)\tfrac{y}{d^3}-\mu\tfrac{y}{\rho^3},\ -(1-\mu)\tfrac{z}{d^3}-\mu\tfrac{z}{\rho^3}\big)$.
\item \textbf{Endpoints.} A cell is a pair of phase fractions $(s_D,s_A)\in[0,1)^2$; the endpoint is the periodic orbit's state at time $s\cdot\tau$ after the family getter's reference point (spline interpolation of the propagated orbit). Endpoints are \emph{fixed} rotating-frame states; the phasing torus is the $12\times12$ (or $6\times6$) grid of these pairs, and $\tf^{\min}$ is the accepted, \file{tfMin}-confirmed min-time value of the same cell --- a numerically resolved reference, not a proven minimum.
\item \textbf{Direct transcription} (\file{casadi\_mintime\_dro}, objective \texttt{'time'} or \texttt{'energy'}). Hermite--Simpson in separated form on $N$ uniform intervals of the normalized time $\sigma\in[0,1]$, $h=1/N$: node variables $X_k,U_k$ and midpoint variables $\bar X_k,\bar U_k$ with the interpolation equality $\bar X_k=\tfrac12(X_k+X_{k+1})+\tfrac{h\tf}{8}(f_k-f_{k+1})$ and the defect $X_{k+1}-X_k-\tfrac{h\tf}{6}(f_k+4\bar f_k+f_{k+1})=0$; $U=(\alpha,s)$ with $|\alpha|=1$ as an equality and $s\in[0,1]$ at nodes and midpoints; $\tf$ is carried as one lifted copy per interval with the continuity $T_{k+1}=T_k$ (this keeps the KKT matrix banded; a single scalar $\tf$ produced a dense column that crashed MUMPS) and pinned to $\gamma\tf^{\min}$ for the energy problem; the energy cost is Simpson-quadratured $\int s^2\,dt$; an optional minimum-altitude path constraint. $N$ of order $10^3$ (1600 for the certified flagship).
\item \textbf{Dual map} (\file{oc.duals\_to\_costates}). The multiplier $\mu_k$ of the defect on interval $k$ is a sample of $\lambda$ at that interval's \emph{midpoint} for Hermite--Simpson (at the node, step-weighted over the two adjacent intervals, for trapezoid); with unit-scaled defects $\lambda\approx\pm\mu$; the global sign is fixed by a majority vote that the implied primer $-\lv/|\lv|$ align with the solved thrust direction; the multiplier paired with the lifted $\tf$ must then read $+1$ (it is $-H$), the cheapest quality gate on a harvest. \file{harvest\_ms\_seed} interpolates these samples onto the $K{+}1$ breakpoints.
\item \textbf{Segments.} Uniform breakpoints. Fixed-$\tf$ line: the ladder $K\in\{12,24,48\}$ is escalated until multiple shooting converges (12 sufficed on every record). Min-time: $K=24$ in the catalog conjugate pass, 12--48 elsewhere.
\item \textbf{``ms residual''} is $\|R(p)\|_\infty$ of \eqref{eq:msres}, all rows, in ND units. \textbf{``Reflight''} is the propagation of the entry's own field from its first junction over the full $\tf$; the number quoted is $|r(\tf)-r_f|$ in km.
\end{itemize}
...
\draw[arr] (tfmin)--(acc);
\draw[arr,dashed] (z8.south) |- node[lbl,pos=0.75,above]{$z_8$, endpoints, $T$, $c$} (tfmin.west);
\draw[arr,dashed] (lad.south) -- ++(0,-3.1) -| ($(ms.west)+(-0.5,0)$) -- (ms.west);
\node[lbl] at (8.6,-8.1) {next rung: warm seed back into multiple shooting};
\end{tikzpicture}
\caption{The min-time pipeline. Blue = our algorithms, green = pumpkyn (Darin's astrodynamics toolbox, used as physics source and as the independent acceptance solver), yellow = data. Every catalog entry has passed the indirect solve and the foreign single-shooting re-solve; the direct stage seeds the first rung of each cell and later rungs are warm-started from the accepted neighbour.}
...
depends only on a \emph{short} propagation, the Jacobian is block-banded with the amplification confined to one short segment per block (the assembled Jacobian can still be ill-conditioned --- cond$(J)$ is reported for exactly that reason), and Newton converges from the collocation seed in 1--3 iterations. The price --- a bigger linear solve --- is trivial at this size.
\end{intu}
...
\end{equation}
where $\Phi^{(k)}$ propagates segment $k$ over the duration $\beta_k\tf$, $\beta_k=\tau_{k+1}-\tau_k$, with $\tau_k=t_k/\tf$ the fixed normalized breakpoints ($\tau_1=0$, $\tau_{K+1}=1$), and the $H$ row exists only when $\tf$ is free.
...
$\tf$, $\partial R_k/\partial\tf=\beta_k F(y_{k+1}^-)$ with $F$ the 14-state PMP field; the last block row is $Dg\,\Phi_K$ and, for free $\tf$, $Dg\,\beta_K F(y_{K+1}^-)$. \textbf{Solve.} \texttt{fsolve}
...
\State $p\gets[\,Y_1(\mathcal F);\,Y_{2:K};\,(\tf)\,]$; $\tau\gets\mathrm{tGrid}/\tf$, $\beta_k\gets\tau_{k+1}-\tau_k$
\Function{residual}{$p$}
  \If{$p$ non-finite or outside the guards} \Return $R\gets 10^8\mathbf 1$, $J\gets I$ \EndIf
  \For{$k=1..K$} $[y^-_{k+1},\Phi_k]\gets\mathrm{prop}(\beta_k\tf,\,Y_k)$ \Comment{one short propagation + STM}
...
is literally ``hand $z_8$ to \file{tfMin}; does it come back unchanged?'' --- PASS if \file{tfMin} reports convergence on its own terminal residual \emph{and} moves the eight numbers by less than $10^{-6}$ (norm of the ND $z_8$ difference; observed $\sim10^{-9}$). Both halves matter: a solver that did not move because it did not run would also return $\Delta z=0$. That is a
...
\end{tikzpicture}}
\caption{What a min-time entry has passed. Top row: level 1, the Pontryagin extremal --- every catalog entry. Bottom row: additional assessment with a stored verdict --- 61 entries are refuted and 50 unverified by the conjugate test, so a catalog entry is not automatically a level-2 pass; the hypotheses of the Bonnard--Caillau--Tr\'elat sufficiency theorem are checked numerically by \texttt{conj\_catalog\_pass} and \texttt{gates\_catalog\_pass} (the determinant is sampled at junctions, so this is an assessment, not a proof --- \S\ref{sec:proves}).}
\label{fig:mt-verify}
\end{figure}

\begin{eli}
Being a root of the equations only means the trajectory is a \emph{candidate} --- a hilltop and a valley bottom both have zero slope. To know it is a valley we look at the second derivative, which for trajectories means: can an infinitesimally nearby trajectory that starts at the same place and satisfies the same equations be bent back to touch this one before the end? If it can (a \emph{conjugate point}), the trajectory stops being shortest from that point on. Think of great circles leaving the north pole: every arc is shortest until it reaches the south pole, and the moment it continues past it (the first conjugate point) a shorter route exists.
\end{eli}
...
projects out $\lambda_{rv}(0)$ (columns $P$ spanning its complement) and appends
...
(\file{doc/mintime\_second\_order\_audit.tex}, image-space lemma), under the theorem's hypotheses --- normal extremal, all-burn control, $|\lv|>0$ so the feedback is smooth --- which is what the three gates below check.
\end{intu}
...
\[
D(t_{k+1})=\det M(t_{k+1}),\qquad M=\big[\,\Phi_{rv,\lambda_{rv}}(t_{k+1},0)\,P,\ \ f_{rv}(y(t_{k+1}))\,\big]\in\R^{6\times6},\qquad k=1..K,
\]
with $P\in\R^{6\times5}$ an orthonormal basis of $\lambda_{rv}(0)^\perp$ (the $\lm$ column of $\Phi$ is dropped: $\lm$ never enters the control on the all-burn branch), sampled through $\tf$ (the terminal state comes from the engine). The block is
...
before the last bracket; ENDPOINT if the only sign change lies in the last bracket $(t_K,\tf]$: the root may sit at $\tf$ or strictly before it and the instrument cannot tell at junction resolution, so the verdict is inconclusive (never a PASS; refine the bracket before classifying --- 2 of 18,360 catalog entries); UNDETERMINED
...
\file{tfMinEoM}'s own switching function); and the abnormal-lift probe: \emph{stationary} lifts of a fixed trajectory form a linear space $S=\{\lambda:\dot\lambda=-A(t)^{\!\top}\lambda,\ \lv\parallel\alpha(t),\ \lm(\tf)=0\}$ with $A$ the fixed-control Jacobian (only the stationarity part of the direction condition is imposed, not the inequalities); $\lambda\cdot f$ is a linear functional on $S$, our normal lift has $\lambda\cdot f=-1$, so its kernel has dimension $\dim S-1$; a nonzero kernel element is an abnormal stationary lift, and every PMP abnormal lift is one of those. Hence \emph{$\dim S=1$ excludes every abnormal lift} --- the gate is sufficient for exclusivity of the normal lift, and conservative in the other direction ($\dim S>1$ would flag a stationary abnormal lift that need not satisfy the minimum condition). $\dim S=7-\operatorname{rank}C$ with $C=[\,[\alpha_k]_\times\Psi_v(t_k),\ k=1..200;\ e_m^{\!\top}\Psi(\tf)\,]$ stacked from the fundamental matrix $\Psi$ of $\dot\Psi=-A^{\!\top}\Psi$ ($\Psi_v$ its $\lv$ rows), with the rank threshold tied to the
...
fixed $10^{-8}$ under-counted 512 entries as ``$\dim S=0$'' on the first pass). This is a residual-informed numerical-rank heuristic, not a bound: when the residual scale exceeds the cap the dimension is reported unresolved (flagged, never counted as one), and the gate stores the singular values and the threshold it used.
\textbf{Status.} Conjugate test (stored verdicts, produced by the pre-09-05 legacy instrument; the re-sweep on the corrected one is queued): 18,249 PASS / 61 refuted / 50 unverified, 2 of them ENDPOINT; the corrected instrument reproduces the 20 golden verdicts. Gates: 18,360/18,360 pass, written into the catalogs
...
\node[proc] (msE) at (8.6,0) {\textbf{ms\_minenergy}\\$p=1$, fixed $\tf$\\7 costates + $K{-}1$ junctions};
...
\node[data] (fuel) at (8.6,-2.6) {smoothed fuel candidate\\$\lambda_0$, $Y_j$, $m_f$\\family, $p_{\rm floor}$, $\delta$};
...
\draw[arr] (mt)--(dir); \draw[arr] (dir)--node[lbl,above]{duals $\to$ seed}(msE); \draw[arr] (msE)--(rec); \draw[arr] (rec)--(walk);
...
\end{axis}
\end{tikzpicture}
\caption{Throttle law $s^*=\arg\min_{[0,1]}(L(s)-sQ)$ for the three families at $p=0.3$ ($\delta=0.2$ for huberc). eps and huber tend to the bang-bang step as $p\to0$; huberc needs $\delta\to0$ too. They differ in \emph{how} the step is approached --- and that difference decides where Newton walks and where it walls.}
\label{fig:families}
\end{figure}
...
\text{huber (PLQ)} & L=\begin{cases}s^2/2p & s\le p\\ s-p/2 & s>p\end{cases} & s^*=\begin{cases}\max(pQ,0)&Q<1\\ [p,1]&Q=1\\ 1&Q>1\end{cases}\ \text{(jump; set-valued at }Q=1)\\[8pt]
\text{huberc (hybrid)} & L=\begin{cases}s^2/2p & s\le p\\ \tfrac p2+(s-p)+\tfrac{\delta(s-p)^2}{2(1-p)} & s>p\end{cases} & s^*=\begin{cases}\max(pQ,0)&Q<1\\ p+(1-p)\tfrac{Q-1}{\delta}&1\le Q<1+\delta\\ 1&Q\ge1+\delta\end{cases}
\end{array}
\]
...
min-energy but with costates halved (\S\ref{sec:seed}). As $p\to0$, eps and huber tend to the bang-bang law; huberc at fixed $\delta$ tends instead to $s^*=\mathrm{clip}((Q-1)/\delta,0,1)$ and needs $\delta\to0$ as well --- which is exactly why its schedule has a second stage (\S\ref{sec:seed}). huberc is $C^0$ in $Q$ with $L$ strictly convex
...
core and replaces the jump with a short ramp of width $\delta$: continuous in $Q$ (its derivative still has corners at the band edges) --- it passed the two Huber walls tested --- and, when
...
at the floor, and by then $m_f$ has stabilized to $10^{-6}$ across rungs (an observed stabilization, not an error bound against the exact bang-bang optimum).
\end{intu}
...
\Phi^+=\Big[I+\frac{(F^+-F^-)\,n^{\!\top}}{n^{\!\top}F^-}\Big]\Phi^-,\qquad n=\nabla_y Q,\qquad n^{\!\top}F^-\neq0\ (\text{transversal crossing}),
\end{equation}
...
the reflight (fly \file{cr3bp\_minfuel\_prop} from the first junction, i.e. from $\lambda_0$ alone, over the full $\tf$ in the entry's own family) checks that the deliverable's own recipe reproduces the arc --- worst 0.22 km over the 18 entries, all $\le30$ d and about three revolutions, which is why $\lambda_0$-only reconstruction still works here; the junctions are shipped for the multiple-shooting re-solve and for longer arcs. There is no foreign witness for min-fuel (pumpkyn has no twin;
...
true flow derivative provided every band-edge crossing ($Q=1\pm p$; $Q=1$ and $1+\delta$) is transversal ($\dot Q\neq0$ there; the feedback is Lipschitz with a piecewise-smooth derivative, so the flow is $C^1$), and then this is the Jacobi necessary condition; for huber entries the saltation STMs are used under the same transversality. Skipping the initial-coast samples is a diagnostic convention (the block is structurally rank-deficient there), not a theorem.
...
$\lambda_0$, $p$, family and rows it was computed on; \file{build\_minfuel\_catalog} looks the verdict up by source, cell and $\gamma$, then requires the re-solved initial costate to agree with the packaged one to $10^{-8}$ relative, rows $1{:}7$, and a converged re-solve --- the agreement test that ties the verdict to the shipped solution (the re-solve starts from the packaged junctions in the packaged family at its $p$ and $\delta$; the packaged junctions are kept, the re-solve is not substituted). Current: 26/26 PASS, all
...
bang-bang problem; $m_f$ has stabilized to $\sim10^{-6}$ across rungs by then, switch times
...
switching-time problem with fixed structure, which is the natural endgame and the base on which a strict certificate (reduced second variation plus switching regularity, Maurer--Osmolovskii) could then be tested.
\end{warn}
...
\end{tikzpicture}
\caption{Files of the fixed-$\tf$ line and what each carries. Everything downstream of a record is reproducible from the record's $\lambda_0$ and $K$ (or, for min-fuel, from $Y_j$: the junctions carry the numerically stable representation the re-solve needs; energy records get by with $\lambda_0$ and $K$ because the smooth $p=1$ flow is far better conditioned).}
\label{fig:files}
\end{figure}
...
what a PASS licenses & ``a numerically resolved normal PMP candidate, independently re-solved by a second code; the sampled Legendre and all-burn gates hold, the numerical lift space is one-dimensional, and the BCT determinant detector finds no conjugate-point evidence at the sampled times'' & ``a numerically resolved extremal candidate of the specified smoothed problem, reflying to $<0.25$ km, with no interior conjugate point detected by the sampled instrument'' \\
...
with fixed structure, which is also the base a strict second-order certificate would be tested on.
