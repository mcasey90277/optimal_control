...
RES="$HERE/results/minfuel";  LOGD="$HERE/results/front_logs";  mkdir -p "$LOGD" "$RES"
...
for F in "${FACTORS[@]}"; do
  ...
  TAG=$(printf 'minfuel_f%04d_en' "$MILLI")
  ...
  "$MATLAB" -batch "cd('$HERE'); TULIP_OVERRIDES=struct('factor',$F,'epsMin',$EPSMIN,'runName','$TAG','doPlot',false); run_gto_tulip" \
      > "$LOGD/$TAG.log" 2>&1
  RC=$?
