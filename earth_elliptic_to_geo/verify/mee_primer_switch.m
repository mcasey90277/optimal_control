...
%   info      - struct: .pel [3x(N+1)] (B(X)'*lam_el, unflipped), .Ldot0
%               [1x(N+1)], .Ldot [1x(N+1)] (actual, at the solver's U), .KL
%               [1x(N+1)], .G [1x(N+1)], .C1 [1x(N+1)], .KLoverLdot0
%               [1x(N+1)] (coupling-strength diagnostic, should be << 1 for
%               "low thrust" to justify treating Ldot's control-dependence as
%               a small correction; NOT assumed small anywhere in the code
%               above, reported for honesty only)
%
% REFERENCES:
...
for k = 1:Nn
    ...
    [~,       Ld0] = lt_mee_rhs(Xk, [e3; 0], parK);   % thr=0 probe -> Ldot0(X)
    ...
    Bcol1 = (dXdLe1(1:5) * Le1) * (mK / par.Tmax);
    Bcol2 = (dXdLe2(1:5) * Le2) * (mK / par.Tmax);
    Bcol3 = (dXdLe3(1:5) * Le3) * (mK / par.Tmax);
    lamEl = lam(1:5, k);
    ...
    S(k)  = C1(k) * Ld0 - lam(7, k) * KL(k) * Uk(3);
end

info = struct('pel', pel, 'Ldot0', Ldot0, 'Ldot', Ldot, 'KL', KL, 'G', G, ...
              'C1', C1, 'KLoverLdot0', KL ./ Ldot0);
end
