...
bang-bang switch structure. Converting that residual into kilograms, and then
verifying the conversion by predicting and making a mesh change, yields the
number the study opened by asking for: the $10$~N production mesh carries
$0.233$~kg of discretization error, $1.7\times10^{-4}$ of the final mass, which
is about one quarter of the $1.015$~kg available on the same mesh from a better
choice of local minimum.
\end{abstract}
...
\begin{itemize}
  ...
  (Section~\ref{sec:phase3} later re-establishes a dominance ratio of $4.4$ at
  $10$~N by direct single-branch measurement. The retraction stands: the
  original number was unsupported by the evidence then available.)
...
\subsection{The measured error bar}

Two solutions on the same branch, at the same node count, differing only in
node placement, with $\sum h^3$ of $2.668$ and $1.003$ and final masses
$0.1456$~kg apart. Taking $E = k\sum h^3$---an assumption supported by the
\emph{independent} measurement $R_x/h^3 = 0.171/0.171/0.163$ rather than fitted
here---gives $k = 0.0874$~kg per unit and

\begin{table}[h]
\centering\small
\begin{tabular}{lr}
\toprule
discretization error of the production mesh & $\mathbf{0.233}$~\textbf{kg} \quad($1.7\times10^{-4}$ relative) \\
discretization error after redistribution & $0.088$~kg \\
first-order estimate, for comparison & $1.237$~kg (overpredicts $5.3\times$) \\
basin gain on the same row & $1.015$~kg \quad---\quad $\mathbf{4.4\times}$ \textbf{the mesh error} \\
\bottomrule
\end{tabular}
\end{table}

This is the number the study set out to obtain. Unlike every earlier attempt it
rests on a same-branch comparison with the branch verified, not on an
extrapolation across a topology change. It also restores, in defensible form,
the claim withdrawn in Section~\ref{sec:corrections}: \textbf{basin selection
does dominate discretization error---by roughly a factor of four, not the
factor of one hundred that was retracted.}

\paragraph{Prediction 2's miss is itself a result.} The worst cell got eight
times \emph{worse} under redistribution. So the coefficient $C$ in
$E \approx C h^3$ is \emph{not} phase-independent: making $h$ uniform removes
the apogee penalty but creates a worse cell elsewhere, most plausibly at
perigee where a now-larger step meets rapidly varying dynamics. Uniform-in-time
is therefore not the optimal mesh; equidistributing $Ch^3$ would require $C$
measured per orbital phase. The mass improved regardless, because the total
fell even as the worst cell rose.

\section{Experiments still to run}
\label{sec:next}

\begin{enumerate}
  \item \textbf{Objective sensitivity, $\Delta J \approx \sum_k \lambda_k^{\!\top} R_k$.}
  The costate \emph{is} the sensitivity of the objective to a state
  perturbation, and both the costates and the residual map are already in hand.
  This converts a residual into kilograms and is the missing link to the
  question the study opened with.
  \item \textbf{Mesh redistribution.} Re-solve $10$~N at the \emph{same} node
  count with nodes redistributed to equidistribute $h^3$, and measure whether
  the residual falls and whether $m_f$ moves by the amount item 1 predicts.
  Items 1 and 2 together form a closed falsifiable loop: predict, then perturb,
  then check the prediction.
  \item \textbf{Cislunar adapters.} Both need their own: the tulip builds its
  dynamics inline with no reusable right-hand side and stores no duals, and
  ELFO uses a free-$t_f$ two-primary nine-state solver.
  \item \textbf{External review} of the Phase 3 conclusions, with the measured
  residuals and figures attached rather than intentions.
\end{enumerate}
