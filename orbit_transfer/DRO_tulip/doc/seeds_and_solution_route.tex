...
\begin{tabular}{@{}rrrrl@{}}
\toprule
$N$ & $\tf$ (ND) & vs.\ reference & periselene (km) & status \\
\midrule
\multicolumn{5}{@{}l@{}}{\emph{2026-08-03, unconstrained}}\\
400  & 4.3807 & $+9.1\%$  & 4819 & converged \\
800  & 4.4507 & $+10.8\%$ & $-343$ & converged, \textbf{inside the Moon} \\
1600 & 4.7833 & $+19.1\%$ & 4680 & converged, 7.8\,m worst-interval error \\
\midrule
\multicolumn{5}{@{}l@{}}{\emph{2026-09-15, reproduced by \file{root\_origins\_study}}}\\
400  & 4.6809 & $+16.6\%$ & 6563 & converged \\
800  & 4.9909 & $+24.3\%$ & 2243 & iteration limit \\
...
machinery works cold; basin selection does not.} Mesh density acts as a de
facto random seed. That is why seeding or continuation is required equipment
on a catalog pair rather than a convenience.
\end{rig}
...
converged rung kept: past a winding wall the correct next solution has a
different number of revolutions, and only a different exponent puts the guess
in its basin.

\textbf{Every rung must be screened against its own guess.} A rung that lands
far from where the scaling predicted has left the branch, and seeding the
next rung from it walks a different family down --- while looking perfect.
...
the mass burned, a different winding number. The shipped catalog steps by
...
falling 4.6\% at fixed thrust --- but it does not move this wall. Revolutions
climb $0.81 \to 1.31$ across the walk: it is a \emph{winding} wall, and the
anchor phase meets it near 0.11\,N while the sheet's fastest 0.5\,N cell
reaches 0.09\,N. Where you start the deep walk is a choice, not a detail.
\end{rig}
