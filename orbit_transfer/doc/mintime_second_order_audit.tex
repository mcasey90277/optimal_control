...
\]
whose two coefficients are H3 and H2: uniform lower bounds on them are the
quantitative strict-bang and Legendre statements.
...
$\dot\lambda_v=-\lr-(\partial_v f_v)^{\!\top}\lv$ with
$\|\partial_v f_v\|=2$ (Coriolis), so
$\big|\tfrac{d}{dt}|\lv|\big|\le|\lr|+2|\lv|$; and on an all-burn arc, with
$\dot m=-T/c$ and $\dot\lambda_m=-T|\lv|/m^{2}$, the two mass terms of
$\dot Q_{mt}$ cancel:
\[
 \dot Q_{mt}\;=\;\frac{1}{m}\,\frac{d|\lv|}{dt}\qquad\text{exactly.}
\]
With $L_k$ the larger of the slope bound at the two ends of a sample interval,
$\min_{[t_k,t_{k+1}]}v\ \ge\ \tfrac12(v_k+v_{k+1})-\tfrac12L_k\,\Delta t_k$
(\texttt{between\_sample\_bound}). The certifier and the catalog audit gate the
\emph{bound} above $10^{-5}$, ten times the measured $\lm$ uncertainty. On the
...
\textbf{Not checked.} It is checkable: the lift-space rank on prefixes
$[0,t]$ and on a few interior windows, by the same construction.
...
transfer at $s\pm2\times10^{-4}$ and difference the two flight times --- an
oracle that never reads a costate. On the $70$\,mN library, twelve sampled
derivatives agree with the formula to between $4\times10^{-8}$ and
$9\times10^{-4}$ relative. It is the first check of $\lambda(\tf)$ against
anything other than the solve that produced it.
\emph{Stationarity:} an entry is a first-order candidate for the
orbit-to-orbit minimum where both sensitivities vanish. None of the $576$ is:
the fastest, $16.226$\,d, has $(\partial_{s_D},\partial_{s_A})\tf=(-0.92,-2.05)$
d per unit phase, so the free-phase minimum lies between grid points. The
