...
In code, \file{cartpole\_common/cartpole\_field.m} is exactly this split:
...
\file{cartpole\_common/cartpole\_field.m} & $f$ and $g$, the one home for the physics\\
...
\begin{rig}
Here $H(u) = u^2 + \sigma u$ (dropping the parts independent of $u$), with
$\sigma = \lambda^\top g$ the switching function already named in \S1. \S4
already showed why this problem gives a formula: $H$ is \emph{strictly}
convex in $u$ ($H_{uu}=2>0$ everywhere) and $u$ ranges over all of $\R$, so
the stationary point $H_u=0$ is both necessary and sufficient for the
global minimizer --- one equation, one answer.

That argument leaned on two properties together: \emph{no bound on $u$},
and a cost \emph{strictly} convex in $u$. Both are what a different
objective can change. Bound the control, or replace $u^2$ by a cost that is
merely convex rather than strictly so (linear in $u$, or in $|u|$), and $H$
no longer has an interior minimum for calculus to find: the minimizer is
pushed onto the boundary of the allowed set (a wall) or onto the one point
where a piecewise-linear cost fails to be smooth (a corner) --- and which
wall or corner it sits at can change during the flight. That is a switch,
decided by a sign, not a derivative. Minimum time (control bounded, cost
$t_f$) and minimum fuel (control bounded, cost $\int|u|\,dt$) are exactly
the two objectives in this family that do this. Locating and tracking the
switch --- including the saltation matrices a jump in structure requires
--- is sub-projects B and C's content, and arrives with their code; their
