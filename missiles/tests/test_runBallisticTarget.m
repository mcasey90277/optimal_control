...
%  the SEEDED-AND-SOLVED launch azimuth, the CERTIFIED bracketing of the
%  max-range loft angle, the TWO full-burn branch solves on the certified
%  one-sided intervals either side of it, the MINIMUM-ENERGY CONSTRAINED
%  MINIMISATION on loft angle and cutoff fraction, the TWO-AXIS aim solve that
%  closes the cross-range on top of all three modes, the after-the-fact
%  measurement of which branch was actually flown, and the refusals at both ends
%  of the envelope, for an unreachable branch, for a minimum-energy request that
%  cannot be solved and for an aim solve that will not converge.
%
%  THE AZIMUTH IS A SOLVED QUANTITY AS OF 08/08/2026, and part 12 is where that
%  is graded. It used to be the closed-form great-circle bearing and a rotating
%  Earth was REFUSED because that bearing is not the answer on one; the bearing
%  is now the SEED of a damped Newton on the azimuth and the mode's own range
%  control, and the rotating case is flown. Part 12 pins the deflection it
%  removes AND its decomposition, because the decomposition is what distinguishes
%  the real mechanism -- the vehicle is deflected -- from the one this library
%  used to claim, that the target is carried east.
%
...
%  takes 64 to 67 of them, about 3 s; a 'minimum-energy' run takes 733, roughly
%  29 s, because the constrained minimisation is about 670 of those on its own
%  -- every outer evaluation is a whole inner feasibility solve. THE TWO-AXIS
%  SOLVE IS NOT WHAT MAKES THIS EXPENSIVE: it adds one propagation where the
%  seed is already inside tolerance and seven where it is not, against the 731
%  that stage 1 spends, so the shipped minimum-energy run went from 731 to 733.
%  The SHIPPED configuration is flown at its shipped settings throughout, which
%  is the stronger test: what is pinned is exactly what a user gets.
%
...
%  Michael Casey  Part 12 rewritten: a rotating Earth is FLOWN   08/08/2026
%                 rather than refused, and the deflection it
%                 costs is pinned by its decomposition
...
%% of this library actually acts on:
    ...
    assert(~isfield(inDef,'crossWarn'), ...
        ['info.crossWarn is gone and must stay gone. It flagged a cross-range ' ...
         'the solve could not control; the solve now DRIVES cross-range to ' ...
         'zero, so a converged run cannot trip it and a field that can only ' ...
         'ever read false is worse than no field at all']);
...
%% Cross-range is zero in this configuration, so the whole miss IS the down-range
%% component. Not a general guarantee: it holds because the bank angle is zero
%% AND the Earth is still, which is exactly what part 12 flies without:
    ...
    assertAbs(inDef.missM,hypot(inDef.downM,inDef.xTrackM),1e-6, ...
        ['the two miss components must compose to the directly measured miss ' ...
         '(m); they are the same vector in two bases and nothing else']);

%% THE SHIPPED RUN NEVER MOVES OFF ITS SEED, and that is asserted rather than
%% left to the numbers to imply. Its seed miss is 39.01 m against a 707.11 m
%% per-component tolerance, so coorbital.util.aimSolve tests convergence BEFORE
%% it does any work, converges at iteration 0, builds no Jacobian, takes no step
%% and hands x0 straight back. That is why every literal below this line is
%% bit-for-bit what it was before the azimuth became a solved quantity -- not
%% "to within a tolerance", unmoved -- and pinning the iteration count is what
%% keeps that claim honest if the seed ever drifts towards the tolerance:
    assert(inDef.aimIter == 0 && inDef.aimEval == 1, ...
        ['the shipped run''s seed is inside tolerance, so the two-axis solve ' ...
         'must converge at iteration 0 for the cost of the one evaluation that ' ...
         'checks it; it took %d iteration(s) and %d evaluation(s)'], ...
        inDef.aimIter,inDef.aimEval);
    assert(inDef.psiLaunch == inDef.psiSeed, ...
        ['the shipped launch azimuth must be the SEED azimuth TO THE BIT: ' ...
         '%.17g against %.17g. It is not "solved to the closed-form value" ' ...
         'here; it IS the closed-form value, returned unmoved, and an equality ' ...
         'is the only assertion that says so'],inDef.psiLaunch,inDef.psiSeed);
    assert(inDef.seedMissM == inDef.missM, ...
        ['the shipped seed miss and the achieved miss must be ONE number: ' ...
         '%.17g against %.17g'],inDef.seedMissM,inDef.missM);
...
%% of a run against the returned one had no way to tell which was wrong. Stage 2
%% adds its own evaluations and the one re-flight of the answer:
    assertAbs(summaryNumber(outDef,'propagations +(\d+) +\(whole'), ...
        inDef.nProp,0.5,'printed propagation count against info.nProp');
    assert(inDef.propOne == inDef.maxRange.nEval + 1 + ...
                            inDef.depressed.nProp + inDef.lofted.nProp + ...
                            inDef.minEnergy.nProp, ...
        ['info.propOne is %d against %d bracketing propagations, one re-flying ' ...
         'the maximum, %d and %d on the two full-burn branches and %d on the ' ...
         'minimum-energy solve'], ...
        inDef.propOne,inDef.maxRange.nEval,inDef.depressed.nProp, ...
        inDef.lofted.nProp,inDef.minEnergy.nProp);
    assert(inDef.nProp == inDef.propOne + inDef.aimEval + 1, ...
        ['info.nProp is %d against %d spent before the two-axis solve, %d in ' ...
         'it and one re-flying its answer'], ...
        inDef.nProp,inDef.propOne,inDef.aimEval);
    assert(inDef.nProp == 733, ...
        ['the shipped minimum-energy run cost %d propagations against the 733 ' ...
         'measured on the committed code. THE COST IS THE HEADLINE NUMBER OF ' ...
         'THIS MODE and it is pinned so that a change to it has to be a ' ...
         'decision: 731 of them are the stage-1 minimisation, and the two-axis ' ...
         'solve adds only the one evaluation that finds its seed already ' ...
         'converged plus the one that re-flies the answer'],inDef.nProp);
...
%% ---------------------------------------------------------------------
%% The SEED azimuth must BE the closed-form bearing, and on this configuration
%% the solved one is the seed unmoved, so the flown heading state must be it
%% too. Nothing integrates the initial condition, so these are equalities and
%% not tolerances -- 1e-13 rad is 6e-7 m of arc at Earth radius, far below any
%% physical scale here. THE THIRD ASSERTION IS THE ONE THAT MATTERS AFTER
%% 08/08/2026: the heading in the launch state must be the SOLVED azimuth and
%% not the seed, which on this run are the same number and on a rotating run are
%% 5.6 deg apart:
    ...
    assertAbs(trDef.x(1,6),inDef.psiLaunch,1e-13, ...
        'flown initial heading against the SOLVED launch azimuth (rad)');
...
%% misled, and the script says so at length precisely so it cannot be:
    ...
    assert(contains(outDef,'THE AZIMUTH IS SOLVED, NOT CLOSED FORM'), ...
        ['the summary must state that the closed-form bearing is only the SEED. ' ...
         'It used to say the azimuth was exact only for a non-rotating Earth, ' ...
         'which stopped being true on 08/08/2026']);
    assert(contains(outDef,'the target never slides east under the vehicle'), ...
        ['the summary must retract the moving-target mechanism explicitly. The ' ...
         'state is Earth-fixed and the speed planet-relative, so rotation ' ...
         'DEFLECTS THE VEHICLE and does not carry the target anywhere, and a ' ...
         'reader who learnt the wrong mechanism from an earlier edition has to ' ...
         'be told it was wrong rather than left to notice the sentence is gone']);
    assert(~contains(outDef,'sweeps') && ~contains(outDef,'east during'), ...
        ['the summary must not quote an eastward ground sweep. That figure -- ' ...
         'omegaE*rI*cos(latTarget)*tFly, hundreds of kilometres -- belongs to ' ...
         'the mechanism the paragraph above it now retracts, and a retracted ' ...
         'mechanism''s number left standing under a corrected sentence is the ' ...
         'harder error of the two to notice']);
    assert(~isfield(inDef,'driftDepM') && ~isfield(inDef,'driftLofM'), ...
        ['info.driftDepM and info.driftLofM are gone and must stay gone, for ' ...
         'the same reason: they are the scale of a mechanism this script no ' ...
         'longer claims. The scale of the rotation effect is the DEFLECTION, ' ...
         'and it is in seedMissM and seedXTrackM, measured rather than derived']);
    assert(contains(outDef,'measured at the SEED azimuth') || ...
           contains(outDef,'flown at the SEED'), ...
        ['the summary must say that everything stage 1 certifies -- the hump, ' ...
         'the maximiser interval, the branch brackets and the reachable band ' ...
         '-- was measured at ONE azimuth and is indicative for the solved one']);
...
%% ---------------------------------------------------------------------
    ...
    assert(contains(outLof,'(nominal)') && ~contains(outLof,'targeting: REFUSED'), ...
        'the lofted request did not solve. Summary was:\n%s',outLof);
    assert(contains(outDep,'(nominal)') && ~contains(outDep,'targeting: REFUSED'), ...
...
%% feasible family because the full burn OVERSHOOTS the target there:
    ...
    assert(contains(outFar2,'(nominal)') && ~contains(outFar2,'targeting: REFUSED'), ...
...
%% and a different thing to tell the user to change:
   [outNear,infNear,threwNear] = tryRun(struct('latTarget',47,'lonTarget',-98, ...
    ...
    assert(contains(outNear,'targeting: REFUSED') && contains(outNear,'TOO CLOSE'), ...
...
%% asked for:
  [outBr,infBr,threwBr,trBr] = tryRun(struct('lonTarget',-60, ...
    ...
    assert(contains(outBr,'targeting: REFUSED'),'no refusal banner for an unreachable branch:\n%s',outBr);
...
%% so that raising the clamp cannot be done quietly:
             out12 = evalc(['[~,in12] = run_ballistic_target(struct(' ...
                            ...
                            '''showPlots'',false));']);
    assert(~in12.refused && ~contains(out12,'targeting: REFUSED'), ...
...
%% because a sentinel nothing exercises is a sentinel that can rot:
    ...
    assert(contains(outAut,'(nominal)') && ~contains(outAut,'targeting: REFUSED'), ...
...
%% ---------------------------------------------------------------------
%% 12. A ROTATING EARTH IS FLOWN, not refused -- and the deflection is measured
%% ---------------------------------------------------------------------
%% earthSpin true used to be REFUSED outright, and before that it ran to
%% completion and printed three statements that contradicted each other: a
%% limitations paragraph reporting rotation ON and then that the initial bearing
%% was the WHOLE answer, a cross-track WARNING telling the user to set a bank
%% angle that was already zero, and a miss printed in the same format as a
%% converged solution. The closed-form bearing is still not the answer on a
%% turning Earth -- what changed on 08/08/2026 is that it is now only the SEED,
%% and coorbital.util.aimSolve closes the rest.
%%
%% THE MECHANISM MATTERS AND IT IS PINNED HERE. Rotation does NOT carry the
%% target east under the vehicle: the integrated state is planet-relative and
%% the target is a ground point, so both live in the rotating frame and neither
%% moves in it. Rotation DEFLECTS THE VEHICLE, through the Coriolis and
%% centrifugal terms. The two mechanisms predict different misses and the
%% decomposition below is what tells them apart -- a moving target is a
%% DOWN-RANGE effect on an easterly course, a deflected vehicle an overwhelming
%% CROSS-RANGE one -- so the ratio is asserted rather than the total:
  [outSpn,infSpn,threwSpn,trSpn] = tryRun(struct('earthSpin',true,'showPlots',false));
    assert(~threwSpn,'earthSpin true must not throw:\n%s',outSpn);
    assert(~contains(outSpn,'targeting: REFUSED') && ~infSpn.refused, ...
        ['earthSpin true must now be FLOWN. It was refused until 08/08/2026 ' ...
         'because the azimuth was a closed form that a turning Earth ' ...
         'invalidates; the azimuth is solved now. Summary was:\n%s'],outSpn);
    assert(~isempty(trSpn),'the rotating run must return a trajectory');
    assert(infSpn.stopOK,'every phase of the rotating run must end as intended');

%% THE SEED MISS IS THE OLD BEHAVIOUR, MEASURED. It is what the closed-form
%% bearing and the stage-1 range control produce on their own, which is exactly
%% what this script used to print as an answer, and it is now a returned number
%% rather than a memory in a comment:
    assertRel(infSpn.seedMissM   , 463211.192195506  ,1e-4,'rotating seed miss (m)');
    assertRel(infSpn.seedDownM   ,-17764.1661697878  ,1e-4,'rotating seed miss, down-range (m)');
    assertRel(infSpn.seedXTrackM , 462870.438649385  ,1e-4,'rotating seed miss, cross-range (m)');
    assert(abs(infSpn.seedXTrackM) > 20.*abs(infSpn.seedDownM), ...
        ['the rotating seed miss is %.1f m cross-range against %.1f m ' ...
         'down-range, a ratio of only %.1f. THIS RATIO IS THE EVIDENCE FOR THE ' ...
         'MECHANISM: a target carried east under a fixed heading would be a ' ...
         'down-range effect, and a deflected vehicle is a cross-range one. A ' ...
         'ratio near unity would mean the retracted explanation was the right ' ...
         'one after all'],infSpn.seedXTrackM,infSpn.seedDownM, ...
        abs(infSpn.seedXTrackM)./abs(infSpn.seedDownM));

%% ...AND THE SOLVE REMOVES IT. This is the assertion the whole change exists
%% for: a 463 km miss closed to metres by aiming off, at a cost of a handful of
%% propagations. Pinned as a budget on the miss and as literals on what it took,
%% because a solve that converged by loosening something would satisfy the first
%% and not the second:
    assert(infSpn.missM <= tolM, ...
        ['the rotating run missed by %.2f m against a %.1f m tolerance; the ' ...
         'two-axis solve did not close the deflection'],infSpn.missM,tolM);
    assertRel(infSpn.missM   ,52.4611212230006  ,1e-3,'rotating achieved miss (m)');
    assertRel(infSpn.downM   ,52.2052598454043  ,1e-3,'rotating miss, down-range (m)');
    assertRel(infSpn.xTrackM , 5.17494777250857 ,1e-3,'rotating miss, cross-range (m)');
    assertRel(rad2deg(infSpn.dPsiAim),-5.62952996492772,1e-4, ...
        'rotating azimuth correction off the seed (deg)');
    assert(infSpn.aimEval == 7 && infSpn.aimIter == 2, ...
        ['the rotating solve took %d evaluation(s) in %d iteration(s) against ' ...
         'the 7 in 2 measured on the committed code. A clean aimSolve iteration ' ...
         'costs exactly three propagations, plus the one that evaluates the ' ...
         'seed'],infSpn.aimEval,infSpn.aimIter);
    assert(infSpn.nProp == infSpn.propOne + infSpn.aimEval + 1, ...
        'the rotating run''s propagation count does not decompose');
    assertRel(infSpn.nProp,701,1e-9,'rotating total propagations');

%% THE SOLVED AZIMUTH IS NOT THE SEED, which is the one thing a mutation putting
%% the closed form back would break while leaving every branch assertion in this
%% file intact:
    assert(abs(rad2deg(wrapPi(infSpn.psiLaunch - infSpn.psiSeed))) > 1, ...
        ['the rotating run flew the SEED azimuth: %.9f deg solved against ' ...
         '%.9f deg seeded. The whole point of the two-axis solve is that these ' ...
         'differ on a turning Earth'], ...
        rad2deg(infSpn.psiLaunch),rad2deg(infSpn.psiSeed));
    assertAbs(trSpn.x(1,6),infSpn.psiLaunch,1e-13, ...
        ['the flown initial heading must be the SOLVED azimuth (rad), not the ' ...
         'seed the launch-state template carries']);

%% ...and independently, from the flown state rather than from the bookkeeping:
            missIS = c.rE.*coorbital.util.greatCircle(trSpn.x(end,3), ...
                                                      trSpn.x(end,2),latTg,lonTg);
    assert(missIS <= tolM, ...
        'the rotating impact point is %.2f m from the target, independently measured', ...
        missIS);
    assertAbs(missIS,infSpn.missM,1e-6, ...
        'independently measured rotating miss against the reported one (m)');

%% THE MINIMISED ENERGY AND THE FLOWN ENERGY ARE TWO DIFFERENT NUMBERS HERE, and
%% keeping them apart is the whole of a correction made on 08/08/2026. Stage 2
%% moves the CUTOFF FRACTION to close the cross-range, at the loft angle the
%% minimisation settled on, so the vehicle leaves burnout with an energy that is
%% NOT the minimum of anything -- the minimum was found at the seed azimuth.
%% Both are reported; neither may stand in for the other. This first ran with
%% the flown value printed in a column headed MINIMISED and differenced against
%% neighbours measured at the other azimuth, which understated the far side of
%% the valley by 47 per cent and printed a noise ratio that did not close
%% against its own printed numbers.
%%
%% NOTHING IN THE STILL-EARTH PARTS CAN SEE THIS, because there the aim solve is
%% a no-op and all four numbers share one basis. That is why these assertions
%% are here and not in part 6:
             meSpn = infSpn.minEnergy;
    assert(meSpn.dCutFrac ~= 0, ...
        ['the rotating run''s cutoff did not move in stage 2, so this case ' ...
         'cannot see a two-basis error at all and the assertions below are ' ...
         'vacuous']);
    assertRel(meSpn.seedEpsBo,-45977595.2095501,1e-6, ...
        'the MINIMISED burnout energy, at the seed azimuth (J/kg)');
    assertRel(meSpn.seedCutFrac,0.992294311523438,1e-6, ...
        'the MINIMISED cutoff fraction, at the seed azimuth (-)');
    assertRel(meSpn.epsBo,-45924474.8804371,1e-6, ...
        'the FLOWN burnout energy, after the aim solve moved the cutoff (J/kg)');
    assertRel(meSpn.cutFrac,0.99262145976272,1e-6, ...
        'the FLOWN cutoff fraction (-)');
    assert(meSpn.epsBo > meSpn.seedEpsBo, ...
        ['the flown burnout energy %.6f MJ/kg is not ABOVE the minimised ' ...
         '%.6f MJ/kg. It has to be: the minimum was found at the seed azimuth ' ...
         'and stage 2 moved off it to close the cross-range, so the flown ' ...
         'point cannot be lower without the minimisation having been wrong'], ...
        meSpn.epsBo./1e6,meSpn.seedEpsBo./1e6);

%% THE VALLEY ARITHMETIC MUST CLOSE AGAINST THE MINIMISED VALUE, and against the
%% depth the ratio actually divides by. Both of these fail on the code as first
%% written:
    assertAbs(meSpn.epsDepthJkg, ...
              min(meSpn.epsNeighLo,meSpn.epsNeighHi) - meSpn.seedEpsBo,1e-6, ...
        ['the reported valley depth against the shallower of the two ' ...
         'neighbour differences taken from the MINIMISED energy (J/kg)']);
    assertRel(meSpn.epsNoiseRel,meSpn.epsNoiseJkg./meSpn.epsDepthJkg,1e-12, ...
        ['the printed noise ratio against noise over the printed depth. If ' ...
         'these disagree the summary''s own arithmetic does not close']);
    assert(meSpn.epsNeighLo > meSpn.seedEpsBo && meSpn.epsNeighHi > meSpn.seedEpsBo, ...
        ['the two neighbouring feasible points must both lie ABOVE the ' ...
         'MINIMISED energy: %.6f and %.6f against %.6f MJ/kg'], ...
        meSpn.epsNeighLo./1e6,meSpn.epsNeighHi./1e6,meSpn.seedEpsBo./1e6);

%% ...and the summary must SHOW both columns and say which is which, because the
%% struct being right is no use to a reader of the page:
    assert(contains(outSpn,'MINIMISED (seed)') && contains(outSpn,'FLOWN (solved)'), ...
        ['the minimum-energy table must carry two labelled columns on a run ' ...
         'where the aim solve moved:\n%s'],outSpn);
    assert(contains(outSpn,'THE TWO COLUMNS ARE DIFFERENT OPERATING POINTS'), ...
        ['the summary must say in words that the flown column is not a ' ...
         'minimum:\n%s'],outSpn);
    assert(contains(outSpn,'FLOWN COLUMN IS NOT A MINIMUM OF ANYTHING'), ...
        'the summary must refuse to call the flown energy minimised:\n%s',outSpn);

%% ON THE SHIPPED RUN THE TWO COLLAPSE TO ONE, exactly, and the summary says so
%% rather than printing two identical columns without comment:
    assert(inDef.minEnergy.dCutFrac == 0 && inDef.minEnergy.dEpsBoJkg == 0, ...
        ['the shipped run''s aim solve is a no-op, so the minimised and flown ' ...
         'operating points must be the SAME to the bit; they differ by %g of ' ...
         'the burn and %g J/kg'],inDef.minEnergy.dCutFrac, ...
        inDef.minEnergy.dEpsBoJkg);
    assert(inDef.minEnergy.seedEpsBo == inDef.minEnergy.epsBo, ...
        'the shipped minimised and flown burnout energies must be one number');
    assert(contains(outDef,'THE TWO COLUMNS ARE THE SAME RUN'), ...
        ['the shipped summary must say the two columns coincide, rather than ' ...
         'leaving a reader to compare them digit by digit:\n%s'],outDef);

%% THE ENVELOPE MOVES WITH THE AZIMUTH, and this is where that stops being an
%% abstraction. Stage 1 measures the hump, the branch brackets and the reachable
%% band at ONE azimuth, and on this north-easterly geometry turning the Earth on
%% raises the maximum range and lifts the depressed band clean off the shipped
%% target -- so 'depressed' is legitimately REFUSED there while the other two
%% modes fly. That is physics, not an artefact: an easterly launch gains from
%% the planet's rotation. It is pinned because a reader who saw the refusal
%% without this test would suspect the two-axis solve:
    assert(infSpn.rngMaxM > inDef.rngMaxM + 1e5, ...
        ['turning the Earth on must RAISE the maximum range on this easterly ' ...
         'geometry: %.3f km rotating against %.3f km still'], ...
        infSpn.rngMaxM./1000,inDef.rngMaxM./1000);
    assertRel(infSpn.rngMaxM,5439856.66,1e-4,'rotating maximum range (m)');
  [outSpD,infSpD,threwSpD,trSpD] = tryRun(struct('earthSpin',true, ...
                                                 'branch','depressed', ...
                                                 'showPlots',false));
    assert(~threwSpD,'the rotating depressed request must refuse, not throw:\n%s',outSpD);
    assert(infSpD.refused && strcmp(infSpD.refusedWhy,'envelope') && isempty(trSpD), ...
        ['the rotating depressed request must refuse through the ENVELOPE path: ' ...
         'the depressed band floor rises above the shipped target once the ' ...
         'Earth turns. Got refused = %d, why = "%s"'], ...
        infSpD.refused,infSpD.refusedWhy);
    assert(infSpD.rngReqM < infSpD.depressed.bandLo, ...
        ['the rotating depressed band must start ABOVE the required %.3f km, ' ...
         'or this case is refusing for some other reason'],infSpD.rngReqM./1000);

%% ...and the LOFTED arc does still reach it, and is also solved rather than
%% aimed on the closed form:
  [outSpL,infSpL,threwSpL,trSpL] = tryRun(struct('earthSpin',true, ...
                                                 'branch','lofted', ...
                                                 'showPlots',false));
    assert(~threwSpL && ~infSpL.refused && ~isempty(trSpL), ...
        'the rotating lofted request must solve. Summary was:\n%s',outSpL);
    assert(infSpL.missM <= tolM, ...
        'the rotating lofted arc missed by %.2f m',infSpL.missM);
    assert(abs(rad2deg(wrapPi(infSpL.psiLaunch - infSpL.psiSeed))) > 1, ...
        'the rotating lofted run flew the seed azimuth');
    assertRel(infSpL.seedMissM,536047.549759252,1e-4,'rotating lofted seed miss (m)');
    assert(strcmp(infSpL.branchMeasured,'lofted') && infSpL.branchAgrees, ...
        ['the rotating lofted arc measured as "%s". The two-axis solve moves ' ...
         'the LOFT ANGLE as well as the azimuth in this mode, and a step that ' ...
         'crossed the max-range hump would land on the other branch -- which is ' ...
         'what the maxStep cap on the loft is there to prevent'], ...
        infSpL.branchMeasured);

%% THE THREE CONTRADICTIONS THE OLD ROTATING RUN PRINTED MAY NOT COME BACK. Each
%% is asserted as the string it was, because each was printed and each was read:
    assert(~contains(outSpn,'WHOLE answer'), ...
        ['the summary must not claim the closed-form bearing is the whole ' ...
         'answer on a rotating Earth:\n%s'],outSpn);
    assert(~contains(outSpn,'WARNING'), ...
        ['the rotating run must not print a cross-track warning blaming a bank ' ...
         'angle that is zero. The cross-range is SOLVED now:\n%s'],outSpn);
    assert(contains(outSpn,'Earth rotation ON'), ...
        'the rotating summary must state that Earth rotation is on:\n%s',outSpn);
    assert(contains(outSpn,'IT IS MODELLED ON THIS RUN'), ...
        ['the rotating summary must say the deflection is modelled and quote ' ...
         'the measured size of it:\n%s'],outSpn);

%% ...and the surviving prose in a NON-rotating run says so, and says why the
%% still-Earth case is the simple one rather than the only one:
    ...
    assert(contains(outDef,'earthSpin'), ...
        ['the closing block must name the switch that decides whether the ' ...
         'deflection is modelled:\n%s'],outDef);
...
%% rather than asserted about, or the exemption is only a claim:
           outNoSep = evalc(['[~,inNoSep] = run_ballistic_target(struct(' ...
                             ...
                             '''showPlots'',false));']);
    assert(~inNoSep.refused && ~contains(outNoSep,'targeting: REFUSED'), ...
...
%% Compared on seedEpsBo and NOT on epsBo, because seedEpsBo is the MINIMISED
%% quantity and epsBo is what was flown after the aim solve moved the cutoff.
%% They are the same number on both of these still-Earth runs, which is exactly
%% why the wrong one would pass here and has to be got right on principle:
    assert(inTight.minEnergy.seedEpsBo <= inDef.minEnergy.seedEpsBo + 1e-6, ...
        ['the tightened run minimised to %.9f MJ/kg against the shipped run''s ' ...
         '%.9f MJ/kg. A finer search of the SAME objective cannot end up ' ...
         'higher'],inTight.minEnergy.seedEpsBo./1e6, ...
        inDef.minEnergy.seedEpsBo./1e6);

%% ---------------------------------------------------------------------
%% 15. THE TWO-AXIS SOLVE'S OWN REFUSAL, and the guard that feeds it
%% ---------------------------------------------------------------------
%% THIS PART EXISTS BECAUSE THE FIRST VERSION OF IT DID NOT. The aim-solve
%% refusal was written and shipped untested on the belief that a non-convergence
%% could not be provoked without contrivance. It can: near the top of the
%% reachable band a rotating run has to aim off several degrees AND sit on a
%% range control whose sensitivity is collapsing towards the max-range hump, and
%% the damped Newton runs out of road. 5409.869 km at 62 N 16.4 W does it.
  [outAim,infAim,threwAim,trAim] = tryRun(struct('earthSpin',true, ...
                                                 'branch','lofted', ...
                                                 'lonTarget',-16.4, ...
                                                 'showPlots',false));
    assert(~threwAim, ...
        ['a two-axis solve that cannot converge must REFUSE, not throw. ' ...
         'coorbital.util.aimSolve returns converged = false with the best ' ...
         'point it reached precisely so the caller can refuse in its own ' ...
         'vocabulary:\n%s'],outAim);
    assert(infAim.refused && strcmp(infAim.refusedWhy,'aimSolve'), ...
        'expected an aimSolve refusal; got refused = %d, why = "%s"', ...
        infAim.refused,infAim.refusedWhy);
    assert(isempty(trAim), ...
        ['a refused run must return an EMPTY trajectory. A near miss handed ' ...
         'back with a trajectory, a loft angle and a summary is the failure ' ...
         'mode this whole file exists to prevent: it LOOKS like a solution']);
    assert(strcmp(infAim.aimInfo.identifier,'coorbital:aimSolve:stepStalled'), ...
        'expected the step-stalled refusal from aimSolve, got "%s"', ...
        infAim.aimInfo.identifier);
    assertRel(infAim.missM,12715.0195657149,1e-3, ...
        'best miss the stalled two-axis solve reached (m)');
    assert(infAim.missM > tolM, ...
        ['the "refused" case missed by only %.2f m, inside the %.1f m ' ...
         'tolerance -- it converged and this part is testing nothing'], ...
        infAim.missM,tolM);
    assert(infAim.nProp == infAim.propOne + infAim.aimEval, ...
        ['a refusal costs stage 1 plus the Newton and NOT the re-flight, which ' ...
         'never happens: %d against %d + %d'], ...
        infAim.nProp,infAim.propOne,infAim.aimEval);

%% THE BANNER IS WHAT A USER ACTS ON, so it is asserted as text and not only as
%% struct fields. ctlWord's full-burn branch is exercised here -- "best loft
%% angle" -- and its minimum-energy branch below:
    assert(contains(outAim,'targeting: REFUSED'), ...
        'the stalled solve printed no refusal banner:\n%s',outAim);
    assert(contains(outAim,'best loft angle'), ...
        ['the banner must name the second control the solve was moving, which ' ...
         'for a full-burn branch is the LOFT ANGLE:\n%s'],outAim);
    assert(contains(outAim,'seed azimuth') && contains(outAim,'best azimuth'), ...
        'the banner must show where the Newton started and where it got to:\n%s',outAim);
    assert(contains(outAim,'solver said'), ...
        'the banner must quote aimSolve''s own identifier:\n%s',outAim);
    assert(contains(outAim,'THE BEST POINT REACHED IS NOT AN ANSWER'), ...
        ['the banner must say in words that the best point is not being ' ...
         'returned as a solution:\n%s'],outAim);
    assert(contains(outAim,'flown at the SEED azimuth'), ...
        ['the refusal must repeat the seed-azimuth caveat on the reachable ' ...
         'band it quotes:\n%s'],outAim);

%% ...AND THE cutoffAboveFullBurn GUARD IS EXERCISED, which is the other thing
%% shipped untested. Ask the SAME geometry for minimum-energy and the two-axis
%% solve drives the cutoff fraction to 0.999991 of the full burn; the
%% finite-difference probe a step above that asks for more burn than the booster
%% carries, flyMiss refuses it, and aimSolve reports jacobianFailed with the
%% underlying message quoted. That is the guard doing exactly its job -- the
%% alternative, clamping to 1, would have silently handed the solver a FULL-BURN
%% trajectory for a control it did not ask for and flattened the Jacobian column
%% with nothing in the record to say why:
  [outCut,infCut,threwCut,trCut] = tryRun(struct('earthSpin',true, ...
                                                 'lonTarget',-16.4, ...
                                                 'showPlots',false));
    assert(~threwCut,'the cutoff-guard case must refuse, not throw:\n%s',outCut);
    assert(infCut.refused && strcmp(infCut.refusedWhy,'aimSolve') && isempty(trCut), ...
        'expected an aimSolve refusal with an empty trajectory; why = "%s"', ...
        infCut.refusedWhy);
    assert(infCut.cutFrac > 0.9999 && infCut.cutFrac <= 1, ...
        ['this case only exercises the guard if the cutoff is driven hard up ' ...
         'against the full burn; it reached %.9f'],infCut.cutFrac);
    assert(infCut.aimInfo.nInfeasible >= 1, ...
        ['aimSolve recorded %d infeasible evaluations. The guard in flyMiss is ' ...
         'what makes a cutoff past the full burn infeasible rather than ' ...
         'silently full-burn, and if nothing was infeasible here the guard was ' ...
         'never reached'],infCut.aimInfo.nInfeasible);
    assert(contains(outCut,'asks for more burn than the booster carries'), ...
        ['the guard''s own message must reach the user. aimSolve quotes the ' ...
         'underlying error on the jacobianFailed path alone, and this is the ' ...
         'only way a reader learns why the solve stopped:\n%s'],outCut);
    assert(contains(outCut,'best cutoff'), ...
        ['ctlWord''s MINIMUM-ENERGY branch must name the cutoff fraction as ' ...
         'the second control, and say the loft angle was held:\n%s'],outCut);
    assert(contains(outCut,'at the held'), ...
        'the banner must say the loft angle was held in this mode:\n%s',outCut);

%% ---------------------------------------------------------------------
%% 16. A BANK NEEDS loftMin RAISED WITH IT, and that is REFUSED not thrown
%% ---------------------------------------------------------------------
%% The user block now says a non-zero bank is supported, because the two-axis
%% solve absorbs the cross-range one produces. That is true and part of it is
%% flown below -- but at the SHIPPED loftMin of -140 deg it is unreachable, and
%% it used to be unreachable by way of an unhandled library identifier out of
%% coorbital.guide.pitchProgram. A bank tilts the lift vector out of the
%% vertical plane, so less of it is left to push the nose over, and the deepest
%% commanded attitude in the bracket stops being deliverable at the vehicle's
%% 6 deg clamp. Even half a degree of bank is enough. A comment inviting a user
%% into a raw library throw is worse than no comment, so it is a refusal:
  [outBnk,infBnk,threwBnk,trBnk] = tryRun(struct('bankAngle',2,'showPlots',false));
    assert(~threwBnk, ...
        ['a bank the shipped loft bracket cannot be flown across must REFUSE, ' ...
         'not throw coorbital:pitchProgram:unreachableAttitude at the ' ...
         'user:\n%s'],outBnk);
    assert(infBnk.refused && strcmp(infBnk.refusedWhy,'loftBracket') && isempty(trBnk), ...
        'expected a loftBracket refusal with an empty trajectory; why = "%s"', ...
        infBnk.refusedWhy);
    assert(strcmp(infBnk.libraryErr,'coorbital:pitchProgram:unreachableAttitude'), ...
        ['the refusal must hand back the library identifier it translated, so ' ...
         'a caller can tell this apart from a targeting failure; got "%s"'], ...
        infBnk.libraryErr);
    assert(contains(outBnk,'RAISE loftMin'), ...
        ['the refusal must name the entry that has to move, not merely report ' ...
         'that something is unreachable:\n%s'],outBnk);
    assert(contains(outBnk,'THE TWO-AXIS SOLVE') && ...
           contains(outBnk,'HANDLES THE CROSS-RANGE'), ...
        ['the refusal must say what it is NOT about. A user who reads it as ' ...
         '"banked runs are not supported" has been told the opposite of the ' ...
         'truth:\n%s'],outBnk);
    assert(~isfield(infBnk,'rngMaxM'), ...
        ['the loftBracket refusal fires on the FIRST propagation of the coarse ' ...
         'scan, so it must not hand back a maximum range -- if it did, the ' ...
         'whole bracketing was paid for before the failure']);

%% ...and half a degree is genuinely enough, which is what makes the shipped
%% [-90 .. 90] range in the old comment a promise the script could not keep:
  [~,infBnkS,threwBnkS] = tryRun(struct('bankAngle',0.5,'showPlots',false));
    assert(~threwBnkS && infBnkS.refused && strcmp(infBnkS.refusedWhy,'loftBracket'), ...
        'even 0.5 deg of bank must refuse at the shipped loftMin, and refuse cleanly');

%% THE CAPABILITY CLAIM ITSELF, FLOWN. With the companion change the refusal
%% names -- loftMin raised to -60 deg -- a banked run solves, and the azimuth is
%% aimed off by degrees to cancel the cross-range the bank produces. This is the
%% assertion that makes "the SOLVE NOW SEES THAT" in the user block a measured
%% statement rather than an untested one:
  [outBk2,infBk2,threwBk2,trBk2] = tryRun(struct('bankAngle',2,'loftMin',-60, ...
                                                 'showPlots',false));
    assert(~threwBk2 && ~infBk2.refused && ~isempty(trBk2), ...
        ['a 2 deg bank at loftMin = -60 deg must FLY. If it does not, the ' ...
         'capability the user block claims does not exist. Summary was:\n%s'], ...
        outBk2);
    assert(infBk2.missM <= tolM, ...
        'the 2 deg banked run missed by %.2f m against %.1f m',infBk2.missM,tolM);
    assertRel(infBk2.missM,1.59441067717672,1e-2,'2 deg banked miss (m)');
    assertRel(rad2deg(infBk2.dPsiAim),3.27068208175369,1e-3, ...
        '2 deg banked azimuth correction off the seed (deg)');
            xRatBk = abs(infBk2.seedXTrackM)./max(abs(infBk2.seedDownM),eps);
    assert(xRatBk > 100 || abs(infBk2.seedXTrackM) > 1e5, ...
        ['the bank must produce a miss that is overwhelmingly CROSS-range ' ...
         '(%.1f m of %.1f m, a ratio of %.1f); if it were not, the azimuth ' ...
         'would not be the control that fixes it'], ...
        infBk2.seedXTrackM,infBk2.seedMissM,xRatBk);
    assertRel(infBk2.seedMissM,250010.791463517,1e-3, ...
        '2 deg banked seed miss, which the solve removes (m)');
    assert(contains(outBk2,'THIS RUN COMMANDS A NON-ZERO BANK'), ...
        ['the bank-specific paragraph in the closing block is UNREACHABLE at ' ...
         'shipped settings, so this is the only run in the file that renders ' ...
         'it. Summary was:\n%s'],outBk2);
