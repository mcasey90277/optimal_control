...
| `orbit_transfer/` | **The main body of work** — optimal orbit-transfer campaigns, one folder per problem, each split into `direct/` + `indirect/` codebases: elliptic→GEO (HMG-2004 benchmark, certified 10→0.1 N thrust ladder), GTO→tulip and GTO→ELFO (Earth–Moon CR3BP), a shared `cr3bp_common/` library, Lambert + min-energy tutorials, and the source papers; plus the **costate-catalog program** (`DRO_tulip`, `HALO_tulip`, `DPO_tulip`, `HALO_HALO` on the shared `costate_common/` core — ~15,900 accepted min-time PMP entries for pumpkyn's `tfMin`, and since 2026-08-14 the first fixed-t_f min-energy pilot). Start at `orbit_transfer/README.md`. |
| `optimal_control_examples/` | Standalone direct-collocation tutorials (Kelly-style): `ex1_block_move` (minimum-energy point-to-point) and `ex2_cart_pole_swing_up` (underactuated swing-up), trapezoidal transcription via `fmincon`. |
