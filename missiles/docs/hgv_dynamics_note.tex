...
of the horizon basis relative to inertial space and using \eqref{eq:dvhat},
\begin{equation}
  \frac{\dd\vhat}{\dd t}
...
For a non-rotating planet the horizon basis turns only because the vehicle
moves over it. Increasing longitude rotates the basis about the polar axis at
...
\begin{align*}
  ...
  \text{liftoff } T/W &= \frac{950000}{32400\times 9.80665} = 2.9899 .
\end{align*}
...
\paragraph{The range.}
For the range integral take $\gamma$ small, so $\cos\gamma\to 1$, and write
\eqref{eq:eqglide} as
\begin{equation}
  L = mg\Bigl(1 - \frac{V^2}{V_c^2}\Bigr), \qquad V_c^2 = gr ,
...
\end{equation}
Substitute $u = V^2/V_c^2$, so $V\,\dd V = \tfrac12 V_c^2\,\dd u$ and, crucially,
$V_c^2/g = r$:
\begin{equation}
  \dd s = -\frac{(L/D)}{g}\cdot\frac{V_c^2}{2}\cdot\frac{\dd u}{1-u}
        = -\frac{(L/D)\,r}{2}\cdot\frac{\dd u}{1-u} .
  \label{eq:dsdu}
\end{equation}
...
\paragraph{Physical reading.} The logarithm is the interesting part. Range is
not proportional to kinetic energy; it diverges as $V\to V_c$, because at
circular speed the vehicle needs no lift, generates no induced drag through the
$L/D$ relation, and does not come down. Conversely the last increment of speed
buys very little range. With $L/D = 2.5$ and $V = 6000~\mathrm{m/s}$
($V_c = 7905.4~\mathrm{m/s}$), \eqref{eq:sanger} predicts $6842~\mathrm{km}$;
...
\emph{meridional cardinals}: a due-north leg, $(0,0)\to(0.1,0)$, correctly
returns $0.0000^{\circ}$ and returns $90.0000^{\circ}$ transposed. The
east--west cardinals survive the swap. So the assertion to write is an absolute
bearing on a \textbf{near-meridional} leg, not a reverse-course offset.

\paragraph{Three degenerate cases, all with conventions rather than answers.}
Coincident points return zero, because $\operatorname{atan2}(0,0)$ is defined
as zero --- a convention that keeps a degenerate input from poisoning a state
vector, not an answer. Antipodal points admit \emph{every} great circle, so the
returned value is an artifact of how two vanishing terms round: from $(0,0)$ to
$(0,\pi)$ the function returns exactly $90^{\circ}$, and nudging either
latitude by $10^{-9}$~rad swings the answer across the entire circle. And at
the poles every direction is south. None of the three is wrong; all three are
places where a caller must not treat the output as meaningful.
...
is computed from the flown state by the standard spherical relation
\begin{equation}
  x_{\text{track}} = r_I \arcsin\bigl(\sin\Delta\theta\;\sin\delta\psi\bigr),
  \label{eq:xtrack}
\end{equation}
where $\Delta\theta$ is the flown central angle and
$\delta\psi = \psi_{\text{flown}} - \psi_0$ the bearing error at the launch
point, and it is printed every run with a warning when it exceeds the range
tolerance. Measured:
