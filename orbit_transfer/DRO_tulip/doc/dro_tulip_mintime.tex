...
independent second opinion exists. Three lessons came out. First, the problem as
ordinarily posed is \textbf{ill-posed}: without a minimum lunar altitude
constraint, ``the minimum time'' is not a number but a family parameterized by
flyby depth, bounded below only by the lunar surface. Second, adding that path
constraint costs one block of algebraic inequalities in the direct formulation
against a multipoint boundary-value problem with costate jumps in the indirect
one---a concrete asymmetry between the methods. Third, and most cautionary: the
mesh-dependent solutions that appeared to \emph{beat} the reference minimum time
are not better optima but quadrature error being optimized against. Their
trapezoidal defects are $10^{-14}$ while their true local error at periselene is
$O(1)$, a discrepancy of seven orders of magnitude, scaling as
altitude$^{-4.2}$. A machine-tight defect is a statement about the
discretization, not about the trajectory.
\end{abstract}
...
\paragraph{A necessary qualification, added after the diagnostics ran.} The
statement above is correct \emph{about the reference trajectory}, and about any
solution with a comparable periselene. It is \textbf{not} correct about the
deep-flyby branch the direct method finds. Panel (c) of the diagnostic figure
measures both notions of mesh spacing on the $442$~km solution: the grid is
uniform in time to a ratio of $1.000$, but the lunar angle swept per interval
varies by a factor of $\mathbf{353}$ between the cruise and the close approach.
The mesh has, in other words, no idea the flyby is happening. That is precisely
the condition a Sundman regularization exists to fix. So the accurate form of
the lesson is: \emph{no regularization is needed for this transfer at a sane
periselene, and one would be needed if a deep flyby were to be resolved
properly.} The original claim was drawn from the reference alone, and the
reference does not visit the regime that breaks it.
